...
int rd_image_to_mesh2(int N_ext, Exo_DB *exo) {
  ...
  /*  int first_time_field = TRUE; // Flag to check whether this is the first time mapping to the
   * specified field */
  ...
  */
  ...
  for (ielem = e_start; ielem < e_end; ielem++) {
    ...
    for (igp = 0; igp < ngp; igp++) // Loop over GP's
    {
      ...
      if (pixinterp) {
        ...
        iy = (int)((yg - y0) / resy);
        iz = (int)((zg - z0) / resz);
        if (ix < 0 || ix >= pixsize[0] || iy < 0 || iy >= pixsize[1] || iz < 0 ||
            iz >= pixsize[2]) {
          nmiss++;
  ...
  if (nmiss) {
    printf("WARNING: Some portions of the mesh/element block being mapped to are located outside "
  ...
  ipix_blkid, si, my_N_ext); printf("Scaled RMSD error was %g %%\n",rmsd_error/(pmax-pmin)*100.0);
  if (pixsize[2] > 1) {
    printf("Warning: For 3D problems with unstructured or tet meshes, this error value may be
  inaccurate (tends to be overestimated)\n");
  }
  */
...
extern double calc_error(double ***pixdata,
  ...
  /* the closest center, which is not true in general. For 3D, we just use a bounding box check
   * here,  */
  ...
  for (ielem = e_start; ielem < e_end; ielem++) {
    ...
    for (ilnode = 0; ilnode < ei[pg->imtrx]->num_local_nodes; ilnode++) {
      ignode = Proc_Elem_Connect[ei[pg->imtrx]->iconnect_ptr + ilnode];
      if (Coor[0][ignode] < xmin)
        xmin = Coor[0][ignode];
      if (Coor[0][ignode] > xmax)
        xmax = Coor[0][ignode];
      if (Coor[1][ignode] < ymin)
        ymin = Coor[1][ignode];
      if (Coor[1][ignode] > ymax)
        ymax = Coor[1][ignode];
      nodecoor[ilnode][0] = Coor[0][ignode];
      nodecoor[ilnode][1] = Coor[1][ignode];
      nodecoor[ilnode][2] = Coor[2][ignode];
      if (pixsize[2] > 1) {
        if (Coor[2][ignode] < zmin)
          zmin = Coor[2][ignode];
        if (Coor[2][ignode] > zmax)
          zmax = Coor[2][ignode];
      }
    }
        ...
        for (k = 0; k < pixsize[2]; k++) {
          ...
          if (pixsize[2] > 1) {
            ...
            for (iside = 0; iside < ei[pg->imtrx]->num_sides; iside++) {
              ...
              if (r_cross_u == 0)
                continue; // Unlikely case that ray is perfectly parallel to an element edge
              n1p_x = n1x - px;
              n1p_y = n1y - py;
          ...
          for (ilnode = 0; ilnode < ei[pg->imtrx]->num_local_nodes; ilnode++) {
            ignode = Proc_Elem_Connect[ei[pg->imtrx]->iconnect_ptr + ilnode];
            phi_i = newshape(xi_data, ei[pg->imtrx]->ielem_type, PSI, ilnode,
                             ei[pg->imtrx]->ielem_shape, efv->i[N_ext], ilnode);
