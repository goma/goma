...
int assemble_spalart_allmaras(dbl time_value, /* current time */
  ...
  calc_shearrate(&S, omega, dS_dvelo, dS_dmesh);
  ...
  double dchi_dmu_e = 1.0/mu;
  double dfv1_dchi =  3.0*chi*chi/(pow(chi,3) + pow(cv1,3))
                    - (pow(chi,3) * 3.0*chi*chi)/(pow(chi,3) + pow(cv1,3))/(pow(chi,3) + pow(cv1,3));
  double dfv1_dmu_e = dfv1_dchi * dchi_dmu_e;
  double dfv2_dmu_e = - dchi_dmu_e/(1.0 + chi*fv1)
                      + (chi/(1.0 + chi*fv1)/(1.0 + chi*fv1)) * (dchi_dmu_e*fv1 + chi*dfv1_dmu_e);
  double dS_e_dmu_e = (fv2 + mu_e*dfv2_dmu_e)/(kappa*kappa*d*d);
  double dr_dmu_e = 0.0;
  if (r < r_max) {
    dr_dmu_e =   1.0/(kappa*kappa*d*d*S_e)
               - (mu_e*kappa*kappa*d*d*dS_e_dmu_e)/(kappa*kappa*d*d*S_e)/(kappa*kappa*d*d*S_e);
  }
  double dg_dr = 1.0 + cw2*(6.0*pow(r,5)-1.0);
  double dg_dmu_e = dg_dr * dr_dmu_e;
  double dfw_inside_dg = -((1.0 + pow(cw3,6))*6.0*pow(g,5))/(pow(g,6) + pow(cw3,6))/(pow(g,6) + pow(cw3,6));
  double dfw_dg = pow(fw_inside, (1.0/6.0)) +
                  ((1.0/6.0) * g * dfw_inside_dg) / (pow(fw_inside, (1.0/6.0)));
  ...
  if (af->Assemble_Residual) {
    ...
    for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
       ...
       diff_2 = 0.0;
       for (int p = 0; p < VIM; p++) {
           diff_1 += bf[eqn]->grad_phi[i][p] * (mu + mu_e) * fv->grad_eddy_mu[p];
    ...
    for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
       ...
       if (pdv[var]) {
          ...
          for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
             ...
             src_1 = cb1 * ( dS_e_dmu_e * bf[eqn]->phi[j] * mu_e  +
                             S_e * bf[eqn]->phi[j] );
             src_2 = (cw1/d/d) * bf[eqn]->phi[j] * ( dfw_dmu_e * mu_e * mu_e +
                                                     fw * 2.0 * mu_e );
             src = -src_1 + src_2;
             src *= bf[eqn]->phi[i] * d_area;
             src *= pd->etm[pg->imtrx][eqn][(LOG2_SOURCE)];
             ...
             for (int p = 0; p < VIM; p++) {
               diff_1 += bf[eqn]->grad_phi[i][p] * ( bf[eqn]->phi[j] * fv->grad_eddy_mu[p] +
                                                     (mu + mu_e) * bf[eqn]->grad_phi[j][p] );
               diff_2 += bf[eqn]->phi[i] * cb2 * 2.0 * fv->grad_eddy_mu[p]
                         * bf[eqn]->grad_phi[j][p];
             }
             diff = (1.0/sigma) * (diff_1 - diff_2);
             diff *= d_area;
