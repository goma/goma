...
INTERFACE_SOURCE_STRUCT *interface_source_alloc(int num_terms, int is_type, int do_Jac)
...
void interface_source_free(INTERFACE_SOURCE_STRUCT *is)
...
void interface_source_destroy(INTERFACE_SOURCE_STRUCT **is_hdl)
...
void interface_source_zero(INTERFACE_SOURCE_STRUCT *is)
 ...
 *********************************************************************/
{
  ...
  if (is->Do_Jac) {
    (void)memset((void *)is->JacMatrix[0], 0, sizeof(double) * is->Num_Terms * is->Num_Terms);
  }
}
/************************************************************************/
/************************************************************************/
...
void is_change1_speciesVT(INTERFACE_SOURCE_STRUCT *is,
...
void is_change1_lastspecies(INTERFACE_SOURCE_STRUCT *is,
                            const int is_species_entry,
                            const int is_species_start,
                            MATRL_PROP_STRUCT *mp_local,
                            const int intf_id)
...
int match_interface_source_string(char *istring)
...
{
  int match = -1;
  if (!strcasecmp(istring, "IS_EQUIL_PSEUDORXN") || !strcasecmp(istring, "is_equil_prxn_bc")) {
  ...
  if (match == -1) {
    fprintf(stderr, "match_interface_source_string ERORR: string %s not recognized\n", istring);
