...
void createEpetraGomaProblemGraph(struct Aztec_Linear_Solver_System *ams, Exo_DB *exo, Dpi *dpi) {
  ...
  for (inode = 0; inode < total_nodes; inode++) {
    ...
    for (iunknown = 0; iunknown < row_num_unknowns; iunknown++) {
      ...
      insertEpetraRowMatrix(ams->RowMatrix, ams->GlobalIDs[irow_index], Indices.size(), &Values[0],
          &Indices[0]);
      nnz += Indices.size();
      irow_index++;
    }
  ...
  fillCompleteEpetraRowMatrix(ams->RowMatrix);
  putScalarEpetraRowMatrix(ams->RowMatrix, 0);
          ...
          for (i = 0; i < dofs; i++) {
            ...
            if (ei->owned_ledof[ledof]) {
              ...
              if (af->Assemble_Jacobian) {
                ...
                sumIntoEpetraRowMatrix(ams->RowMatrix, ams->GlobalIDs[row_index],
        ...
        for (i = 0; i < dofs; i++) {
          ...
          if (ei->owned_ledof[ledof]) {
            ...
            if (af->Assemble_Jacobian) {
              ...
              sumIntoEpetraRowMatrix(ams->RowMatrix, ams->GlobalIDs[row_index], Indices.size(),
                  &Values[0], &Indices[0]);
              Indices.clear();
              Values.clear();
            }
