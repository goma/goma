  ...
  if ((EQN == R_LUBP) || (EQN == R_LUBP_2)) {
    ...
    /* moving wall parts  */
      ...
      if (movwall_model ) {    /*  non-Newtonian models with a moving wall */
        ...
        int guess = 0;
        err = lub2D_flow2D(gn, pg_cmp, ev, dev_dpg, q, DQ_DP, DQ_DP1, DQ_DH, H, &srate, veloL, veloU, guess, &wstrs, relax);
        if (isnan(srate))
          DPRINTF(stderr, "lub_srate isnan %g %g %g %g\n", wstrs, q[0], q[1], srate);
        if (err < 0) {
          GOMA_WH(GOMA_ERROR, "Some trouble with Numerical Lubrication...\n");
...
/** Will need conversion to general here too  */
    ...
    dbl D_Q_DGRADP[DIM][DIM][MDE], D_V_DGRADP[DIM][DIM][MDE];
 ...
 **************************/
double lub_viscos_fcn(const struct Generalized_Newtonian *gn_local,
  ...
  double yield = gn_local->tau_y, F = gn_local->fexp, mu0 = gn_local->mu0, P_eps = gn_local->epsilon;
 ...
 ********************************************/
  iconv = 1; step = 1.0; ifail = 0; th = 0.; ifin = 0; th1 = 1.;
  for( icnt =0 ; icnt < ICNTMAX ; icnt++)  {
    double deter, detinv;
    if(th+step >= th1) {
      step = th1-th;
...
/*       evaluate first viscosity integral  */
      ...
      ierr = lub2D_viscint_2D(gn_tmp, rate0, rate1, Kconst, &xint1, &xint2, &xint1dK, &xint2dK,epsxint);
      if(! ierr) GOMA_EH(GOMA_ERROR,"2D viscosity integral failed.");
...
/*       Check numerical Jacobian  */
      ...
      if( 0) {
        int i1, j1;
        ierr = lub2D_numjac(h,pgrad,rate0,rate1,Kconst,epsxint,ev,fv,veloU,veloL,gn_tmp,xjnum);
        for( i1=0 ; i1<3 ; i1++) {
          for( j1=0 ; j1<3 ; j1++) {
            xjs[i1][j1] = xj[i1][j1];
...
/*        solve 3x3 linear system  */
    ...
    if(eps > soln_tol || resid > res_tol) iconv = 0;
...
/*     if not converged : cut step, go back  */
    ...
    if( !iconv) {
      ...
      rate10 = rate1;
      Kconst0 = Kconst;
      if(iter < ITERMAX/4) step *= 1.8;
      if(iter >= ITERMAX/2) step *= 0.5;
    }
    if(ifin) break;
  }
...
/*       rates of opposite sign - do as two integrals  */
    ...
    if(eps > tolerance) GOMA_EH(GOMA_ERROR,"xint1 opposite sign integration diverged");
  }
...
/*       rates of the same sign - do as one integral */
    ...
    if(eps > tolerance) GOMA_EH(GOMA_ERROR, "xint2 same sign integration diverged");
  } else {
    double ratea, rateb, sheara, shearb, dvisdKa, dvisdKb;
    double visa, dvisa, visb, dvisb;
...
/*       rates of opposite sign - do as two integrals */
    ...
    if(eps > tolerance) GOMA_EH(GOMA_ERROR,"xint2 opposite sign integration diverged");
  }
  return(1);
}
/***************************
...
/*       determine cross rate which corresponds to this shearx  */
   ...
   if(eps > tolerance) GOMA_EH(GOMA_ERROR,"xint3 same sign integration diverged");
  } else {
...
/*       determine cross rate which corresponds to this shearx  */
          ...
          xint3 += (SQUARE(visa*ratea) * rate1 - SQUARE(visb*rateb) * rate0) * delx * mp->Lub_wts[l];
        }
      } 
      res = fabs(xint3-xintold);
      res0 = fabs(xint3);
    ...
    if( eps > tolerance) GOMA_EH(GOMA_ERROR,"xint3 opposite sign integration diverged");
  }    
  return(xint3);
}
/*===========================================================================*/
