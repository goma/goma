      ...
      */
{
  ...
  if (bulk_dim == 2)
    {
      if(bulk_type == LINEAR_TRI) GOMA_EH(GOMA_ERROR, "find_stu_on_shell not fixed for 2d bulk triangles");
  ...
  /* 3D bulk element, 2D shell element */
    ...
    {
      /* GOMA_EH(GOMA_ERROR,"DRN believes that this is too simplified, more tests are needed as done above for 2D"); */
      xi2[2] = 0.0;
      switch (id_side)
        {
	case 1:
     ...
     ***********************************************************************/
{
  ...
  if (!pd->e[pg->imtrx][eqn]) GOMA_EH(-1, "No normal equations on shell!");
  dofs = ei[pg->imtrx]->dof[eqn];
  pdim = pd->Num_Dim;

  /* Algorithm not tested for 3D yet! */
  if (pdim == 3) GOMA_EH(GOMA_ERROR, "Cannot handle shell elements in 3D yet!");
     ...
     ***********************************************************************/
{
  ...
  if ( ielem_surf_dim == 1 ) {
      ...
      //GOMA_EH(GOMA_ERROR, "you should not be here until mapbf->J in beerbelly is corrected for 1D case");
 ...
 ******************************************************************************/
{
  ...
  if ( FSIModel == 0 ) GOMA_EH(GOMA_ERROR, "ERROR: Could not find FSI Model");
  ...
  case FSI_SHELL_ONLY_MESH:
    ...
    if (!pd->e[pg->imtrx][R_MESH1]) GOMA_EH(GOMA_ERROR, "ERROR:  FSI_SHELL_ONLY_MESH requires mesh equation turned on!");
  ...
  case FSI_SHELL_ONLY_UNDEF:
    ...
    if (!pd->e[pg->imtrx][R_MESH1]) GOMA_EH(GOMA_ERROR, "ERROR:  FSI_SHELL_ONLY_UNDEF requires mesh equation turned on!");
