...
int dof_lnode_interp_type(const int n,
...
int type2shape(const int element_type) {
  int shape = -1;
  ...
  case QUADRATIC_TET:
    ...
    break;
  default:
    fprintf(stderr, "type2shape ERROR: unknown element type: %d\b", element_type);
...
int shape2sides(const int element_shape) {
  int num_sides = -1;
...
int getdofs(const int element_shape, const int interpolation)
...
void find_stu(const int iquad,      /* current GQ index  */
...
void find_surf_st(const int iquad,      /* current GQ index */
                  const int ielem_type, /* element type */
...
int find_edge_s(const int iquad,      /* current GQ index  */
                const int ielem_type, /* element type   */
...
void find_surf_center_st(const int ielem_type, /* element type */
                         const int iside,      /* current side of element */
                         const int dim,        /* dimensions of element */
                         double xi[DIM], /* (returned) local GQ coordinates for surface integral */
                         double *s,      /* Gaussian-quadrature points (s, t) */
...
void find_nodal_stu(const int inode,      /* current node index */
                    const int ielem_type, /* element type */
  ...
  case C_TRILINEAR_HEX: /* trilinear hexahedron with additional centroid node */
    ...
    case 8:
      ...
      *u = 0.0;
      break;
    default:
      GOMA_EH(GOMA_ERROR, "Trying to get nodal local stu for TRILINEAR at illegal node\n");
      break;
    }
    break;
  ...
  case TRIQUAD_HEX: /* triquadratric hexahedron */
    ...
    case 26:
      ...
      *u = 0.0;
      break;
    default:
      GOMA_EH(GOMA_ERROR, "Trying to get nodal local stu for TRIQUAD_HEX at illegal node\n");
      break;
    }
    break;
...
double Gq_weight(const int iquad,      /* current GQ index */
                 const int ielem_type) /* element type     */
...
double Gq_surf_weight(const int iquad,      /* current GQ index  */
                      const int ielem_type) /* element type      */
...

{
  ...
  case QUAD_TRI: /* quadratic triangle */
    switch (iquad) {
    case 0:
      weight = 0.5 * five_ninths;
      break;
    case 1:
      weight = 0.5 * eight_ninths;
      break;
    case 2:
      weight = 0.5 * five_ninths;
      break;
  ...
  case QUADRATIC_TET:
    switch (iquad) {
    case 0:
      weight = 1.5902069087198858469718450103758e-1;
      break;
    case 1:
      weight = 9.0979309128011415302815498962418e-2;
      break;
    case 2:
      weight = 1.5902069087198858469718450103758e-1;
      break;
  ...
  case TRIQUAD_HEX:   /* triquadratic hexahedron */
    switch (iquad) {
    case 0:
      weight = five_ninths * five_ninths;
      break;
    case 1:
      weight = eight_ninths * five_ninths;
      break;
    case 2:
      weight = five_ninths * five_ninths;
      break;
    case 3:
      weight = five_ninths * eight_ninths;
      break;
    case 4:
      weight = eight_ninths * eight_ninths;
      break;
    case 5:
      weight = five_ninths * eight_ninths;
      break;
...
double Gq_edge_weight(int iquad,      /* current GQ index  */
...
int get_type(char string[],   /* EXODUS name of parent element  */
 ...
 */
{
  ...
  if (strncmp(string, "BAR", 3) == 0) { /* select element shape */
  ...
  else if (strncmp(string, "SHELL", 5) == 0) { /* select element shape */
    switch (nodes) {                           /* select number of nodes in this element */
  ...
  else if (strncmp(string, "QUAD", 4) == 0) { /* select element shape */
    switch (nodes) {                          /* select number of nodes in this element */
    case 4:                                   /* bilinear quadralateral */
  ...
  else if (strncmp(string, "TETRA4", 6) == 0 ||
           strncmp(string, "TETRA", 5) == 0) { /* select element shape */
    switch (nodes) {                           /* select number of nodes in this element */
    case 4:                                    /* bilinear quadralateral */
      answer = LINEAR_TET;
      break;
...
/*****************************************************************************/
int centroid_node(int elem_type) {
...
int load_surf_st(int ielem_type, int id_side, int dim, double xi[DIM], double s, double t)
 ...
 *
 *  Description -
 *             This function performs the task of mapping a local surface coordinate to the
 * corresponding point in the higher dimensional element to which that surface is attached. For
 * example, for a two-dimesional element I can express a position on a side by a parameter that
 *             varies [-1,1], but if I want to evaluate variable values and shape function stuff at
 * that point I must map that surface coordinate to the two-dimensional (s,t) space of the element
 * that the surfac is attached to.  This is a function of the elem_type and the value of id_side
 *  input -
