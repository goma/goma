  	...
  	*func -= vtangent;
    ...
    else
      {
        ...
        switch(((int)u_par[0])) {
           ...
           double yc, gap, xpt1, xpt2, ypt1, ypt2, theta1, theta2, rad1, rad2, width;
	   double alpha1, alpha2, xcen1, xcen2, ycen1, ycen2, theta1m, theta2m, th1, th2, th2t, f;
	   double d_xpt1_dt, d_xpt2_dt, d_ypt1_dt, d_ypt2_dt, d_theta1m_dt, d_theta2m_dt;
	   double d_xcen1_dt, d_xcen2_dt, d_ycen1_dt, d_ycen2_dt;
           case 19:
		omega = u_par[1];
		roll_rad = u_par[2];
		t_offset = u_par[5];
		tdc_delay=u_par[8];
        	...
        	time1 = time_value + t_offset;
        	if (time_value <= 0.0)
		   { 
		    vel_user[0] = 2.*roll_rad*(-omega*cos(omega*time1));
		    vel_user[1] = 0;
		   }
		else if(time_value <= tdc_delay)
		   {
		    vel_user[0] = 0.0;
		    vel_user[1] = 0;
		   }
		...
		tdc_delay=u_par[8];
          ...
          theta2= omega*time1;
	  d_xpt1_dt = width*omega*sin(2.*omega*time1);
	  d_xpt2_dt = -width*omega*sin(2.*omega*time1);
	  d_ypt1_dt = -2*roll_rad*omega*cos(omega*time1)-width*omega*cos(2.*omega*time1);
	  d_ypt2_dt = -2*roll_rad*omega*cos(omega*time1)+width*omega*cos(2.*omega*time1);
        /*  slope of middle line                */

          theta1m = atan2(ypt2-ypt1,xpt2-xpt1);
          theta1m = theta1m > theta1 ? theta1m : theta1m + 2*M_PIE;
	  d_theta1m_dt = ((xpt2-xpt1)*(d_ypt2_dt-d_ypt1_dt)-(ypt2-ypt1)*(d_xpt2_dt-d_xpt1_dt))
				/(SQUARE(xpt2-xpt1)+SQUARE(ypt2-ypt1));
          ...
          d_xcen1_dt = d_xpt1_dt - rad1/sin(alpha1)*omega*sin(theta1+alpha1);
          ycen1 = ypt1 + (rad1/sin(alpha1))*sin(theta1+alpha1);
          xcen2 = xpt2 + (rad2/sin(alpha2))*cos(theta2m+alpha2);
          ycen2 = ypt2 + (rad2/sin(alpha2))*sin(theta2m+alpha2);
         ...
         /**  use different f depending on theta  **/
          ...
          if( (theta1-0.5*M_PIE) <= th1 && th1 <= theta1)
            {
              f = (fv->x[1]-ypt1)*cos(theta1) - (fv->x[0]-xpt1)*sin(theta1);
              vel_user[0] = (fv->x[1]-ypt1)*cos(theta1) - (fv->x[0]-xpt1)*sin(theta1);
            }
          else if ( theta2 <= th2t && (th2t - 0.5*M_PIE) <= theta2)
            {
              f = (fv->x[1]-ypt2)*cos(theta2) - (fv->x[0]-xpt2)*sin(theta2);
            }
          else if ( theta2m <= (th1+0.5*M_PIE) && th1 <= (theta1-0.5*M_PIE))
            {
              f = SQUARE(fv->x[0]-xcen1)+SQUARE(fv->x[1]-ycen1)-SQUARE(rad1);
            }
          else if ( (theta2m-0.5*M_PIE) <= th2 && (th1+0.5*M_PIE) <= theta2m)
            {
              f = (fv->x[1]-ypt1)*cos(theta1m) - (fv->x[0]-xpt1)*sin(theta1m);
            }
          else
            {
              f = SQUARE(fv->x[0]-xcen2)+SQUARE(fv->x[1]-ycen2)-SQUARE(rad2);
            }
