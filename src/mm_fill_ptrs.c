					  ...
					  * computationally expensive pointers */
{
 ...
 eqn = R_SHELL_NORMAL3;
  ...
  eqn = R_LUBP_3;
  if (upd->ep[pg->imtrx][eqn] >= 0) {
	  load_varType_Interpolation_ptrs(eqn, esp->lubp_3, esp_old->lubp_3, esp_dot->lubp_3);
  }
  eqn = R_SHELL_FILMP;
  if (upd->ep[pg->imtrx][eqn] >= 0) {
                     ...
                     const Exo_DB * exo)
{
  ...
  for (imtrx = 0; imtrx < upd->Total_Num_Matrices; imtrx++) {
    ...
    eqn = R_LUBP_3;
    if (upd->ep[imtrx][eqn] >= 0) {
    	load_varType_Interpolation_ptrs_mat(imtrx, eqn, esp->lubp_3, esp_old->lubp_3, esp_dot->lubp_3);
    }
    eqn = R_SHELL_FILMP;
    if (upd->ep[imtrx][eqn] >= 0) {
