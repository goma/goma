	...
	for (ibc = 0; 
		...
		for (j = 0; j < MAX_MAT_PER_SS; j++) {
	      ...
	      if ((blk_index = in_list(Current_EB_ptr->Elem_Blk_Id, 0,
				       MAX_MAT_PER_SS+1,
				       dup_blks_list)) == -1) {
		GOMA_EH(GOMA_ERROR,"Cannot match current element block with that in ss_to_blks array");
	      }
	      blk_index++;
  ...
  for (irc = 0; irc < Num_ROT; irc++) {
	  ...
	  if ( num_nodes_on_edge > 0 )
	    {
	      ...
	      if ( ei[pg->imtrx]->ielem_type == LINEAR_TET ||
	           ei[pg->imtrx]->ielem_type == QUADRATIC_TET ) {
                id_edge = find_id_edge_TET(ielem, num_nodes_on_edge, 
				     local_edge_node_list, edge_elem_node_id, 
				     &param_dir, exo);
              }
	    }
	...
	if (ROT_Types[irc].type == FACE) {
	  ...
	  for ( k=0; k<num_ROT_nodes; k++) {
		...
		} else GOMA_EH(GOMA_ERROR, "can't calculate normal vector");
		  ...
		  if (vector[p].ok ) {
			...
			for (b=0; b<dim; b++) {
			  for (j_id=0; j_id<vector[p].d_vector_n; j_id++) {
				...
				GOMA_EH(GOMA_ERROR, "Increase the number of available rotation sensitivities (MNROT)");
			      }
  ...
  for( inode=0; inode<num_total_nodes; inode++)
      ...
      for( jvar=0; jvar<NUM_VECTOR_EQUATIONS; jvar++)
	{
	  ...
	  if( ( ssid = local_ROT_list[inode][jvar] ) != -1 )
	    {
	      if (  ( iss = in_list( ssid, 0, exo->num_side_sets, exo->ss_id ) ) == -1 ) GOMA_EH( -1,"Error trap out");
		...
		{					
		  ...
		  if( (lnn = in_list( inode, 0, num_nodes_on_side, gnn_side_list )  ) != -1 )
		      ...
		      if( SS_Internal_Boundary[iss] != -1 )
			{
			  ...
			  if( index == (MAX_MAT_PER_SS+1) ) GOMA_EH(GOMA_ERROR," WTH.  \n");
...
/* LOCAL VARIABLES */
  ...
  } else GOMA_EH(GOMA_ERROR,"Illegal topology type");
      ...
      if (pd->v[pg->imtrx][var]) {
        ...
        for (n = 0; n < ei[pg->imtrx]->dof[var]; n++) {
          ...
          for (kdir = 0; kdir < dim; kdir++) {
            for (ldir = 0; ldir < dim; ldir++) {
              rotated_jacobian_scalar[kdir][n] +=
                  rc[kdir][ldir] * lec->J[LEC_J_INDEX(upd->ep[pg->imtrx][R_MESH1 + ldir],pvar,id,n)];
            }
          }
        ...
        if (var >= R_MESH1 && var <= R_MESH3) {
          ...
          for (kdir = 0; kdir < dim; kdir++) {
            ...
            for (n = 0; n < ei[pg->imtrx]->dof[var]; n++) {
              ...
              lec->J[LEC_J_INDEX(peqn_mesh,pvar,id,n)] += rc_dx[kdir][p][n] * lec->R[LEC_R_INDEX(peqn_mesh, id)];
            }
          }
        }
      } /* end of if variable */
