 ...
 *******************************************************************************/
/*ARGSUSED*/
int apply_rotated_bc(double resid_vector[], /* Residual vector for the current processor         */
 ...
 ****************************************************************************/
{
  ...
  do {
      ...
      if (((irot = in_list(I, 0, num_mom_rotate[pg->imtrx], mom_rotate_node[pg->imtrx])) != -1) &&
        ...
        for (ibc = 0; (bc_input_id = (int)elem_side_bc->BC_input_id[ibc]) != -1; ibc++) {
          if (BC_Types[bc_input_id].BC_ID == ss_rot) {
             ...
             * force balance
             */
            if ((ss_index = in_list(ss_rot, 0, Proc_Num_Side_Sets, ss_to_blks[0])) == -1) {
            ...
            if ((blk_index = in_list(Current_EB_ptr->Elem_Blk_Id, 0, MAX_MAT_PER_SS + 1,
                                     dup_blks_list)) == -1) {
              GOMA_EH(GOMA_ERROR,
                      "Cannot match current element block with that in ss_to_blks array");
            }
            blk_index++;
      ...
      if (((irot = in_list(I, 0, num_mesh_rotate[pg->imtrx], mesh_rotate_node[pg->imtrx])) != -1) &&
        ...
        for (ibc = 0; (bc_input_id = (int)elem_side_bc->BC_input_id[ibc]) != -1; ibc++) {
          if (BC_Types[bc_input_id].BC_ID == ss_rot) {
            ...
            if ((ss_index = in_list(ss_rot, 0, exo->num_side_sets, ss_to_blks[0])) == -1) {
              sprintf(err_msg, "Could not find SS %d in ss_to_blks", ss_rot);
    ...
    if (rotate_this_side) {
      ...
      xi[0] = 0.0;
      xi[1] = 0.0;
      xi[2] = 0.0; /* need to update for 3D ??*/
      ...
      */
      ...
      if (ielem_dim != 3) {
        ...
        GOMA_EH(GOMA_ERROR, "Illegal dimension in old rotation scheme");
      }
      ...
      for (i = 0; i < (int)elem_side_bc->num_nodes_on_side; i++) {
        ...
        if (rot_mesh[i] && ei[pg->imtrx]->ln_to_dof[MESH_DISPLACEMENT1][id] != -1) {
          rotate_res_jac_mesh(ei[pg->imtrx]->ln_to_dof[MESH_DISPLACEMENT1][id], I, iconnect_ptr,
                              num_local_nodes, ielem_dim - 1, fv->snormal, fv->dsnormal_dx,
                              fv->stangent, fv->dstangent_dx, sign_rot_mesh[i]);
        }
        if (rot_mom[i] && ei[pg->imtrx]->ln_to_dof[VELOCITY1][id] != -1 &&
            pd->i[pg->imtrx][VELOCITY1] != I_Q2_D && pd->i[pg->imtrx][VELOCITY1] != I_Q1_D) {
          rotate_res_jac_mom(ei[pg->imtrx]->ln_to_dof[VELOCITY1][id], I, iconnect_ptr,
                             num_local_nodes, ielem_dim - 1, fv->snormal, fv->dsnormal_dx,
                             fv->stangent, fv->dstangent_dx, sign_rot_mom[i]);
        } else if (rot_mom[i] && ei[pg->imtrx]->ln_to_dof[VELOCITY1][id] != -1) {
          if (((Current_EB_ptr->Elem_Blk_Id + 1) % 2) == 0) {
            rotate_res_jac_mom(ei[pg->imtrx]->ln_to_dof[VELOCITY1][id] - 1, I, iconnect_ptr,
                               num_local_nodes, ielem_dim - 1, fv->snormal, fv->dsnormal_dx,
                               fv->stangent, fv->dstangent_dx, sign_rot_mom[i]);
          } else {
            rotate_res_jac_mom(ei[pg->imtrx]->ln_to_dof[VELOCITY1][id] - 1, I, iconnect_ptr,
...
/*********************************************************************************/
/*ARGSUSED*/
void rotate_res_jac_mesh(int irow_index,           /* Elemental stiffness matrix row index */
  ...
  case 1:
    ...
    for (id = 0; id < ei[pg->imtrx]->dof[ShapeVar]; id++) {
      ...
      if (Dolphin[pg->imtrx][I][MESH_DISPLACEMENT1] > 0 &&
          Dolphin[pg->imtrx][Id][MESH_DISPLACEMENT1] > 0) {
        ...
        for (n = 0; n < ei[pg->imtrx]->dof[var]; n++) {
          ...
          for (ldir = 0; ldir < ielem_surf_dim + 1; ldir++) {
            rotated_jacobian_vector[kdir][j][n] +=
                svector[kdir][ldir] *
                lec->J[LEC_J_INDEX(upd->ep[pg->imtrx][R_MESH1 + ldir], pvar, irow_index, n)];
          }
        }
    ...
    if (pd->v[pg->imtrx][var]) {
      for (w = 0; w < pd->Num_Species_Eqn; w++) {
        for (n = 0; n < ei[pg->imtrx]->dof[var]; n++) {
          ...
          for (ldir = 0; ldir < ielem_surf_dim + 1; ldir++) {
            peq = upd->ep[pg->imtrx][R_MESH1 + ldir];
            lec->J[LEC_J_INDEX(peq, MAX_PROB_VAR + w, irow_index, n)] =
...
/********************************************************************************/
/*ARGSUSED*/
void rotate_mesh_eqn(int id,           /* Elemental stiffness matrix row index */
                     int I,            /* Global node number                   */
 ...
 ****************************************************************************/
{
  ...
  for (ldir = 0; ldir < dim; ldir++) {
    ...
    if (ndof != 1) {
      sprintf(Err_Msg, "rotate_mesh_eqn: Limited to 1 mesh dof: found %d dofs\n", ndof);
      GOMA_EH(GOMA_ERROR, Err_Msg);
    }
  ...
  for (kdir = 0; kdir < dim; kdir++) {
    ...
    if (rot->ok) {
      ...
      for (ldir = 0; ldir < dim; ldir++) {
        rotated_resid[kdir] += (rot->vector[ldir] * lec->R[LEC_R_INDEX(peqn_mesh[ldir], id)]);
      }
    }
  }
...
/*************************************************************************************/
/*ARGSUSED*/
void rotate_momentum_eqn(int id,           /* Elemental stiffness matrix row index */
                         int I,            /* Global node number                   */
                         int iconnect_ptr, /* Pointer to beginning of connectivity list
                                              for the current element	         */
  ...
  for (kdir = 0; kdir < dim; kdir++) {
    if (rotation[I][eq][kdir]->ok) {
      ...
      for (ldir = 0; ldir < dim; ldir++) {
        ...
        rotated_resid[kdir] += rotation[I][eq][kdir]->vector[ldir] * lec->R[LEC_R_INDEX(peq, id)];
      }
    }
          ...
          if (strcmp(Matrix_Format, "msr") == 0) {
            for (j = 0; j < rotation[I][eq][kdir]->d_vector_n; j++) {
              ...
              if (Dolphin[pg->imtrx][I][R_MOMENTUM1] > 0 &&
                ...
                if ((index_var = Index_Solution(J, var, ktype, ndof, -2, pg->imtrx)) == -1) {
                  GOMA_EH(GOMA_ERROR, "Cant find var index");
                }
                ...
                for (ldir = 0; ldir < dim; ldir++) {
                  a[index] += rotation[I][eq][kdir]->d_vector_dx[ldir][b][j] *
            ...
            for (j = 0; j < rotation[I][eq][kdir]->d_vector_n; j++) {
              ...
              if (Dolphin[pg->imtrx][I][R_MOMENTUM1] > 0 &&
                ...
                index = indx[K] + row_dofs * blk_col +
                        blk_row; /* here we assume only 1 mesh dof per node */
            ...
            for (w = 0; w < ei[pg->imtrx]->dof[var]; w++) {
              ...
              if ((j = in_list(J, 0, rotation[I][eq][kdir]->d_vector_n,
                               rotation[I][eq][kdir]->d_vector_J)) != -1) {
        ...
        for (kdir = 0; kdir < dim; kdir++) {
          if (rotation[I][eq][kdir]->ok) {
            ...
            for (ldir = 0; ldir < dim; ldir++) {
              rotated_jacobian_scalar[kdir][n] +=
                  rotation[I][eq][kdir]->vector[ldir] *
                  lec->J[LEC_J_INDEX(upd->ep[pg->imtrx][R_MOMENTUM1 + ldir], pvar, id, n)];
            }
          }
        }
          ...
          for (kdir = 0; kdir < dim; kdir++) {
            if (rotation[I][eq][kdir]->ok) {
              ...
              for (ldir = 0; ldir < dim; ldir++) {
                rotated_jacobian_conc[kdir][w][n] +=
                    rotation[I][eq][kdir]->vector[ldir] *
                    lec->J[LEC_J_INDEX(upd->ep[pg->imtrx][R_MOMENTUM1 + ldir], MAX_PROB_VAR + w, id,
                                       n)];
              }
            }
          }
...
/*************************************************************************** */
/*ARGSUSED*/
void rotate_res_jac_mom(int irow_index,           /* Elemental stiffness matrix row index */
                        int I,                    /* Global node number                   */
                        int iconnect_ptr,         /* Pointer to beginning of connectivity list
                                                      for the current element	         */
                        int nodes_per_elem,       /* number of nodes in the element       */
        ...
        if (pd->v[pg->imtrx][var]) {
          ...
          for (n = 0; n < ei[pg->imtrx]->dof[var]; n++) {
            ...
            for (ldir = 0; ldir < ielem_surf_dim + 1; ldir++) {
              ...
              rotated_jacobian_vector[kdir][j][n] +=
                  svector[kdir][ldir] *
                  lec->J[LEC_J_INDEX(upd->ep[pg->imtrx][R_MOMENTUM1 + ldir], pvar, irow_index, n)];
            }
          }
...
void calculate_all_rotation_vectors(Exo_DB *exo, /* the mesh */
...

{
  ...
  /* BEGIN EXECUTION */
  if (Debug_Flag > 0)
    DPRINTF(stderr, "Starting to calculate rotation vectors\n");
  ...
  if (!rotation_allocated) {
    rotation = (struct Rotation_Vectors ****)smalloc(num_total_nodes *
                                                     sizeof(struct Rotation_Vectors ***));

    for (i = 0; i < num_total_nodes; i++) {
      rotation[i] = (struct Rotation_Vectors ***)smalloc(NUM_VECTOR_EQUATIONS *
                                                         sizeof(struct Rotation_Vectors **));
      for (j = 0; j < NUM_VECTOR_EQUATIONS; j++) {
        ...
        if (ROT_list[i] != NULL && ROT_list[i][j] != -1) {
          rotation[i][j] =
              (struct Rotation_Vectors **)smalloc(dim * sizeof(struct Rotation_Vectors *));
          for (k = 0; k < dim; k++) {
            rotation[i][j][k] = (struct Rotation_Vectors *)smalloc(sizeof(struct Rotation_Vectors));
          }
        }
      }
    }
  ...
  for (irc = 0; irc < Num_ROT; irc++) {
    if (ROT_Types[irc].eq_type == R_MESH1)
      eq = VECT_EQ_MESH;
    if (ROT_Types[irc].eq_type == R_MOMENTUM1)
      eq = VECT_EQ_MOM;
    ...
    if (ROT_Types[irc].ROTATE) {
      ...
      if (ROT_Types[irc].elems == NULL) {
        ...
        get_ss_element_list(ROT_Types[irc].ss_id[0], ROT_Types[irc].ss_id[1],
                            ...
                            &ROT_Types[irc].elems, &ROT_Types[irc].ss_ptr, eq, irc, exo);
      }
      ...
      for (i = 0; i < ROT_Types[irc].num_elem; i++) {
        ...
        num_nodes_on_side =
            count_nodes_on_SS(ROT_Types[irc].ss_id[0], -1, -1, ei[pg->imtrx]->iconnect_ptr, ielem,
                              num_local_nodes, local_side_node_list, side_elem_node_id);

        id_side = find_id_side(ei[pg->imtrx]->ielem, num_nodes_on_side, local_side_node_list,
                               side_elem_node_id, exo);
        /* Calculates the ID side correctly for tets */
        int current_id;

        current_id = in_list(ROT_Types[irc].ss_id[0], 0, exo->num_side_sets, exo->ss_id);
        if (ei[pg->imtrx]->ielem_type == LINEAR_TET || ei[pg->imtrx]->ielem_type == QUADRATIC_TET) {
        ...
        if (ROT_Types[irc].type == CURVE || ROT_Types[irc].type == VERTEX) {
          num_nodes_on_edge = count_nodes_on_SS(
              ROT_Types[irc].ss_id[0], ROT_Types[irc].ss_id[1], -1, ei[pg->imtrx]->iconnect_ptr,
              ielem, num_local_nodes, local_edge_node_list, edge_elem_node_id);

          if (num_nodes_on_edge > 0) {
            param_dir = -1;
            id_edge = find_id_edge(ielem, num_nodes_on_edge, local_edge_node_list,
                                   edge_elem_node_id, &param_dir, exo);

            if (ei[pg->imtrx]->ielem_type == LINEAR_TET ||
                ei[pg->imtrx]->ielem_type == QUADRATIC_TET) {
              id_edge = find_id_edge_TET(ielem, num_nodes_on_edge, local_edge_node_list,
                                         edge_elem_node_id, &param_dir, exo);
            }
          }
        }
        ...
        } else if (ROT_Types[irc].type == VERTEX && num_nodes_on_edge > 0) {
          num_ROT_nodes = 1;
          /* determine id of vertex node */
          v_id = 0;
          while (Proc_Elem_Connect[ei[pg->imtrx]->iconnect_ptr + v_id] != ROT_Types[irc].node)
            v_id++;
          elem_node_id = &(v_id);
        }
        ...
        for (k = 0; k < num_ROT_nodes; k++) {
          ...
          if (ROT_list[I] != NULL && ROT_list[I][eq] == irc) {
            ...
            num_rots_finished = set_pointers_to_vectors(&normal, &tangent1, &tangent2, &normal2,
                                                        &normal3, &line_tangent, &binormal,
            ...
            if (num_rots_finished < dim) {
              ...
              if (normal != NULL) {
                ...
                GOMA_EH(GOMA_ERROR, "can't calculate normal vector");
              ...
              if (ROT_Types[irc].method == ROT_BASIS_RESEED) {
                ...
                kdir = -1;
                for (p = 0; p < dim; p++)
                  if (ROT_Types[irc].BC_Type[p] == ROT_T1)
                    kdir = p;
                GOMA_EH(kdir, "cant find tangent1");
              ...
              if (tangent1 != NULL || tangent2 != NULL) {
                if (ROT_Types[irc].method == ROT_BASIS || ROT_Types[irc].method == ROT_BASIS_ONCE ||
                    (ROT_Types[irc].method == ROT_BASIS_RESEED && !reseed)) {

                  calc_tangent_along_basis(tangent1, normal, dim, id_side, num_nodes_on_side,
                                           side_elem_node_id);
                }
              ...
              if (line_tangent != NULL || binormal != NULL) {
                ...
                if (ei[pg->imtrx]->ielem_type == LINEAR_TET ||
                    ei[pg->imtrx]->ielem_type == QUADRATIC_TET) {
                  calc_unseeded_edge_tangents_TET(line_tangent, ei[pg->imtrx]->iconnect_ptr, dim,
                                                  id_side, id_edge, num_nodes_on_edge,
                                                  edge_elem_node_id, param_dir);
                } else {
                  calc_unseeded_edge_tangents(line_tangent, ei[pg->imtrx]->iconnect_ptr, dim,
                                              id_side, id_edge, num_nodes_on_edge,
                                              edge_elem_node_id, param_dir);
                }
              }
                      ...
                      for (b = 0; b < dim; b++) {
                        for (j_id = 0; j_id < vector[p].d_vector_n; j_id++) {
                          ...
                          if ((j_new = in_list(vector[p].d_vector_J[j_id], 0,
                                               ...
                                               rotation[I][eq][p]->d_vector_J)) == -1) {
                            if (rotation[I][eq][p]->d_vector_n >= MNROT) {
                              /* illegal number of sensitivities */
                              GOMA_EH(GOMA_ERROR, "Increase the number of available rotation "
                                                  "sensitivities (MNROT)");
                            }
                            j_new = rotation[I][eq][p]->d_vector_n++;
                            rotation[I][eq][p]->d_vector_J[j_new] = vector[p].d_vector_J[j_id];
                          }
  ...
  if (Debug_Flag > 0)
    DPRINTF(stderr, "Done calculating rotation vectors\n");

  return;
} /* END of calculate_all_rotation_vectors  */

void calculate_2D_rotation_vectors(Exo_DB *exo, /* the mesh */
                                   double x[])  /* Soln vector */

/*
 *   Routine for determining the rotation vectors to be applied at nodes for 2D problems
 *   Normally, for 2D problems rotation vectors can be determined "on the fly" because of the
 * simplicity and localization of the interactions. However, for meshes that involve triangular
 * elements, it is less easy to handle the rotation vectors on an element by element approach since
 * there are likely to be element node residuals that must be rotated, but do not have a
 * corresponding element face on the boundary from which to determine the appropriate rotation
 * vector Thus, it is necessary to determine a set of globally-known rotation vectors for each node
 * at which a rotated condition is applied.  This routine adapts the rotation structure developed
 * for three dimensional problems to two dimensional problems.
 */
{
      ...
      if ((ssid = local_ROT_list[inode][jvar]) != -1) {
        ...
        for (i = 0; i < exo->ss_num_sides[iss]; i++) {
          ...
          if ((lnn = in_list(inode, 0, num_nodes_on_side, gnn_side_list)) != -1) {
            ...
            if (x == x_static)
              err = load_elem_dofptr(elem, exo, x_static, x_old_static, xdot_static,
                                     xdot_old_static, 0);
            else
            ...
            if (SS_Internal_Boundary[iss] != -1) {
              ...
              while (ss_to_blks[index + 1][iss] != curr_eb && index < (MAX_MAT_PER_SS + 1))
                index++;

              if (index == (MAX_MAT_PER_SS + 1))
                GOMA_EH(GOMA_ERROR, " WTH.  \n");
            ...
            if (add_vectors) {
              ...
              id_side =
                  find_id_side(elem, num_nodes_on_side, gnn_side_list, local_node_side_list, exo);

              if (ei[pg->imtrx]->ielem_type == LINEAR_TET ||
                  ei[pg->imtrx]->ielem_type == QUADRATIC_TET) {
                id_side = find_id_side_SS(ei[pg->imtrx]->ielem, iss, exo);
              }
              ...
              surface_determinant_and_normal(elem, iconnect_ptr, num_local_nodes,
              ...
              if (rotation[inode][jvar] == NULL) {
                rotation[inode][jvar] = (struct Rotation_Vectors **)smalloc(
                    pd->Num_Dim * sizeof(struct Rotation_Vectors *));
                rotation[inode][jvar][0] = NULL;
                rotation[inode][jvar][1] = NULL;
              }

              append_vectors(inode, num_nodes_on_side, iconnect_ptr, local_node_side_list,
                             rotation[inode][jvar]);
            }
            /*if( inode == 4 )
              {
              ...
              rotation[inode][jvar][0]->vector[0],  rotation[inode][jvar][0]->vector[1],
              rotation[inode][jvar][1]->vector[0],  rotation[inode][jvar][1]->vector[1]);
              printf("\t %d   %7.4g : %d   %7.4g : %d   %7.4g \n",
              rotation[inode][jvar][vvec]->d_vector_J[0]+1,
              rotation[inode][jvar][vvec]->d_vector_dx[0][comp][0], \
              rotation[inode][jvar][vvec]->d_vector_J[1]+1,
              rotation[inode][jvar][vvec]->d_vector_dx[0][comp][1], \
              rotation[inode][jvar][vvec]->d_vector_J[2]+1,
              rotation[inode][jvar][vvec]->d_vector_dx[0][comp][2] );
              }
              if( inode == 1)
              printf("I: %d, elem: %d, side: %d,  N(%7.4g, %7,4g), TN(%7.4g, %7,4g) \n", inode,
              elem, id_side, fv->snormal[0], fv->snormal[1], fv->stangent[0][0], fv->stangent[0][1]
              );
            */
          }
        }
      ...
      if (rotation[inode][jvar] != NULL) {
        ...
        */
        /*printf("\t %d   %7.4g : %d   %7.4g : %d   %7.4g \n",
          ...
          \ rotation[inode][jvar][1]->d_vector_J[2]+1,
          rotation[inode][jvar][1]->d_vector_dx[0][0][2] );
          }
          printf("I: %d, jvar: %d, d_vector_n[0]: %d, d_vector_n[1]: %d  \n", inode+1, jvar,
          rotation[inode][jvar][0]->d_vector_n, rotation[inode][jvar][1]->d_vector_n  ); for(i=0; i<
          rotation[inode][jvar][0]->d_vector_n; i++)  printf("\t %d ",
          rotation[inode][jvar][0]->d_vector_J[i]+1, rotation[inode][jvar][0]->d_vector_dx[ ) ;
          printf("\n"); for(i=0; i< rotation[inode][jvar][1]->d_vector_n; i++)  printf("\t %d ",
          rotation[inode][jvar][1]->d_vector_J[i]+1 ) ; printf("\n");*/
      }
    }
  }
  return;
...
void get_ss_element_list(const int ss_id1,  /*  side set ID                   (in) */
...

{
  ...
  *ss_ptr = -1;
  ...
  if (is_edge && (ss_id1_found && ss_id2_found)) {
    ...
    len2 = exo->ss_node_side_index[ss_2_index][exo->ss_num_sides[ss_2_index]];
      ...
      for (k = exo->ss_node_side_index[ss_1_index][e];
        ...
        if (index_node_in_ssid2 > -1) {
          if (ROT_list[inode] != NULL) {
            ...
            if (ROT_Types[irc_node].type == VERTEX && in_list(ss_id1, 0, 3, node_ssid_list) != -1 &&
    ...
    if (common_elem_count > 0) {
      *num_elem = common_elem_count;
      edge_elem_list = (int *)realloc(edge_elem_list, common_elem_count * sizeof(int));
...
#endif
#ifndef PARALLEL
    log_err("ROT [%d] wanted an edge, but one of SS ID %d %d %d gone!", irc, ss_id1, ss_id2,
            ss_id3);
#endif
  }
  ...
  if (is_vertex) {
    ...
    len3 = exo->ss_node_side_index[ss_3_index][exo->ss_num_sides[ss_3_index]];
    ...
    for (e = 0; e < exo->ss_num_sides[ss_1_index]; e++) {
      ...
      for (k = exo->ss_node_side_index[ss_1_index][e];
        ...
        if (index_node_in_ssid2 > -1 && index_node_in_ssid3 > -1) {
          /*
          ...
          if (ROT_list[inode] != NULL) {
            ...
            if (ROT_list[inode][eq] == irc) {
              ...
              if (inode == ROT_Types[irc].node) {
                ...
                if (-1 == in_list(elem, 0, common_elem_count, vertex_elem_list)) {
    ...
    if (common_elem_count > 0) {
      *num_elem = common_elem_count;
      vertex_elem_list = (int *)realloc(vertex_elem_list, common_elem_count * sizeof(int));
      *elems = vertex_elem_list;
      safe_free(elem_list);
      return;
    } else {
#ifdef PARALLEL
      log_dbg("ROT [%d] on SS ID %d & %d & %d not found.", irc, ss_id1, ss_id2, ss_id3);
      safe_free(elem_list);
      return;
#endif
#ifndef PARALLEL
       ...
       * You had all your chances.
       */
      log_err("ROT [%d] on SS ID %d & %d & %d not found.", irc, ss_id1, ss_id2, ss_id3);
#endif
    }
  }

  log_msg("A bad fall through.");
  log_err("Blame? ss_id1=%d, ss_id2=%d, ss_id3=%d, ROT [%d] on eqn %d", ss_id1, ss_id2, ss_id3, irc,
          eq);
...
int set_pointers_to_vectors(struct Rotation_Vectors **normal, /* defined in mm_as_structs.h */
...

{
  ...
  if (ROT_Types[irc].type == FACE) {
    ...
    *tangent2 = NULL;
    ...
    GOMA_EH(GOMA_ERROR, "Illegal topology type");
  ...
  for (p = 0; p < dim; p++) {
    if (ROT_Types[irc].BC_Type[p] > 0) {
      ...
      case ROT_N2:
        *normal2 = vector + p;
        GOMA_EH(GOMA_ERROR, "Second Normal Rotation not enabled yet");
        break;

      case ROT_N3:
        *normal3 = vector + p;
        GOMA_EH(GOMA_ERROR, "Third Normal Rotation not enabled yet");
        break;

      case ROT_T:
        *line_tangent = vector + p;
        if (ROT_Types[irc].type == FACE)
          GOMA_EH(GOMA_ERROR, "Need to specify T1 or T2 on Surfaces");
        break;
      ...
      default:
        GOMA_EH(GOMA_ERROR, "illegal rotation vector");
      } /* end of switch */
    }
  }
...
void rotate_eqns_at_node_2D(int iconn,
                            int dim,
                            int num_local_nodes,
                            struct GomaLinearSolverData *ams) {
  ...
  for (i = 0; i < num_local_nodes; i++) {
    ...
    if (rotation[I][VECT_EQ_MOM] != NULL && pd->e[pg->imtrx][R_MOMENTUM1]) {
      ...
      if (id_mom > -1)
        rotate_momentum_eqn(id_mom, I, iconn, dim, ams);
    }
    ...
    if (rotation[I][VECT_EQ_MESH] != NULL && pd->e[pg->imtrx][R_MESH1]) {
      ...
      if (id_mesh > -1)
        rotate_mesh_eqn(id_mesh, I, iconn, dim, ams);
    }
  }
  return;
}
...

{
  ...
  for (unsigned int i = 0; i < 3; i++) {
    for (unsigned int j = 0; j < 3; j++) {
      rc[i][j] =
          gds_vector_get(goma_automatic_rotations.rotation_nodes[I].rotated_coord[i]->normal, j);
    }
  }
...

{
  ...
  for (kdir = 0; kdir < dim; kdir++) {
    peq = upd->ep[pg->imtrx][R_MESH1 + kdir];
    lec->R[LEC_R_INDEX(peq, id)] = rotated_resid[kdir];
  }
      ...
      if (pd->v[pg->imtrx][var]) {
        ...
        for (n = 0; n < ei[pg->imtrx]->dof[var]; n++) {
          ...
          for (kdir = 0; kdir < dim; kdir++) {
            for (ldir = 0; ldir < dim; ldir++) {
              rotated_jacobian_scalar[kdir][n] +=
                  rc[kdir][ldir] *
                  lec->J[LEC_J_INDEX(upd->ep[pg->imtrx][R_MESH1 + ldir], pvar, id, n)];
            }
          }
        ...
        for (kdir = 0; kdir < dim; kdir++) {
          ...
          for (n = 0; n < ei[pg->imtrx]->dof[var]; n++) {
            ...
            lec->J[LEC_J_INDEX(upd->ep[pg->imtrx][R_MESH1 + kdir], pvar, id, n)] =
                rotated_jacobian_scalar[kdir][n];
          }
        }
        ...
        if (var >= R_MESH1 && var <= R_MESH3) {
          double rc_dx[DIM][DIM][MDE];
          int p = var - R_MESH1;
          for (unsigned int i = 0; i < 3; i++) {
            for (unsigned int j = 0; j < 3; j++) {
              // d / d_p normal of dof n
              for (n = 0; n < ei[pg->imtrx]->dof[var]; n++) {
                rc_dx[i][j][n] = gds_vector_get(
                    goma_automatic_rotations.rotation_nodes[I].rotated_coord[i]->d_normal_dx[p][n],
                    j);
              }
            }
          }
          for (kdir = 0; kdir < dim; kdir++) {
            ...
            for (n = 0; n < ei[pg->imtrx]->dof[var]; n++) {
              ...
              lec->J[LEC_J_INDEX(peqn_mesh, pvar, id, n)] +=
                  rc_dx[kdir][p][n] * lec->R[LEC_R_INDEX(peqn_mesh, id)];
            }
          }
        }
      } /* end of if variable */
