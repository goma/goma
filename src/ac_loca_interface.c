...
static void print_final(
    double conparam, int step_num, int num_newt_its, int num_modnewt_its, int total_linear_its);
 ...
 */
{
  ...
  if (ProcID == 0) {
    ...
    default:
      if (loca_in->Cont_Alg != LOCA_LSA_ONLY) {
        ...
        GOMA_EH(GOMA_ERROR, "Bad cont->upType");
      }
      break; /* duh */
    }
  ...
  if (strcmp(Matrix_Format, "epetra") == 0) {
    GOMA_EH(GOMA_ERROR, "Error epetra Matrix format not currently supported with loca interface");
  }
  ...
  if (strcmp(Matrix_Format, "vbr") == 0) {
    ...
    AZ_set_VBR(passdown.amat, passdown.ams->rpntr, passdown.ams->cpntr, passdown.ams->bpntr,
               passdown.ams->indx, passdown.ams->bindx, passdown.ams->val, passdown.ams->data_org,
               0, NULL, AZ_LOCAL);
  } else
    passdown.amat = NULL;
  ...
  if (Linear_Stability) {
...
#ifndef HAVE_PARPACK
    if (Num_Proc > 1) {
      GOMA_WH(-1, "PARPACK was not compiled in -- cannot do eigensolves!");
    ...
    GOMA_EH(err, "Unable to open eigenvector output files!");
  }
  ...
  case TP_CONTINUATION:
    ...
    if (loca_in->NVRestart) {
      con.turning_point_info.nv =
          (double *)array_alloc(1, con.general_info.numUnks, sizeof(double));
  ...
  case PF_CONTINUATION:
    ...
    con.pitchfork_info.psi = (double *)array_alloc(1, con.general_info.numUnks, sizeof(double));
    if (Num_Proc > 1)
      multiname(loca_in->NV_exoII_infile, ProcID, Num_Proc);
    DPRINTF(stdout, "Reading previous null vector ...\n");
  ...
  case HP_CONTINUATION:
    ...
    con.hopf_info.y_vec = (double *)array_alloc(1, con.general_info.numUnks, sizeof(double));
    con.hopf_info.z_vec = (double *)array_alloc(1, con.general_info.numUnks, sizeof(double));
    ...
    err = rd_vectors_from_exoII(con.hopf_info.y_vec, loca_in->NV_exoII_infile, 0, 0, INT_MAX,
                                &timeValueRead, exo);
    if (err != 0)
      GOMA_EH(GOMA_ERROR, "do_loca: error reading real part of null vector");
    ...
    #ifdef SQP_OPTIMIZER
          GOMA_EH(GOMA_ERROR, "sqp optimization not yet available in Goma!");
          solve_sqp_optimization(con_par_ptr, passdown.mesh, nstep);
    #else
          GOMA_EH(GOMA_ERROR, "sqp optimization requested but not compiled in!");
    #endif
          break;
    */
  ...
  if (Linear_Stability) {
    ...
    if (con.eigen_info.Num_Eigenvalues > 0) {
      ...
      GOMA_EH(GOMA_ERROR, "Number of eigenvalues must be specified!");
  }
  ...
  if (con.general_info.method == LOCA_LSA_ONLY) {
    initialize_util_routines(NumUnknowns[pg->imtrx],
  ...
  *con_par_ptr = con.general_info.param;
  ...
  if (con.general_info.printproc)
    print_final(con.general_info.param, nstep, passdown.num_mat_fills, passdown.num_res_fills,
                passdown.num_linear_its);

  if (strlen(Soln_OutFile))
    fclose(passdown.file);
...
/*****************************************************************************/
/*****************************************************************************/
int nonlinear_solver_conwrap(double *x, void *con_ptr, int step_num, double lambda, double delta_s)
 ...
 */
{
  ...
  if (ProcID == 0) {
    ...
    if (passdown.method != LOCA_LSA_ONLY) {
      nCC = cpcc[0].nCC;
      DPRINTF(stdout, "\n\tStep number: %4d of %4d (max)", step_num + 1, cont->MaxPathSteps);
      theta =
          (lambda - cont->BegParameterValue) / (cont->EndParameterValue - cont->BegParameterValue);
      if (nCC > 1 || nUC > 0) {
      ...
      if (cont->upType == 5 && nUC > 0) {
        for (iUC = 0; iUC < nUC; iUC++) {
          ...
          case 1: /* BC */
            DPRINTF(stdout, "\n\tBCID=%3d DFID=%5d", cpuc[iUC].BCID, cpuc[iUC].DFID);
            break;
          case 2: /* MT */
            DPRINTF(stdout, "\n\tMTID=%3d MPID=%5d", cpuc[iUC].MTID, cpuc[iUC].MPID);
            break;
          case 3: /* AC */
            DPRINTF(stdout, "\n\tACID=%3d DFID=%5d", cpuc[iUC].BCID, cpuc[iUC].DFID);
            break;
          case 4: /* UM */
            DPRINTF(stdout, "\n\tMTID=%3d MPID=%5d FLOAT=%3d", cpuc[iUC].MTID, cpuc[iUC].MPID,
                    cpuc[iUC].MDID);
            break;
          case 5: /* UF */
            break;
          ...
          DPRINTF(stdout, " Parameter= %10.6e delta_s= %10.6e", cpuc[iUC].value,
                  cpuc[iUC].value - cpuc[iUC].old_value);
        }
      } else {
        for (iCC = 0; iCC < nCC; iCC++) {
          switch (cpcc[iCC].Type) {
          case 1: /* BC */
            DPRINTF(stdout, "\n\tBCID=%3d DFID=%5d", cpcc[iCC].BCID, cpcc[iCC].DFID);
            break;
          case 2: /* MT */
            DPRINTF(stdout, "\n\tMTID=%3d MPID=%5d", cpcc[iCC].MTID, cpcc[iCC].MPID);
            break;
          case 3: /* AC */
            DPRINTF(stdout, "\n\tACID=%3d DFID=%5d", cpcc[iCC].BCID, cpcc[iCC].DFID);
            break;
          case 4: /* UM */
            DPRINTF(stdout, "\n\tMTID=%3d MPID=%5d FLOAT=%3d", cpcc[iCC].MTID, cpcc[iCC].MPID,
                    cpcc[iCC].MDID);
            break;
          case 5: /* UF */
            break;
  ...
  if (converged) {
    ...
    if (passdown.method != LOCA_LSA_ONLY) {
      for (iCC = 0; iCC < nCC; iCC++)
        cpcc[iCC].old_value = cpcc[iCC].value;
      for (iTC = 0; iTC < nTC; iTC++)
        tpcc[iTC].old_value = tpcc[iTC].value;
    }
    ...
    for (i = 0; i < nn_post_fluxes; i++)
      evaluate_flux(passdown.exo, passdown.dpi, pp_fluxes[i]->ss_id, pp_fluxes[i]->flux_type,
                    pp_fluxes[i]->flux_type_name, pp_fluxes[i]->blk_id,
                    pp_fluxes[i]->species_number, pp_fluxes[i]->flux_filenm,
                    pp_fluxes[i]->profile_flag, x, passdown.xdot, NULL, delta_s, lambda, 1);

    /* COMPUTE FLUX, FORCE SENSITIVITIES */

    for (i = 0; i < nn_post_fluxes_sens; i++)
      evaluate_flux_sens(
          passdown.exo, passdown.dpi, pp_fluxes_sens[i]->ss_id, pp_fluxes_sens[i]->flux_type,
          pp_fluxes_sens[i]->flux_type_name, pp_fluxes_sens[i]->blk_id,
          pp_fluxes_sens[i]->species_number, pp_fluxes_sens[i]->sens_type,
          pp_fluxes_sens[i]->sens_id, pp_fluxes_sens[i]->sens_flt, pp_fluxes_sens[i]->sens_flt2,
          pp_fluxes_sens[i]->vector_id, pp_fluxes_sens[i]->flux_filenm,
          pp_fluxes_sens[i]->profile_flag, x, passdown.xdot, passdown.x_sens_p, delta_s, lambda, 1);
 ...
 */
{
  ...
  case UMFPACK2F:
    ...
    /*  */
    matr_form = 1;
    LOCA_UMF_ID = SL_UMF(LOCA_UMF_ID, &first_linear_solver_call, &Factor_Flag, &matr_form,
                         &NumUnknowns[pg->imtrx], &NZeros, &ija[0], &ija[0], &a[0], &xr[0], &x[0]);
    /*  */
  ...
  case AZTEC:
    ...
    while ((!matrix_solved) && (linear_solver_blk < num_linear_solve_blks)) {
      ...
      /* Solve the matrix */
      AZ_solve(x, xr, ams->options, ams->params, ams->indx, ams->bindx, ams->rpntr, ams->cpntr,
               ams->bpntr, ams->val, ams->data_org, ams->status, ams->proc_config);
...
#endif
int komplex_linear_solver_conwrap(KOMPLEX_UNUSED double *c,
                                  KOMPLEX_UNUSED double *d,
                                  KOMPLEX_UNUSED int jac_flag,
                                  KOMPLEX_UNUSED double *omega,
                                  KOMPLEX_UNUSED double *tmp)
 ...
 */
{
  ...
  */
  ...
  AZK_create_linsys_ri2k(a, b, c, d, ams->options, ams->params, ams->proc_config, J, wM, &x, &rhs,
                         &Kmat);
  ...
  AZK_create_precon(ams->options, ams->params, ams->proc_config, x, rhs, Kmat, &Prec);

  /* Solve the linear system */
  AZ_iterate(x, rhs, ams->options, ams->params, ams->status, ams->proc_config, Kmat, Prec, NULL);
  /*
  ...
  */
  ...
  AZK_extract_solution_k2ri(ams->options, ams->params, ams->proc_config, Kmat, Prec, x, c, d);

  /* Clean up Memory */
  AZ_free_memory(Kmat->data_org[AZ_name]);
  AZK_destroy_precon(ams->options, ams->params, ams->proc_config, Kmat, &Prec);
  AZK_destroy_linsys(ams->options, ams->params, ams->proc_config, &x, &rhs, &Kmat);
  safe_free((void *)wM);
  safe_free((void *)a);
  safe_free((void *)b);
 ...
 */
{
  ...
  if (matflag == RHS_MATRIX_SAVE) {
    if (passdown.method != ARC_LENGTH_CONTINUATION)
      save_flag = TRUE;
    matflag = RHS_MATRIX;
  }

  /* Get global element size and velocity norm if needed for PSPG or Cont_GLS */
  if ((PSPG && Num_Var_In_Type[pg->imtrx][PRESSURE]) ||
      (Cont_GLS && Num_Var_In_Type[pg->imtrx][VELOCITY1])) {
    h_elem_avg = global_h_elem_siz(x, passdown.x_old, passdown.xdot, passdown.resid_vector,
                                   passdown.exo, passdown.dpi);
    U_norm = global_velocity_norm(x, passdown.exo, passdown.dpi);
  } else {
    h_elem_avg = 0.;
    U_norm = 0.;
  }
  ...
  /* Remaining cases: perform requested fill */
    ...
    (void)matrix_fill_full(ams, x, rhs, passdown.x_old, passdown.x_older, passdown.xdot,
                           passdown.xdot_old, passdown.x_update, &(passdown.delta_t),
                           &(passdown.theta), First_Elem_Side_BC_Array[pg->imtrx],
                           &(passdown.time_value), passdown.exo, passdown.dpi,
                           &(passdown.dpi->num_universe_nodes), &(h_elem_avg), &(U_norm), NULL);
  }
 ...
 */
{
  ...
  etm_save = (dbl ***)array_alloc(3, upd->Num_Mat, MAX_EQNS, MAX_TERM_TYPES, sizeof(dbl));
  ...
  for (i = 0; i < passes; i++) {
    ...
    (void)matrix_fill_full(ams, x, rhs, passdown.x_old, passdown.x_older, passdown.xdot,
                           passdown.xdot_old, passdown.x_update, &(passdown.delta_t),
                           &(passdown.theta), First_Elem_Side_BC_Array[pg->imtrx],
                           &(passdown.time_value), passdown.exo, passdown.dpi, &(nnodes),
  ...
  if (passdown.do_3D_of_2D) {
    for (j = 0; j < NZeros + 1; j++)
      jacobian_matrix[j] += tmp_matrix[j];
  }
  ...
  if (passdown.do_3D_of_2D) {
    for (j = 0; j < NZeros + 1; j++)
      passdown.mass_matrix[j] += tmp_matrix[j];
  }
  ams->val = jacobian_matrix;
  ...
  if (eigen->Eigen_Matrix_Output) {
    ...
    output_stability_matrices(passdown.mass_matrix, jacobian_matrix, ija, nnodes,
                              NumUnknowns[pg->imtrx], NZeros);
  }
...
void shifted_linear_solver_conwrap(double *x, double *y, int jac_flag, double tol) {
  ...
  case UMFPACK2F:
    ...
    /*  */
    matr_form = 1;
    stab_umf_id = SL_UMF(stab_umf_id, &first_linear_solver_call, &Factor_Flag, &matr_form,
                         &NumUnknowns[pg->imtrx], &NZeros, &ija[0], &ija[0], &a[0], &x[0], &y[0]);
    /*  */
    first_linear_solver_call = FALSE;
    strcpy(stringer, " 1 ");
    break;
  ...
  case AZTEC:
    ...
    while ((!matrix_solved) && (linear_solver_blk < num_linear_solve_blks)) {
      ...
      /* Solve the matrix */
      AZ_solve(y, x, ams->options, ams->params, ams->indx, ams->bindx, ams->rpntr, ams->cpntr,
               ams->bpntr, ams->val, ams->data_org, ams->status, ams->proc_config);
  ...
  default:
    GOMA_EH(GOMA_ERROR, "That linear solver package is not implemented for eigensolves");
    break;
  }
 ...
 */
{
  ...
  if (cont->upType == 6) /* AN */
  {
    ...
    for (iCC = 1; iCC < nCC; iCC++) {
      ...
      case 0:
        cpcc[iCC].value = cpcc[iCC].coeff_0 + cpcc[iCC].coeff_1 * sin(theta);
        break;
      case 1:
        cpcc[iCC].value = cpcc[iCC].coeff_0 + cpcc[iCC].coeff_1 * cos(theta);
        break;
      case 2:
        cpcc[iCC].value = cpcc[iCC].coeff_0 + cpcc[iCC].coeff_1 * tan(theta);
        break;
      case 3:
        cpcc[iCC].value =
            cpcc[iCC].coeff_0 + cpcc[iCC].coeff_1 * sin(theta) + cpcc[iCC].coeff_2 * cos(theta);
        break;
      }

      update_parameterC(iCC, cpcc[iCC].value, passdown.x, passdown.xdot, passdown.x_AC,
                        cont->Delta_s0, passdown.cx, passdown.exo, passdown.dpi);
    }
  } else {
    for (iCC = 0; iCC < nCC; iCC++) {
      /* This is for the variable-exponent special case */
      if (cpcc[iCC].fn_flag == 3) {
        cpcc[iCC].value = cpcc[iCC].coeff_0 + cpcc[iCC].coeff_1 * pow(param, cpcc[iCC].coeff_2);
      }
 ...
 */
{
  ...
  if (loca_in->TPupType == 6) /* AN */
  {
    ...
    for (iTC = 1; iTC < nTC; iTC++) {
      ...
      case 0:
        tpcc[iTC].value = tpcc[iTC].coeff_0 + tpcc[iTC].coeff_1 * sin(theta);
        break;
      case 1:
        tpcc[iTC].value = tpcc[iTC].coeff_0 + tpcc[iTC].coeff_1 * cos(theta);
        break;
      case 2:
        tpcc[iTC].value = tpcc[iTC].coeff_0 + tpcc[iTC].coeff_1 * tan(theta);
        break;
      case 3:
        tpcc[iTC].value =
            tpcc[iTC].coeff_0 + tpcc[iTC].coeff_1 * sin(theta) + tpcc[iTC].coeff_2 * cos(theta);
        break;
      }

      update_parameterTP(iTC, tpcc[iTC].value, passdown.x, passdown.xdot, passdown.x_AC,
                         cont->Delta_s0, passdown.cx, passdown.exo, passdown.dpi);
    }
  }
  /* This is for all other continuation types (simple linear relation) */
    ...
    for (iTC = 0; iTC < nTC; iTC++) {
      ...
      if (tpcc[iTC].fn_flag == 3) {
        tpcc[iTC].value = tpcc[iTC].coeff_0 + tpcc[iTC].coeff_1 * pow(bif_param, tpcc[iTC].coeff_2);
      }
      /* This is for all other continuation types (simple linear relation) */
      else {
 ...
 */
{
  ...
  */
  ...
  for (i = 0; i < numOwnedUnks; i++)
    x[i] = x_old[i] + 1.0e-5 * x[i] / scale_vec[i];
 ...
 */
{
  ...
  if (i_print) {
    error = write_ascii_soln(x, passdown.resid_vector, NumUnknowns[pg->imtrx], passdown.x_AC, nAC,
                             param1, passdown.file);
    if (error)
    ...
    for (i = 0; i < n; i++) {
      ...
      if (n == 1) {
        if (Linear_Stability == LSA_3D_OF_2D) {
          GOMA_EH(GOMA_ERROR, "With LOCA, you need to have more than one 3D wave number specified");
        }
        ...
        DPRINTF(stdout, "\n\t3D of 2D LSA -- Wavenumber %d of %d:   %g\n",
                LSA_current_wave_number + 1, LSA_number_wave_numbers, LSA_3D_of_2D_wave_number);
      }
...
/*****************************************************************************/
/*****************************************************************************/
void eigenvector_output_conwrap(int j,
                                int num_soln_flag,
                                double *xr,
                                double evr,
                                double *xi,
 ...
 */
{
  ...
  if (j >= eigen->Eigen_Record_Modes)
    return;

  /* Decide whether to output eigenvectors on this call */
  i_print = FALSE;
  if (freq > 0 && step_num % freq == 0)
    i_print = TRUE;
  if (freq == -1 && passdown.last_step)
    i_print = TRUE;
  if (!i_print)
...
/*****************************************************************************/
/*****************************************************************************/
static void print_final(double param, int step_num, int mat_fills, int res_fills, int linear_its)
