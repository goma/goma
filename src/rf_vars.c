...
/************************************************************************/
/************************************************************************/
int find_base_variable_type(int var_type)
 ...
 *****************************************************************/
{
  ...
  if (var_type >= V_LAST)
    return -1;
  if (var_type >= VELOCITY1 && var_type <= VELOCITY3)
    return VELOCITY1;
  if (var_type >= VELOCITY_GRADIENT11 && var_type <= VELOCITY_GRADIENT33)
    return VELOCITY_GRADIENT11;
  if (var_type >= POLYMER_STRESS11_1 && var_type >= POLYMER_STRESS33_7)
    return POLYMER_STRESS11_1;
  if (var_type >= SPECIES_UNK_0 && var_type <= SPECIES_UNK_LAST)
    return SPECIES_UNK_0;
  if (var_type >= VOLF_PHASE_0 && var_type <= VOLF_PHASE_LAST)
    return VOLF_PHASE_0;
 ...
 *****************************************************************/
{
  ...
  if (vd2 == NULL)
    return FALSE;
  if (vd1->Variable_Type != vd2->Variable_Type)
    return FALSE;
  if (vd1->MatID != vd2->MatID)
    return FALSE;
  if (vd1->Ndof != vd2->Ndof)
    return FALSE;
  if (vd1->Subvar_Index != vd2->Subvar_Index)
    return FALSE;
  return TRUE;
}
/************************************************************************/
...
int variable_description_init(VARIABLE_DESCRIPTION_STRUCT **vd_hdl)
 ...
 ******************************************************************/
{
  if (*vd_hdl == NULL) {
    *vd_hdl = smalloc(sizeof(VARIABLE_DESCRIPTION_STRUCT));
    if (*vd_hdl == NULL)
      GOMA_EH(GOMA_ERROR, "variable_description_init");
  }
...
int variable_description_default(VARIABLE_DESCRIPTION_STRUCT *vd)
...
int variable_description_alloc(VARIABLE_DESCRIPTION_STRUCT *vd,
                               const int var_type,
                               const int matID,
                               const int ndof,
...
void variable_description_free(VARIABLE_DESCRIPTION_STRUCT *vd)
...
void variable_description_destroy(VARIABLE_DESCRIPTION_STRUCT **vd_hdl)
...
VARIABLE_DESCRIPTION_STRUCT *variable_description_create(const int var_type,
                                                         const int matID,
                                                         const int ndof,
                                                         const int subvarIndex)
 ...
 ***********************************************************/
{
  ...
  if (vd != NULL) {
    retn = variable_description_alloc(vd, var_type, matID, ndof, subvarIndex);
    if (retn != 0) {
...
VARIABLE_DESCRIPTION_STRUCT *find_or_create_vd(
    const int var_type, const int ndof, const int mn, const int subvarIndex, const int imtrx)
 ...
 ********************************************************************/
{
   ...
   */
  realloc_ptr_1((void ***)&Var_Info, Num_Var_Info_Records + 1, Num_Var_Info_Records);
...
VARIABLE_DESCRIPTION_STRUCT *Variable_Description_Match(VARIABLE_DESCRIPTION_STRUCT *vd_match)
...
VARIABLE_DESCRIPTION_STRUCT *get_vd_ptr(int var_type, int matID, int idof)
 ...
 ********************************************************************/
{
  ...
  for (i = 0; i < Num_Var_Info_Records; i++) {
    vd = Var_Info[i];
    if (vd->Variable_Type != var_type)
      continue;
    if (vd->MatID == matID) {
      if (vd->Variable_Type == MASS_FRACTION) {
        if (vd->Subvar_Index != idof)
          continue;
      }
      return vd;
    } else if (vd->MatID == -1) {
 ...
 ****************************************************************/
{
  int i, k, have_generic = FALSE, have_MF = FALSE, have_specific_MF = FALSE;
  int imtrx;
...
void assign_species_desc_suffix(const int species_var_name, char *retn_string)
...
void assign_species_name(const int kspec,
                         struct Material_Properties *mp_ptr,
                         char *species_name,
                         char *species_desc,
                         int matID)
 ...
 ************************************************************************/
{
  ...
  if (mp_ptr->Num_Species > kspec) {
    ...
    if ((mp_ptr->Species_Names != NULL) && (strlen(mp_ptr->Species_Names[kspec]) > 0) &&
        (strcmp(pstring, mp_ptr->Species_Names[kspec]))) {
      strcat(species_name, mp_ptr->Species_Names[kspec]);
      if (species_desc != NULL) {
        sprintf(species_desc, "Species %s %s", mp_ptr->Species_Names[kspec], species_desc_end);
      }
    } else {
    ...
    printf("assign_species_name warning: Called with a bad species index: %d", kspec);
  }
}
/******************************************************************************/
/******************************************************************************/
...
void assign_var_name(const int varType,
                     const int idof,
 ...
 ***********************************************************************/
{
  ...
  } else if (matID != -1 && pd_glob[matID]->i[upd->matrix_index[varType]][varType] == I_P0) {
    sprintf(species_name, "%s_E", Var_Name[varType].name2);
    strcpy(species_desc, Var_Name[varType].name1);
  } else if (matID != -1 && pd_glob[matID]->i[upd->matrix_index[varType]][varType] == I_P1) {
