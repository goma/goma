...
void setup_element_storage(void)
...
void init_element_storage(ELEM_BLK_STRUCT *eb_ptr, int mn)
 ...
 *****************************************************************/
{
  ...
  if (!eb_ptr->ElemStorage) {
    ...
    for (i = 0; i < eb_ptr->Num_Elems_In_Block; i++, s_ptr++) {
      if (pd->e[pg->imtrx][R_POR_LIQ_PRES] || pd->e[pg->imtrx][R_SHELL_SAT_OPEN] ||
...
void set_init_Element_Storage(ELEM_BLK_STRUCT *eb_ptr, int mn)
 ...
 *****************************************************************/
{
  ...
  if (mp_glob[mn]->SaturationModel == TANH_HYST) {
    ...
    if (Guess_Flag == 4 || Guess_Flag == 5) {
      GOMA_EH(GOMA_ERROR, "Not a smooth restart for hysteretic saturation function. If you really "
                          "want to do this use read_exoII_file or call us");
    }
      ...
      for (ip = 0; ip < ip_total; ip++) {
        ...
        for (j = 0; j < num_vars; j++) {
          if (!strcasecmp(appended_name, var_names[j])) {
            ...
            error = ex_get_var(exoid, time_step, EX_ELEM_BLOCK, j + 1, eb_ptr->Elem_Blk_Id,
                               eb_ptr->Num_Elems_In_Block, ev_tmp);
            ifound = 1;
          }
        }
        if (ifound) {
    ...
    } else /*Initialize as dictated by input cards */
    {
      ...
      if (Draining_curve == 1.0) {
        ...
        sat_switch =
            sat_max - (sat_max - sat_min) * 0.5 * (1.0 + tanh(beta_w - alpha_w / pc_switch));
      } else {
  ...
  if (elc_glob[mn]->thermal_expansion_model == SHRINKAGE) {
    ...
    if (Guess_Flag == 4 || Guess_Flag == 5) {
      GOMA_EH(
          GOMA_ERROR,
          "Not a smooth restart for solidification shrinkage model.Use read_exoII_file or call us");
    }

    if (Guess_Flag == 5 || Guess_Flag == 6) {
      GOMA_EH(
          GOMA_ERROR,
          "Initializing solidified shrinkage model from exoII file not available yet. Use zero");
    }
...
void free_element_blocks(Exo_DB *exo)
...
void free_element_storage(Exo_DB *exo)
...
void free_elemStorage(ELEM_BLK_STRUCT *eb_ptr)
...
double get_nodalSat_tnm1_FromES(int lnn)
...
double get_Sat_tnm1_FromES(int ip)
...
void put_nodalSat_tn_IntoES(int lnn, double sat)
...
void put_Sat_tn_IntoES(int ip, double sat)
