...
static int get_local_qtensor(double[DIM][DIM]);
 ...
 */
void assemble_qtensor(dbl *el_length) /* 2 x approximate element length scales */
{
  int print;
  int a, i, j, p;
  ...
  */
  ...
  /*   if(pd->e[pg->imtrx][R_MESH1]) GOMA_EH(GOMA_ERROR, "assemble_qtensor is not deformable mesh
   * friendly."); */
  ...
  for (ip = 0; ip < ip_total; ip++) {
    ...
    for (i = 0; i < DIM; i++) {
      ...
      for (j = 0; j < DIM; j++)
        qtensor[ip][i][j] = (dbl)delta(i, j) - 0.5 * vort_dir_local[i] * vort_dir_local[j];
    }
    ...
    */
    ...
    for (p = 0; p < pd->Num_Dim; p++) {
      ...
      */
      ...
      if (!bias_eigenvector_to(vort_dir_delta, vort_dir_local) || print) {
        ...
        printf("             E = | % 10.4g % 10.4g % 10.4g |\n", E[1][0], E[1][1], E[1][2]);
        printf("                 \\ % 10.4g % 10.4g % 10.4g /\n", E[2][0], E[2][1], E[2][2]);
        printf("      vort_dir = [% 10.4g % 10.4g % 10.4g]\n", vort_dir_local[0], vort_dir_local[1],
               vort_dir_local[2]);
        printf("vort_dir_delta = [% 10.4g % 10.4g % 10.4g]\n", vort_dir_delta[0], vort_dir_delta[1],
               vort_dir_delta[2]);
        printf("|v| = % 10.4g\n",
               sqrt(fv->v[0] * fv->v[0] + fv->v[1] * fv->v[1] + fv->v[2] * fv->v[2]));
        printf("sh = % 10.4g, gd = % 10.4g, gd_delta = % 10.4g\n", fv->SH, gd_local, gd_delta);
      }

      for (i = 0; i < DIM; i++)
        d_vort_dir_d_x[i][p] = (vort_dir_delta[p] - vort_dir_local[p]) / (delta[p]);
      /*
    ...
    for (a = 0; a < DIM; a++) {
      for (i = 0; i < DIM; i++) {
        div_qtensor[ip][a] += (hq[a][i] * vort_dir_local[a] * vort_dir_local[i] / h[a] -
    ...
    */
    ...
    if (print) {
      printf("    d/dq[0] = [% 10.4g % 10.4g % 10.4g]\n", d_vort_dir_d_x[0][0],
             d_vort_dir_d_x[1][0], d_vort_dir_d_x[2][0]);
      printf("    d/dq[1] = [% 10.4g % 10.4g % 10.4g]\n", d_vort_dir_d_x[0][1],
             d_vort_dir_d_x[1][1], d_vort_dir_d_x[2][1]);
      printf("    d/dq[2] = [% 10.4g % 10.4g % 10.4g]\n", d_vort_dir_d_x[0][2],
             d_vort_dir_d_x[1][2], d_vort_dir_d_x[2][2]);
      printf("div_qtensor = [% 10.4g % 10.4g % 10.4g]\n", div_qtensor[ip][0], div_qtensor[ip][1],
             div_qtensor[ip][2]);
      /*
 ...
 * direction variable.
 */
void assemble_qtensor_vort(dbl *el_length) /* 2 x approximate element length scales */
{
  int print;
  int a, i, j, p;
  int ielem_type, ip_total, ip;
 ...
 */
void assemble_new_qtensor(dbl *el_length) /* 2 x approximate element length scales */
{
  int err = 0;
  int i, j, p, k1;
  int ielem_type, ip_total, ip;
  ...
  if (pd->e[pg->imtrx][R_MESH1])
    GOMA_EH(GOMA_ERROR, "assemble_qtensor is not deformable mesh friendly.");
  if (pd->Num_Dim == 3)
    GOMA_EH(GOMA_ERROR, "qtensor not ready for 3D problems yet!");
  if (pd->CoordinateSystem != CARTESIAN && pd->CoordinateSystem != PROJECTED_CARTESIAN &&
  ...
  for (ip = 0; ip < ip_total; ip++) {
    ...
    for (p = 0; p < pd->Num_Dim; p++) {
      ...
      for (i = 0; i < DIM; i++) {
        for (j = 0; j < DIM; j++) {
          d_qtensor[i][j][p] = (local_q[i][j] - local_q2[i][j]) / (2.0 * dp);
...
static int get_local_qtensor(double q[DIM][DIM])
/*
 ...
 */
int assemble_vorticity_direction(void) {
          ...
          for (q = 0; q < VIM; q++) {
            for (r = 0; r < VIM; r++) {
              for (k = 0; k < ei[pg->imtrx]->dof[var]; k++) {
                grad_v[p][q] += vv[r][k] * bf[var]->grad_phi_e[k][r][p][q];
              }
            }
          }
        }
        ...
        for (a = 0; a < DIM; a++) {
          ...
          for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
            ...
            R_new[a][i] = (-vort_dir_pert[a] * pd->etm[pg->imtrx][eqn][(LOG2_ADVECTION)] +
                           fv->vd[a] * pd->etm[pg->imtrx][eqn][(LOG2_SOURCE)]) *
                          wt_func * dV;
      ...
      for (b = 0; b < dim; b++) {
        ...
        for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
          ...
          for (a = 0; a < dim; a++) {
            for (p = 0; p < dim; p++) {
              for (k = 0; k < dofs; k++) {
                J[a][p] += (xcoor[p][k] + dd[p][k]) * bf[R_MESH1]->dphidxi[k][a];
              }
            }
          }
          if (dim == 2) {
          ...
          if (pd->CoordinateSystem == CARTESIAN || pd->CoordinateSystem == PROJECTED_CARTESIAN ||
              pd->CoordinateSystem == CARTESIAN_2pt5D) { /* Do nothing. Everything stays 1. */
          ...
          if (pd->CoordinateSystem != CARTESIAN && pd->CoordinateSystem != PROJECTED_CARTESIAN &&
            ...
            for (q = 0; q < VIM; q++) {
              for (p = 0; p < VIM; p++) {
                grad_e[q][p][p] = hq[p][q] / (h[p] * h[q]);
              }
            }
            for (p = 0; p < VIM; p++) {
              for (q = 0; q < VIM; q++) {
                grad_e[p][p][q] -= hq[p][q] / (h[p] * h[q]);
              }
            }
          }
          ...
          for (p = 0; p < VIM; p++) {
            for (q = 0; q < VIM; q++) {
              for (r = 0; r < VIM; r++) {
                for (k = 0; k < ei[pg->imtrx]->dof[var]; k++) {
                  grad_v[p][q] += *esp->v[r][k] * grad_phi_e[k][r][p][q];
                }
              }
            }
          }
 ...
 */
int hydro_qtensor_flux(struct Species_Conservation_Terms *st, int w) /* species number */
{
  int a, b, i, j, var;
  int status = 1;
  ...
  if (!pd->e[pg->imtrx][VORT_DIR3] && 0) {
    GOMA_EH(GOMA_ERROR,
            "Cannot use QTENSOR without the VORT_DIR{1,2,3} equations/variables active!");
    exit(-1);
  }
 ...
 *          R.R. Rao, 7/04
 */
int hydro_qtensor_flux_new(struct Species_Conservation_Terms *st, int w) /* species number */
{
  int a, b, i, j, var;
  ...
  if (!vort_dir_on) {
    GOMA_EH(
        GOMA_ERROR,
        "Cannot use this QTENSOR model without the VORT_DIR{1,2,3} equations/variables active!");
    exit(-1);
  }
  ...
  for (a = 0; a < VIM; a++) {
    for (b = 0; b < VIM; b++) {
      qtensor_loc[a][b] = (dbl)delta(a, b) - 0.5 * fv->vd[a] * fv->vd[b];
    }
  }
        ...
        for (b = 0; b < DIM; b++) {
          for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
            d_qtensor_dvd[p][q][b][j] =
                -0.5 * phi_j * (fv->vd[q] * delta(p, b) + fv->vd[p] * delta(q, b));
          }
        }
      }
    }
    ...
    for (a = 0; a < DIM; a++) {
      for (b = 0; b < DIM; b++) {
        for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
          ...
          d_div_q_dvd[a][b][j] =
              -0.5 * (phi_j * delta(a, b) * fv->div_vd + fv->vd[a] * div_phi_j_e_b);
          for (p = 0; p < VIM; p++) {
 ...
 * cross product.
 */
void cross_really_simple_vectors(const dbl *v0, /* v0 */
                                 const dbl *v1, /* v1 */
                                 dbl *v2)       /* v2 = v0 x v1 */
{
  int i, j, k;
 ...
 */
dbl normalize_really_simple_vector(dbl *v, int dim) {
  int i;
  dbl norm;
...
dbl really_simple_vector_magnitude(dbl *v, int dim) {
  int i;
  dbl norm;

  for (i = 0, norm = 0.0; i < dim; i++, v++)
    norm += (*v) * (*v);
 ...
 * Author: MMH 9/3/99
 */
void compute_principle_directions(dbl *v_flow, dbl *v_norm, dbl *v_vort, int print) {
  dbl vf_norm;
  int i, j, k;
  dbl t1;
 ...
 * Author: MMH 11/8/99
 */
void find_eigenvector(dbl AA[3][3], dbl lambda, dbl *v, int print) {
  int i, p[3];
...
#ifdef DEBUG_DIAGONALIZATION
      if (print)
        printf("Swapping first and third rows.\n");
#endif
      p[0] = 2;
      p[2] = 0;
    }
...
#ifdef DEBUG_DIAGONALIZATION
      if (print)
        printf("Swapping first and second rows.\n");
#endif
      p[0] = 1;
      p[1] = 0;
    } else {
  ...
  if (fabs(A[p[0]][0]) < QTENSOR_SMALL_DBL) {
     ...
     */
    if (fabs(A[p[1]][1] * A[p[2]][2] - A[p[1]][2] * A[p[2]][1]) < QTENSOR_SMALL_DBL)
      x = 1.0;
    else {
#ifdef DEBUG_DIAGONALIZATION
      if (print)
        printf("Echeck, double evector\n");
#endif
    }
  } else {
    m12 = A[p[1]][0] / A[p[0]][0];
...
#ifdef DEBUG_DIAGONALIZATION
      if (print)
        printf("Swapping second and third rows.\n");
#endif
    }
...
#ifdef DEBUG_DIAGONALIZATION
      if (print)
        printf("Third multiplier is %g = %g/%g\n", m23, A[p[2]][1], A[p[1]][1]);
#endif
      A[p[2]][1] = 0.0;
      A[p[2]][2] -= m23 * A[p[1]][2];
    }
...
void find_super_special_eigenvector(
    dbl T[DIM][DIM], dbl *v, dbl *v1, dbl *v2, dbl *v3, dbl *eigenvalue, int print) {
  ...
  *eigenvalue = 0.0;
  ...
  if (fabs(z1) < QTENSOR_SMALL_DBL)
    z1 = 0.0;
  if (fabs(z2) < QTENSOR_SMALL_DBL)
    z2 = 0.0;
  if (fabs(z3) < QTENSOR_SMALL_DBL)
    z3 = 0.0;
  ...
  /* Try to catch some roundoff errors. */
  if (fabs(v[0]) < QTENSOR_SMALL_DBL)
    v[0] = 0.0;
  if (fabs(v[1]) < QTENSOR_SMALL_DBL)
    v[1] = 0.0;
  if (fabs(v[2]) < QTENSOR_SMALL_DBL)
    v[2] = 0.0;
}

void find_eigenvalues_eigenvectors(
    dbl T[DIM][DIM], dbl *e1, dbl *e2, dbl *e3, dbl *v1, dbl *v2, dbl *v3) {
  int num_zero_eigenvalues;
  int i, j, print = 0;
  dbl A[3][3];
  dbl a0, a2, a1;
...
void diagonalize_symmetric_tensor(
    dbl T[DIM][DIM], dbl *v0, dbl *v1, dbl *v2, dbl *eigenvalues, int print) {
  int i, j, k;
  int num_zero_eigenvalues;
  dbl A[3][3];
  dbl a0, a2, a1;
  ...
  a1 = A[1][1] * A[2][2] + A[0][0] * A[2][2] + A[0][0] * A[1][1] - A[0][1] * A[1][0] -
       A[0][2] * A[2][0] - A[1][2] * A[2][1];
  a0 = -A[0][0] * A[1][1] * A[2][2] + A[0][1] * A[0][1] * A[2][2] + A[0][0] * A[1][2] * A[1][2] +
       A[0][2] * A[0][2] * A[1][1] - A[0][1] * A[2][0] * A[1][2] - A[0][2] * A[1][0] * A[2][1];
...
void diagonalize_rate_of_deformation_tensor(
    dbl T[3][3], dbl *v0, dbl *v1, dbl *v2, dbl *eigenvalues, int print) {
  int i, j, k;
  int num_zero_eigenvalues;
  dbl A[3][3];
...
void compute_VQVt_directly(dbl T[3][3], dbl VQVt[3][3], int print) {
          ...
          for (g1 = 0; g1 < G1; g1++) {
            ...
            for (k = 0; k < VIM; k++)
              for (l = 0; l < VIM; l++) {
                ...
                prod *= S[k][i] * S[l][j];
                sum += gauss_weight[G3 - 1][g3] * gauss_weight[G2 - 1][g2] *
 ...
 */
void please_work(dbl T[3][3], /* Rate of deformation tensor */
 ...
 * solution...  */
void get_characteristic_eq_coeffs(dbl E[3][3], dbl *a0, dbl *a1, dbl *a2) {
  *a2 = -(E[0][0] + E[1][1] + E[2][2]);
  *a1 = E[0][0] * E[1][1] + E[0][0] * E[2][2] + E[1][1] * E[2][2] - E[0][1] * E[1][0] -
        E[0][2] * E[2][0] - E[1][2] * E[2][1];
  *a0 = -(E[0][0] * (E[1][1] * E[2][2] - E[1][2] * E[2][1]) +
          E[0][1] * (E[1][2] * E[2][0] - E[1][0] * E[2][2]) +
          E[0][2] * (E[1][0] * E[2][1] - E[1][1] * E[2][0]));
}

int bias_eigenvector_to(dbl *v, dbl *target) {
  dbl dot_prod = 0.0;
  dbl v_mag = 0.0;
  int i, retval;
