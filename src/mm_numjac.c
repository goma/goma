...
static Coloring *find_coloring(
    struct GomaLinearSolverData *ams, int num_unknowns, int num_total_nodes, Exo_DB *exo, Dpi *dpi);
...
******************************************************************************/
{
  ...
  if (Linear_Solver == FRONT)
    GOMA_EH(GOMA_ERROR, "Cannot use frontal solver with numjac. Use umf or lu");
  ...
  for (i = 0; i < MAX_VARIABLE_TYPES; i++) {
    if (count[i])
      x_scale[i] = sqrt(x_scale[i] / count[i]);
  ...
  MPI_Allreduce(&neg_elem_volume, &neg_elem_volume_global, 1, MPI_INT, MPI_LOR, MPI_COMM_WORLD);
  neg_elem_volume = neg_elem_volume_global;

  neg_lub_height_global = FALSE;
  MPI_Allreduce(&neg_lub_height, &neg_lub_height_global, 1, MPI_INT, MPI_LOR, MPI_COMM_WORLD);
  neg_lub_height = neg_lub_height_global;

  zero_detJ_global = FALSE;
  MPI_Allreduce(&zero_detJ, &zero_detJ_global, 1, MPI_INT, MPI_LOR, MPI_COMM_WORLD);
...
void numerical_jacobian(struct GomaLinearSolverData *ams,
                        double x[],            /* Solution vector for the current processor */
                        double resid_vector[], /* Residual vector for the current
                                                * processor */
                        double delta_t,        /* time step size */
...
******************************************************************************/
{
  ...
  if (aj_off_diag == NULL || scale == NULL)
    GOMA_EH(GOMA_ERROR, "No room for storage for computing numerical jacobian");
  ...
  (void)matrix_fill_full(ams, x, resid_vector, x_old, x_older, xdot, xdot_old, x_update, &delta_t,
  ...
  if (Debug_Flag == -2) {
    ...
    abs_max = 0.0;
    for (i = 0; i < NumUnknowns[pg->imtrx]; i++) {
      if (fabs(scale[i]) > abs_max)
        abs_max = fabs(scale[i]);
      if (fabs(scale[i]) < abs_min)
        abs_min = fabs(scale[i]);
    }
  ...
  for (j = 0; j < NumUnknowns[pg->imtrx]; j++) /* loop over each column */
  {
    ...
    for (i = exo->node_elem_pntr[my_node_num]; i < exo->node_elem_pntr[my_node_num + 1]; i++) {
      my_elem_num = exo->node_elem_list[i];
      for (k = exo->elem_node_pntr[my_elem_num]; k < exo->elem_node_pntr[my_elem_num + 1]; k++) {
    ...
    for (i = 0; i < num_elems; i++) {
      ...
      matrix_fill(ams, x_1, resid_vector_1, x_old, x_older, xdot, xdot_old, x_update, &delta_t,
                  &theta, first_elem_side_BC_array, &time_value, exo, dpi, &elem_list[i],
                  &num_total_nodes, h_elem_avg, U_norm, NULL, zeroCA);
      zeroCA = 0;
      if (neg_elem_volume)
        break;
...
#ifdef DEBUG_NUMJAC
      DPRINTF(stderr, "x[%02d]=%-15g, resid_vector_1[%02d]=%-15g, resid_vector[%02d]=%-15g", i,
              x[i], i, resid_vector_1[i], i, resid_vector[i]);
    ...
    if (nj_err >= RESIDUAL_TOLERANCE || nj_scaled_err >= SCALED_RESIDUAL_TOLERANCE) {
      DPRINTF(stderr,
              "Diag%.22s Var%.22s aj=%-10.4g nj=%-10.4g er=%9.4g rer=%9.4g x=%-10.4g r=%-10.4g\n",
      ...
      if (jcolumn[k] == j) /* match the column numbers */
      {
        for (ii = 0; ii < num_dofs; ii++) {
          ...
          if (irow[k] == i) /* match the row numbers */
          {
            ...
            if (nj_err >= RESIDUAL_TOLERANCE || nj_scaled_err >= SCALED_RESIDUAL_TOLERANCE)
              DPRINTF(stderr,
                      "  Eq%.42s Var%.42s aj=%-10.4g nj=%-10.4g er=%9.4g rer=%9.4g x=%-10.4g "
                      "r=%-10.4g\n",
                      resname[pg->imtrx][irow[k]], dofname[pg->imtrx][jcolumn[k]], aj_off_diag[k],
                      nj[i], nj_err, nj_scaled_err, x_1[j], resid_vector_1[j]);
          }
        }
      }
    }
    ...
    for (ii = 0; ii < num_dofs; ii++) {
       ...
       * );*/
      if (fabs(a[i] * x_scale[idv[pg->imtrx][i][0]]) > resid_scale)
        resid_scale = fabs(a[i] * x_scale[idv[pg->imtrx][i][0]]);
      for (k = ija[i]; k < ija[i + 1]; k++) {
        var_j = idv[pg->imtrx][ija[k]][0];
        /*DRN-MAX BROKEN? resid_scale = MAX( resid_scale, fabs( a[k] * x_scale[var_j]) );*/
        if (fabs(a[k] * x_scale[var_j]) > resid_scale)
          resid_scale = fabs(a[k] * x_scale[var_j]);
      }
      ...
      if (resid_vector_1[i] < resid_min || resid_vector_1[i] > resid_max) {
        ...
        /*DRN-MAX BROKEN? resid_error = MAX( resid_vector_1[i] - resid_max, resid_min -
         * resid_vector_1[i] );*/
        if (resid_vector_1[i] < resid_min)
          resid_error = resid_min - resid_vector_1[i];
        else
          resid_error = resid_vector_1[i] - resid_max;
         ...
         * residual error.
         */
        confidence = resid_error / (2. * roundoff * resid_scale);

        if (i == j) {
          DPRINTF(stderr,
                  "Diag%32.32s Var%32.32s x=%-10.4g dx=%-10.4g aj=%-10.4g nj=%-10.4g aj_1=%-10.4g "
                  "d_aj=%-10.4g conf=%-10.4g\n",
...
*/
        ...
        for (k = exo->node_elem_pntr[my_node_num]; k < exo->node_elem_pntr[my_node_num + 1]; k++) {
            ...
            do {
              for (ibc = 0; (bc_input_id = (int)elem_side_bc->BC_input_id[ibc]) != -1; ibc++) {
                ...
                if (in_list(I, 0, elem_side_bc->num_nodes_on_side, elem_side_bc->local_node_id)) {
                  eqn = var_i;
                  if (BC_Types[bc_input_id].desc->vector && (eqn == VELOCITY2 || eqn == VELOCITY3))
                    eqn = VELOCITY1;
#ifdef DEBUG_NUMJAC
                  ...
                  DPRINTF(stderr, "%s on %sID=%d\n", BC_Types[bc_input_id].desc->name1,
                          BC_Types[bc_input_id].Set_Type, BC_Types[bc_input_id].BC_ID);
#endif
                }
              }
      ...
      if (!Inter_Mask_save[var_i][var_j]) {
        ...
        if ((aj[i] != 0.) || (aj_1[i] != 0.)) {
          DPRINTF(stderr,
                  "Potential dependency error: Inter_Mask[pg->imtrx][Eq%.32s][Var%.32s]=0, but "
                  "aj=%-10.4g aj_1=%-10.4g\n",
...
static int intcompare(const void *left, const void *right)
...
static void piksr2(const int n, int arr[], int brr[], dbl crr[]) {
  int i;
  data_t *vals;
 ...
 */
#ifdef FORWARD_DIFF_NUMJAC
static void compute_numerical_jacobian_errors(const dbl exact,    /* analytic value */
                                              ...
                                              dbl *nj_scaled_err) /* scaled error */
{
  ...
  *nj_scaled_err = *nj_err / fexact;
   ...
   */
  if (*nj_scaled_err >= SCALED_RESIDUAL_TOLERANCE && *nj_err < MIXED_RESIDUAL_TOLERANCE)
    *nj_scaled_err = 0.0;
  /* Case 6: have an absolute error that causes reporting, but the
   * scaled error is so small that we don't worry about it.
   */
  if (*nj_err >= RESIDUAL_TOLERANCE && *nj_scaled_err < MIXED_SCALED_RESIDUAL_TOLERANCE)
    *nj_err = 0.0;
}
#endif

#ifndef COUPLED_FILL
void numerical_jacobian_fill(int ijaf[],     /* fill Vector of integer pointers into a matrix */
...

{
  ...
  if (nj == NULL || scale == NULL)
    GOMA_EH(GOMA_ERROR, "No room for numerical jacobian arrays");
  ...
  (void)fill_matrix(afill, ijaf, rf, xf, x, x_old, xdot, delta_t, theta, ADVECT, node_to_fill,
...
double calc_numerical_delta(double base_value)
...
void AF_assemble_Residual_Only(void)
...
void AF_restore_Jacobian_Flag(void)
