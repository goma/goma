         ...
         for ( i=0; i < ei->dof[eqn]; i++)
            {
             ...
             /* Advection term */
             advection = 0.0;
             if (pd->e[R_LUBP] && pd->e[eqn] && T_ADVECTION)
               {
                for ( a = 0; a < dim; a++)
                   {
             ...
             if ( pd->e[eqn] & T_SOURCE )
               {
                if (mp->SpeciesSourceModel[w] == CONSTANT)
                          ...
                          (mp->SpeciesSourceModel[w] == ETCHING_KOH_EXT) )
                  {
                   ...
                   EH(err, "etching_KOH_source");
                   source = mp->species_source[w];
                  }
                else
                  {
                   EH(-1, "Only CONSTANT or ETCHING_KOH is permitted in source model of shell species equation ");
                  }
               }
             source *= phi_i * det_J * wt * h3 * pd->etm[eqn][(LOG2_SOURCE)];
