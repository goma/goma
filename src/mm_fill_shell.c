			   ...
			   const Exo_DB *exo)
{
  ...
  if ( af->Assemble_Residual ) {
    ...
    for (int i = 0; i < ei->dof[eqn]; i++) {
      ...
      diffusion *= penalty*pd0->etm[eqn][(LOG2_DIFFUSION)];
		  ...
		  for ( j_id=0; j_id<ei->dof[var]; j_id++)
		    {
                      lec->J[LEC_J_INDEX(peqn_shell,pvar,id_dofshell,j_id)] +=
                        scale*(fv->snormal[0] * lec->J[LEC_J_INDEX(peqn_mom1,pvar,id_dofmom1,j_id)] +
                               ...
                               fv->dsnormal_dx[0][q][j_id] * lec->R[LEC_R_INDEX(upd->ep[ieqn_mom1],id_dofmom1)] +
                               fv->dsnormal_dx[1][q][j_id] * lec->R[LEC_R_INDEX(upd->ep[ieqn_mom2],id_dofmom2)]);
		    }
		}
	    }
		  ...
		  for ( j_id=0; j_id<ei->dof[var]; j_id++)
		    {
                      lec->J[LEC_J_INDEX(peqn_shell,pvar,id_dofshell,j_id)] +=
                        scale*(fv->stangent[0][0] * lec->J[LEC_J_INDEX(peqn_mom1,pvar,id_dofmom1,j_id)] +
                               ...
                               fv->dstangent_dx[0][0][q][j_id] * lec->R[LEC_R_INDEX(upd->ep[ieqn_mom1],id_dofmom1)] +
                               fv->dstangent_dx[0][1][q][j_id] * lec->R[LEC_R_INDEX(upd->ep[ieqn_mom2],id_dofmom2)]);
		    }
		}
	    }
			  ...
			  double scale) /* Scale factor, nondimension       */
{
  ...
  if (af->Assemble_Residual) {
    ...
    if (!tension_fixed ) {
      ...
      lec->R[LEC_R_INDEX(upd->ep[ieqn_shell],id_dofshell)] +=	  scale * (fv->stangent[0][0] * lec->R[LEC_R_INDEX(upd->ep[ieqn_mom1],id_dofmom1)] +
                                                                   fv->stangent[0][1] * lec->R[LEC_R_INDEX(upd->ep[ieqn_mom2],id_dofmom2)]);
  ...
  if (af->Assemble_Jacobian) {
	...
	for ( b = 0; b < dim; b++) {
	 ...
	 for ( j = 0; j < n_dof[var]; j++)
       ...
       */
      var = SHELL_NORMAL1;
      if (pd->v[var] &&  mp->FSIModel == FSI_SHELL_ONLY_MESH) {
        pvar = upd->vp[var];
...

{
  ...
  dbl d_grad_II_sh_fh_dmesh[DIM][DIM][MDE], d_grad_II_n_dot_d_dmesh[DIM][DIM][MDE], d_grad_II_H_dmesh[DIM][DIM][MDE];
 ...
 /***** Calculate lubrication height *******/
  ...
  case FSI_SHELL_ONLY_UNDEF:
  case FSI_SHELL_ONLY_MESH:
    if ( (pd->e[R_SHELL_NORMAL1]) && (pd->e[R_SHELL_NORMAL2]))
      {
       for ( i = 0; i < dim; i++)
          {
           H -= fv->n[i] * fv->d[i];
           ...
           for ( b = 0; b < dim; b++)
              {
               for ( k = 0; k < ei->dof[MESH_DISPLACEMENT1]; k++)
                  {
                   dH_dot_dmesh[b][k] -= fv->n[i] * delta(i,b) * bf[MESH_DISPLACEMENT1]->phi[k] * (1+2*tt)/dt;
           ...
           for ( b = 0; b < dim; b++)
              {
               for ( k = 0; k < ei->dof[SOLID_DISPLACEMENT1]; k++)
                  {
                   jk = dof_map[k];
                   dH_dot_drealsolid[b][k] -= fv->n[i] * delta(i,b) * bf[SOLID_DISPLACEMENT1]->phi[jk] * (1+2*tt)/dt;
                   dH_dot_drealsolid[b][k] -= fv_dot->n[i] * delta(i,b) * bf[SOLID_DISPLACEMENT1]->phi[jk];
                  }
             }
          }
      }
               ...
               for ( k = 0; k < ei->dof[SHELL_NORMAL1]; k++)
                  {
                   ...
                   dH_dot_dnormal[b][k] -= fv->d[i] * delta(i,b) * bf[SHELL_NORMAL1]->phi[k] * (1+2*tt)/dt;
                  }
              }
          }
      }
               ...
               for ( k = 0; k < ei->dof[SHELL_NORMAL1]; k++)
                  {
                   ...
                   dH_dot_dnormal[b][k] -= fv->d_rs[i] * delta(i,b) * bf[SHELL_NORMAL1]->phi[k] * (1+2*tt)/dt;
                  }
              }
          }
      }
       ...
       for ( i = 0; i < dim; i++)
          {
           for ( j = 0; j < dim; j++)
              {
               grad_H[j] -=  fv->n[i] * fv->grad_d[i][j]
                            +fv->grad_n[j][i] * fv->d[i];
              }
          }
      }
    break;
       ...
       for ( i = 0; i < dim; i++)
          {
           for ( j = 0; j < dim; j++)
              {
               grad_H[j] -=  fv->n[i] * fv->grad_d_rs[i][j]
                            +fv->grad_n[j][i] * fv->d_rs[i];
              }
          }
      }
    break;
             ...
             for ( k = 0; k < ei->dof[MESH_DISPLACEMENT1]; k++)
                {
                 ...
                 for (b = 0; b < dim; b++)
                    {
                     d_grad_n_dot_d_dmesh[i][j][jk] +=   fv->n[b] * fv->d_grad_d_dmesh[b][i][j][k]
                 ...
                 for ( k = 0; k < ei->dof[MESH_DISPLACEMENT1]; k++)
                    {
                     ...
                     d_grad_II_H_dmesh[i][b][k] += d_grad_II_sh_fh_dmesh[i][b][k] - d_grad_II_n_dot_d_dmesh[i][b][k];
                    }
                }
            }
        }
             ...
             for (b = 0; b < dim; b++)
                {
                 for ( k = 0; k < ei->dof[SHELL_NORMAL1]; k++)
                    {
                     d_grad_H_dnormal[i][b][k] -=   bf[SHELL_NORMAL1]->phi[k] * delta(b,j) * fv->grad_d[j][i]
                 ...
                 for ( k = 0; k < ei->dof[SHELL_NORMAL1]; k++)
                    {
                     ...
                     d_grad_II_H_dnormal[i][b][k] -= grad_H[j] * delta(i,b) * bf[SHELL_NORMAL1]->phi[k] * fv->n[j];
                     d_grad_II_H_dnormal[i][b][k] -= grad_H[j] * delta(j,b) * fv->n[i] * bf[SHELL_NORMAL1]->phi[k];
                    }
                }
            }
        }
                        ...
                        if ( pd->e[eqn] & T_MASS )
                          {
                           ...
                           mass *= wt * h3 * pd->etm[eqn][(LOG2_MASS)];
                          }
                       }
	             ...
	             if (pd->e[eqn] & T_SOURCE)
	               {
	                source   = phi_i * EvapRate;
                        source  += phi_i * flux;
	                source  *= fv->dsurfdet_dx[b][jk] * wt * h3 * pd->etm[eqn][(LOG2_SOURCE)];
	               }
                        ...
                        if ( pd->e[eqn] & T_MASS )
                          {
                           ...
                           mass *= wt * det_J * h3 * pd->etm[eqn][(LOG2_MASS)];
                          }
                       }
...
/*ARGSUSED*/
int
assemble_film_1D( double time,     /* present time value */
                  double tt,       /* parameter to vary time integration from
                                   explicit (tt = 1) to implicit (tt = 0)    */
                  double dt,       /* current time step size */
                  double xi[DIM],  /* Local stu coordinates */
...

{
  ...
  if (pd->e[R_MESH1])
    ...
    {
     for (i = 0; i < ei->dof[SHELL_FILMP]; i++)
        {
  ...
  if (pd->e[R_MESH1])
  ...
  else
    {
     for (i = 0; i < ei->dof[SHELL_FILMP]; i++)
        {
         node  = ei->dof_list[R_SHELL_FILMP][i];
         index = Proc_Elem_Connect[ei->iconnect_ptr + node];
         ...
         for (b = 0; b < dim; b++)
            {
             ...
             for (j = 0; j < ei->dof[var]; j++)
                {
                 d_dD_dS_dmesh[p][b][j] =   delta(p,b) * dphi_dS[j]
                                          + *esp->d[b][j] * d_dphi_dS_dmesh[j][b][j];
                }
            }
        }
         ...
         for (j = 0; j < ei->dof[var]; j++)
            {
             ...
             dH_dot_dmesh[b][j] -=   fv->n[b] * bf[var]->phi[j] * (1.0 + 2.0*tt)/dt
                                   + fv_dot->n[b] * bf[var]->phi[j];
         ...
         for (j = 0; j < ei->dof[var]; j++)
            {
             ...
             dH_dot_dnormal[b][j] -=   fv->d[b] * bf[var]->phi[j] * (1+2*tt)/dt
                                     + fv_dot->d[b] * bf[var]->phi[j];

             for (p = 0; p < dim; p++)
                {
                 dgrad_H_dnormal[b][j] -=   bf[var]->phi[j] * dD_dS[p] * delta(p,b)
                                          + dphi_dS[j] * fv->d[p] * delta(p,b);
                }
            }
        }
     break;
          ...
          for (j = 0; j < ei->dof[var]; j++)
             {
              dq_dmesh[b][j] = - (H*H)/mu * dH_dmesh[b][j] * dP_dS
                               - (H*H*H)/(3.0 * mu) * d_dP_dS_dmesh[b][j]
                               + veloL[0] * dH_dmesh[b][j];
             }
         }
     }
      ...
      for (b = 0; b < dim; b++)
         {
          ...
          for (j = 0; j < ei->dof[var]; j++)
             {
              dq_dnormal[b][j] = -(H*H)/mu * dH_dnormal[b][j] * dP_dS
                                 + veloL[0] * dH_dnormal[b][j];
             }
         }
     }
                    ...
                    if ( pd->TimeIntegration != STEADY )
                      {
                       if ( pd->e[eqn] & T_MASS )
                         {
//                          mass += phi_i * dH_dot_dmesh[b][jk] * det_J;
//                          mass  = phi_i * H_dot * d_det_J_dmesh[b][jk];
                    ...
                    if (pd->e[eqn] & T_DIFFUSION)
                      {
//                       diffusion += - dq_dmesh[b][jk] * dphi_i_dS * det_J;
//                       diffusion += - q * d_dphi_dS_dmesh[i][b][jk] * det_J;
//                       diffusion += - q * dphi_i_dS * d_det_J_dmesh[b][jk];
                    ...
                    if (pd->e[eqn] & T_SOURCE)
                      {
//                       source  += phi_i * flux * d_det_J_dmesh[b][jk];
                ...
                if (pd->e[eqn] & T_DIFFUSION)
                  {
                   ...
                   diffusion *= det_J * h3 * wt * pd->etm[eqn][(LOG2_DIFFUSION)];
                  }
                ...
                for ( j=0; j<ei->dof[var]; j++)
                   {
                    ...
                    if (pd->e[eqn] & T_DIFFUSION)
                      {
//                       diffusion += - mp->surface_tension * d_dphi_dS_dmesh[i][b][jk] * grad_H * det_J;
//                       diffusion += - mp->surface_tension * dphi_i_dS * dgrad_H_dmesh[b][jk] * det_J;
//                       diffusion += - mp->surface_tension * dphi_i_dS * grad_H * d_det_J_dmesh[b][jk];
                    ...
                    if (pd->e[eqn] & T_SOURCE)
                      {
//                       source += phi_i * fv->sh_fp * d_det_J_dmesh[b][jk];
                                ...
                                )
{
  ...
  dbl etm_mass[MAX_POR_SHELL] = {0.0}, etm_diff[MAX_POR_SHELL] = {0.0}, etm_sour[MAX_POR_SHELL] = {0.0};
  for (ipore = 0; ipore < pd->Num_Porous_Shell_Eqn; ipore++)
     {
      eqn = porous_shell_eqn[ipore];
      if (pd->e[eqn])
        {
         etm_mass[ipore] = pd->etm[eqn][(LOG2_MASS)];
         etm_diff[ipore] = pd->etm[eqn][(LOG2_DIFFUSION)];
         etm_sour[ipore] = pd->etm[eqn][(LOG2_SOURCE)];
        }
     }
            ...
            for ( a = 0; a < DIM; a++)
               {
                for ( b = 0; b < DIM; b++)
                   {
                    E_DIFF[ipore][a]      += H[ipore] * mp->PorousShellPermTensor[ipore][a][b] * rel_liq_perm[ipore] *
                                             (grad_II_p[ipore][b] + mp->momentum_source[b]) ;
                    E_DIFF_S[ipore][a][b] += H[ipore] * mp->PorousShellPermTensor[ipore][a][b] * rel_liq_perm[ipore];
                    E_DIFF_S2[ipore][a][b] += H[ipore] * mp->PorousShellPermTensor[ipore][a][b] * mp->d_PorousShellRelPerm[ipore][var] *
                                          (grad_II_p[ipore][b] + mp->momentum_source[b]);
                   }
               }
           }
            ...
            for ( a = 0; a < DIM; a++)
               {
                ...
                for ( b = 0; b < DIM; b++)
                   {
                    E_DIFF_S[ipore][a][b]  += H[ipore] * mp->PorousShellPermeability[ipore] * delta(a,b) * rel_liq_perm[ipore];
                    E_DIFF_S2[ipore][a][b] += H[ipore] * mp->PorousShellPermeability[ipore] * delta(a,b) * mp->d_PorousShellRelPerm[ipore][var]
                                              * (grad_II_p[ipore][b] + mp->momentum_source[b]);
                   }
                }
           }
  ...
  if (pd->Num_Porous_Shell_Eqn > 1)
    {
     ...
     dbl dj_1_2[MAX_POR_SHELL][MDE] = {{0.0}};  // Sensitivity of the flux between porous layers 1 and 2
     dbl dj_2_3[MAX_POR_SHELL][MDE] = {{0.0}};  // Sensitivity of the flux between porous layers 2 and 3
  	            ...
  	            for ( j = 0; j < ei->dof[var]; j++)
	                ...
	                if ( (T_DIFFUSION) && (ipore == jpore) )
	                   ...
	                   for ( a = 0; a < DIM; a++)
	                       ...
	                       for ( b = 0; b < DIM; b++)
                                  {
		                   diff -= E_DIFF_S[ipore][a][b] * gradII_phi_i[a] * gradII_phi_j[b] * d_cap_pres_dS[ipore][j];
  ...
  if (af->Assemble_Jacobian) {
    ...
    */
      ...
      if( mp->FSIModel == FSI_SHELL_ONLY_MESH) {
        ...
        for (l = 0; l<DIM; l++) {
          var = SHELL_NORMAL1 + l;
          if (pd->v[var]) /* var = SHELL_NORMAL1,2,3 */ {
            pvar = upd->vp[var];
            // Loop over DOF (j)
            for ( j = 0; j < ei->dof[var]; j++) {
              // Load basis functions
    ...
    for ( i = 0; i < ei->dof[eqn]; i++) {
      ...
      for (l = 0; l<DIM; l++) {
        var = MESH_DISPLACEMENT1 + l;
        if (pd->v[var]) /* var = MESH_DISPLACEMENT1,2,3 */ {
          pvar = upd->vp[var];
          // Loop over DOF (j)
          for ( j = 0; j < ei->dof[var]; j++) {
            // Load basis functions
