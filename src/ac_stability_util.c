...
void modify_basis_and_weight_functions_for_LSA_3D_of_2D(void) {
  ...
  if (!af->Assemble_LSA_Jacobian_Matrix && !af->Assemble_LSA_Mass_Matrix)
    return;
  ...
  */
  ...
  for (bf_index = 0; bf_index < Num_Basis_Functions; bf_index++) {
    ...
    if (var != -1) {
      ...
      if (bf_ptr->interpolation != I_Q2_LSA && bf_ptr->interpolation != I_Q2_D_LSA) {
        for (i = 0; i < ei[pg->imtrx]->dof[var]; i++) {
          ...
          for (a = 0; a < VIM; a++)
            for (b = 0; b < VIM; b++)
              bf_ptr->grad_phi_e[i][a][b][a] = bf_ptr->grad_phi[i][b];
          ...
          if (pd_glob[mn]->e[pg->imtrx][R_MESH1]) {
            for (b = 0; b < 2; b++)
              for (j = 0; j < ei[pg->imtrx]->dof[R_MESH1]; j++) {
                bf_ptr->d_grad_phi_dmesh[i][0][b][j] *= cosine_value;
                bf_ptr->d_grad_phi_dmesh[i][1][b][j] *= cosine_value;
                /* Note that multiplying by N sine_value
                   is almost certainly wrong.  The "d_phi"
                   it's multiplying by should probably
                   just be "phi".  */
                bf_ptr->d_grad_phi_dmesh[i][2][b][j] *= -N * sine_value;
                bf_ptr->d_d_phi_dmesh[i][0][b][j] *= cosine_value;
                bf_ptr->d_d_phi_dmesh[i][1][b][j] *= cosine_value;
                bf_ptr->d_d_phi_dmesh[i][2][b][j] *= -N * sine_value;
                for (a = 0; a < VIM; a++) {
                  bf_ptr->d_grad_phi_e_dmesh[i][a][0][a][b][j] *= cosine_value;
                  bf_ptr->d_grad_phi_e_dmesh[i][a][1][a][b][j] *= cosine_value;
                  bf_ptr->d_grad_phi_e_dmesh[i][a][2][a][b][j] *= -N * sine_value;
                }
              }
          }
        }
       ...
       * appropriately. */
        ...
        for (i = 0; i < ei[pg->imtrx]->dof[var]; i++) {
          ...
          if (pd_glob[mn]->e[pg->imtrx][R_MESH1])
            for (b = 0; b < 2; b++)
              for (j = 0; j < ei[pg->imtrx]->dof[R_MESH1]; j++) {
                bf_ptr->d_grad_phi_dmesh[i][0][b][j] *= sine_value;
                bf_ptr->d_grad_phi_dmesh[i][1][b][j] *= sine_value;
                bf_ptr->d_grad_phi_dmesh[i][2][b][j] *= N * cosine_value;
                bf_ptr->d_d_phi_dmesh[i][0][b][j] *= sine_value;
                bf_ptr->d_d_phi_dmesh[i][1][b][j] *= sine_value;
                bf_ptr->d_d_phi_dmesh[i][2][b][j] *= N * cosine_value;
                for (a = 0; a < DIM; a++) {
                  bf_ptr->d_grad_phi_e_dmesh[i][a][0][a][b][j] *= sine_value;
                  bf_ptr->d_grad_phi_e_dmesh[i][a][1][a][b][j] *= sine_value;
                  bf_ptr->d_grad_phi_e_dmesh[i][a][2][a][b][j] *= N * cosine_value;
                }
              }
        }
      }
    ...
    if (var != -1) {
      ...
      for (i = 0; i < vdof; i++) {
        for (b = 0; b < ei[pg->imtrx]->ielem_dim; b++) {
          for (j = 0; j < mdof; j++) {
            bfv->d_grad_phi_dmesh[i][2][b][j] = -bfv->grad_phi[i][b] * bfx->grad_phi[j][2];
            for (q = 0; q < VIM; q++) {
              bfv->d_grad_phi_e_dmesh[i][q][2][q][b][j] = bfv->d_grad_phi_dmesh[i][2][b][j];
            }
          }
        }
      }
 ...
 ************************************************************************/
{
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_T_dmesh[p][b][j] = -fv->grad_T[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_V_dmesh[p][b][j] = -fv->grad_V[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_qs_dmesh[p][b][j] = -fv->grad_qs[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_sh_J_dmesh[p][b][j] = -fv->grad_sh_J[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_apr_dmesh[p][b][j] = -fv->grad_apr[b] * bfx->grad_phi[j][2];
      }
    }
  }
  v = ACOUS_PIMAG;
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_api_dmesh[p][b][j] = -fv->grad_api[b] * bfx->grad_phi[j][2];
      }
    }
  }
  v = LIGHT_INTP;
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_poynt_dmesh[0][p][b][j] = -fv->grad_poynt[0][b] * bfx->grad_phi[j][2];
      }
    }
  }
  v = LIGHT_INTM;
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_poynt_dmesh[1][p][b][j] = -fv->grad_poynt[1][b] * bfx->grad_phi[j][2];
      }
    }
  }
  v = LIGHT_INTD;
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_poynt_dmesh[2][p][b][j] = -fv->grad_poynt[2][b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_restime_dmesh[p][b][j] = -fv->grad_restime[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_ars_dmesh[p][b][j] = -fv->grad_ars[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_sh_bv_dmesh[p][b][j] = -fv->grad_sh_bv[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_sh_p_dmesh[p][b][j] = -fv->grad_sh_p[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_lubp_dmesh[p][b][j] = -fv->grad_lubp[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_lubp_2_dmesh[p][b][j] = -fv->grad_lubp_2[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_sh_t_dmesh[p][b][j] = -fv->grad_sh_t[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_sh_fp_dmesh[p][b][j] = -fv->grad_sh_fp[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_sh_fh_dmesh[p][b][j] = -fv->grad_sh_fh[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_sh_pc_dmesh[p][b][j] = -fv->grad_sh_pc[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_SH_dmesh[p][b][j] = -fv->grad_SH[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_F_dmesh[p][b][j] = -fv->grad_F[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_P_dmesh[p][b][j] = -fv->grad_P[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_nn_dmesh[p][b][j] = -fv->grad_nn[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  for (w = 0; w < pd->Num_Species_Eqn; w++) {
    if (pd->v[pg->imtrx][v]) {
      for (b = 0; b < dim; b++) {
        for (j = 0; j < mdof; j++) {
          fv->d_grad_c_dmesh[p][w][b][j] = -fv->grad_c[w][b] * bfx->grad_phi[j][2];
        }
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_p_liq_dmesh[p][b][j] = -fv->grad_p_liq[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_p_gas_dmesh[p][b][j] = -fv->grad_p_gas[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_porosity_dmesh[p][b][j] = -fv->grad_porosity[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (q = 0; q < VIM; q++) {
      for (b = 0; b < dim; b++) {
        for (j = 0; j < mdof; j++) {
          fv->d_grad_v_dmesh[p][q][b][j] = -fv->grad_v[b][q] * bfx->grad_phi[j][2];
        }
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (b = 0; b < dim; b++) {
      for (j = 0; j < mdof; j++) {
        fv->d_grad_ext_v_dmesh[p][b][j] = -fv->grad_ext_v[b] * bfx->grad_phi[j][2];
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (q = 0; q < VIM; q++) {
      for (b = 0; b < dim; b++) {
        for (j = 0; j < mdof; j++) {
          fv->d_grad_E_field_dmesh[p][q][b][j] = -fv->grad_E_field[b][q] * bfx->grad_phi[j][2];
        }
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (q = 0; q < VIM; q++) {
      for (b = 0; b < dim; b++) {
        for (j = 0; j < mdof; j++) {
          fv->d_grad_pv_dmesh[p][q][b][j] = -fv->grad_pv[b][q] * bfx->grad_phi[j][2];
        }
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (q = 0; q < VIM; q++) {
      for (b = 0; b < dim; b++) {
        for (j = 0; j < mdof; j++) {
          fv->d_grad_d_dmesh[p][q][b][j] = -fv->grad_d[b][q] * bfx->grad_phi[j][2];
        }
      }
    }
  }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (q = 0; q < VIM; q++) {
      for (b = 0; b < dim; b++) {
        for (j = 0; j < mdof; j++) {
          fv->d_grad_d_rs_dmesh[p][q][b][j] = -fv->grad_d_rs[b][q] * bfx->grad_phi[j][2];
        }
      }
    }
  }
    ...
    for (m = 0; m < vn->modes; m++) {
      for (q = 0; q < VIM; q++) {
        for (r = 0; r < VIM; r++) {
          for (b = 0; b < dim; b++) {
            for (j = 0; j < mdof; j++) {
              fv->d_grad_S_dmesh[m][p][q][r][b][j] = -fv->grad_S[m][b][q][r] * bfx->grad_phi[j][2];
            }
          }
        }
      }
  ...
  if (pd->v[pg->imtrx][v]) {
    for (q = 0; q < VIM; q++) {
      for (r = 0; r < VIM; r++) {
        for (b = 0; b < dim; b++) {
          for (j = 0; j < mdof; j++) {
            fv->d_grad_G_dmesh[p][q][r][b][j] = -fv->grad_G[b][q][r] * bfx->grad_phi[j][2];
          }
        }
      }
    }
...

{
  ...
  if (LSA_3D_of_2D_pass == 0 || !(pd->e[pg->imtrx][R_MESH1]))
    return;

  for (j = 0; j < ei[pg->imtrx]->dof[MESH_DISPLACEMENT1]; j++) {
    for (q = 0; q < 2; q++) {
      fv->dsnormal_dx[2][q][j] = -fv->snormal[q] * bf[MESH_DISPLACEMENT1 + q]->grad_phi[j][2];
...
/*************************************************************************/
/*************************************************************************/
void get_eigen_outfile_name(char *in_name, int j, int k) {
  char prefix[MAX_FNL];
  char suffix[MAX_FNL];
  ...
  if (k != -1)
    sprintf(wave_string, "_wn=%g", LSA_wave_numbers[k]);
  ...
  if (k != -1)
    strcat(new_name, wave_string);
  if (suffix_length > 0)
    strcat(new_name, suffix);

  /* Multiname for this processor if parallel */
  if (Num_Proc > 1)
    multiname(new_name, ProcID, Num_Proc);
...
int anneal_mesh_LSA(double x[], Exo_DB *exo, double **saved_xyz, double **saved_displacement)
...

{
  ...
  for (gnn = 0; gnn < num_nodes; gnn++) {
    ...
    if (dim > 1) {
      idy =
          Index_Solution(gnn, MESH_DISPLACEMENT2, 0, 0, -1, upd->matrix_index[MESH_DISPLACEMENT2]);
      if (idy > -1) {
    ...
    if (dim > 2) {
      idz =
          Index_Solution(gnn, MESH_DISPLACEMENT3, 0, 0, -1, upd->matrix_index[MESH_DISPLACEMENT3]);
      if (idz > -1) {
...
int unanneal_mesh_LSA(double x[], Exo_DB *exo, double **saved_xyz, double **saved_displacement)
...
void add_displacement_LSA(double x[], Exo_DB *exo, double **saved_displacement)
...
void undo_add_displacement_LSA(double x[], Exo_DB *exo, double **saved_displacement)
