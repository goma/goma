...
void make_goma_dofmap(Exo_DB *x,
                      Bevm ***mult,
                      int ***evd,
                      int ***Lucky,
  ...
  for (n = 0; n < nn; n++) {
    ...
    for (l = np[n]; l < np[n + 1]; l++) {
...
#endif
      if (ep[e] + loc >= ep[e + 1]) {
        sr = sprintf(err_msg, "Difficulty finding local node in n=%d, e=%d", n, e);
        EH(-1, err_msg);
      }
      ...
      for (j = 0; j < num_basic_eqnvars[eb_index]; j++) {
        ...
        if (where == -1 && evid != UNDEFINED_EQNVARID) {
          ...
          num_abevs++;
#ifdef DEBUG
          fprintf(stderr, "\tevid not in this nodes catalog. adding...\n");
          fprintf(stderr, "\t\teqnvar ID, wts = %d ( %d %d %d )\n", evid,
                  mult[eb_index][j]->vect_mult, mult[eb_index][j]->conc_mult,
    ...
    while (!found_matching_node_type && d < num_kinds_nodes) {
      ...
      {
        look = 0;
        while ((look < num_abevs) && (pnd[d]->eqnvar_ids[look] == ev_ids[look])) {
          look++;
        }
        if (look == num_abevs) {
    ...
    if (found_matching_node_type) {
      ...
      if (num_kinds_nodes >= MAX_NODE_KINDS) {
        sr = sprintf(err_msg, "@ node (%d) num_kinds_nodes >= MAX_NODE_KINDS", n + 1);
        EH(-1, err_msg);
      }
    }
    ...
    for (i = 0; i < tnd->num_basic_eqnvars; i++) {
      current_dof += (tnd->eqnvar_wts[i][0] * tnd->eqnvar_wts[i][1] * tnd->eqnvar_wts[i][2]);
    }
