...
void rd_input(char *in_file_name,        /* name of the input file */
  ...
  *p_num_basic_eqnvars = (int *)smalloc(neb * SZ_INT);
  ...
  for (eb = 0; eb < neb; eb++) {
    ...
    if (npe != mono->eb_num_nodes_per_elem[eb_index]) {
      fprintf(stderr, "Inconsistent estimates of nodes per element for ebid %d.\n", ebid);
    ...
    for (i = 0; i < npe; i++) {
      ...
      if (rtn == NULL) {
        fprintf(stderr, "Got squat reading the %d line of %d, EB ID %d\n", i + 1, npe, ebid);
        EH(-1, "Giving up.");
      }
