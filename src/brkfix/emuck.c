 ...
 *
 * $Header: /projects/seataf/CVS/ACCESS/analysis/goma/brkfix/emuck.c,v 1.2 2007-04-01 20:23:32
 * tabaer Exp $
 */
    ...
    int **pntr_element_bnd_stat)   /* boolean, !boundary=internal         (out) */
{
  ...
  *pntr_element_bnd_stat = element_bnd_stat;
  ...
  fprintf(stderr, "Phase I assigned %d/%d elements (%g%%).\n",
          ...
          1e2 * (double)(exo->num_elems - num_unassigned_elems) / (double)exo->num_elems);
...
#ifdef DEBUG
      fprintf(stderr, "Phase III - Assigning element [%d] to set/proc %d\n", e,
              viable_owners[min_dex]);
#endif
  ...
  if (num_unassigned_elems != 0) {
    GOMA_EH(GOMA_ERROR, "Something is wrong. Three phases didn't kill all elems.");
  }

  err = get_max_val_index(element_owner_dist, num_sets, &max_val, &max_dex);
  GOMA_EH(err, "Problem finding maximum value and index.");

  err = get_min_val_index(element_owner_dist, num_sets, &min_val, &min_dex);
  GOMA_EH(err, "Problem finding minimum value and index.");
...
void build_elem_elem_xtra(Exo_DB *exo) /* monolith FE db w/ connectivity     (in) */
{
  int length;
  int e;
  int f;
  ...
  if (!exo->elem_elem_conn_exists) {
    GOMA_EH(GOMA_ERROR, "Cannot build xtra w/o basic info.");
  }
  ...
  for (e = 0; e < exo->num_elems; e++) {
    ...
    for (f = exo->elem_elem_pntr[e], face = 0; f < exo->elem_elem_pntr[e + 1]; f++, face++) {
      ...
      if (neighbor > -1) {
        ...
        GOMA_EH(where, "Didn't find my recipricol element!");
        ...
        if (n1 != n2) {
          GOMA_EH(GOMA_ERROR, "Difft number of nodes on facing elems!?!");
        }

        twist = in_list(snl_this[0], 0, n2, snl_other);

        GOMA_EH(twist, "Low node not found in facing neighbor!");
...
void build_elem_elem_dpi(Exo_DB *monolith, int *elem_owner_list, Exo_DB *piece, Dpi *dpi) {
  int econ_global;
  int elem;
  int elem_global;
  int hi;
  ...
  if (!monolith->elem_elem_conn_exists) {
    GOMA_EH(GOMA_ERROR, "Need monolithic connectivity.");
  }

  if (!piece->elem_elem_conn_exists) {
    GOMA_EH(GOMA_ERROR, "Need decomposed e-e connectivity.");
  }
  ...
  for (elem = 0; elem < ne; elem++) {
    ...
    for (i = 0; i < hi - lo; i++) {
      dpi->elem_elem_list_global[lo + i] = monolith->elem_elem_list[lo_global + i];

      dpi->elem_elem_twst_global[lo + i] = monolith->elem_elem_twst[lo_global + i];

      dpi->elem_elem_face_global[lo + i] = monolith->elem_elem_face[lo_global + i];

      econ_global = monolith->elem_elem_list[lo_global + i];

      dpi->elem_elem_proc_global[lo + i] = (econ_global == -1) ? -1 : elem_owner_list[econ_global];
    }
  }
  ...
  for (i = 0; i < ne; i++) {
    if (dpi->elem_owner[i] < 0) {
      GOMA_EH(GOMA_ERROR, "This proc thinks an element is unclaimed!");
    }
  }

  for (i = 0; i < len; i++) {
    if (dpi->elem_elem_list_global[i] == UNASSIGNED) {
      GOMA_EH(GOMA_ERROR, "Unassigned piece elem_elem_list_global[] !");
    }
  }
