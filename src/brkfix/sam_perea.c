...
void assess_weights(Exo_DB *x,
  ...
  for (eqn_node = 0; eqn_node < nn; eqn_node++) {
    ...
    for (i = 0; i < end->num_basic_eqnvars; i++) {
      ...
      nn_ewts[i] = (end->eqnvar_wts[i][0] * end->eqnvar_wts[i][1] * end->eqnvar_wts[i][2]);
    }
    ...
    for (l = np[eqn_node]; l < np[eqn_node + 1]; l++) {
      ...
      for (m = ep[elem]; m < ep[elem + 1]; m++) {
        ...
        if (num_neighbor_nodes > MAX_NEIGHBOR_NODES) {
          sr = sprintf(err_msg, "@ n = %d too many neighbors.", eqn_node);
          EH(-1, err_msg);
        }
      }
    }
    ...
    for (iv = 0; iv < num_neighbor_nodes; iv++) {
...
#ifdef DEBUG
      fprintf(stderr, "en=(%d),vn=(%d) thru %d elems: ", eqn_node + 1, var_node + 1,
              num_connecting_elems);
      ...
      for (i = 0; i < vnd->num_basic_eqnvars; i++) {
        ...
        nn_vwts[i] = (vnd->eqnvar_wts[i][0] * vnd->eqnvar_wts[i][1] * vnd->eqnvar_wts[i][2]);
      }
