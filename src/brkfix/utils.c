...
int count_node_node_interactions(
    int num_nodes, int *node_ptr, int *elem_list, int *elem_ptr, int *node_list) {
  int elem;
  int eqn_node;
  int i;
  int l;
    ...
    for (l = node_ptr[eqn_node]; l < node_ptr[eqn_node + 1]; l++) {
      ...
      for (m = elem_ptr[elem]; m < elem_ptr[elem + 1]; m++) {
        ...
        if (next_free_spot > MAX_NEIGHBOR_NODES) {
          sr = sprintf(err_msg, "@ node=%d. Increase max neighbors.", eqn_node);
          EH(-1, err_msg);
          EH(sr, err_msg);
        }
      }
...
int findex_mono(int val,    /* what integer value to seek */
...
int gcf(int x, int y) { return ((y == 0) ? (x) : gcf(y, x % y)); }
...
int get_node_index(int global_node,
                   int *node_list,
  ...
  if (n != -1) {
    ...
    n = findex_mono(global_node, node_list + num_internal_nodes, num_boundary_nodes);
    if (n != -1) {
      ...
      n = in_list(global_node, 0, num_external_nodes,
...
int get_internal_boundary_index(int global_node,
                                int *node_list,
                                int num_internal_nodes,
                                int num_boundary_nodes) {
  int n;
...
void proc_sort(int *node_list, int len, int len_assignment, int *assignment) {
...
static int proc_ident(const void *arg1, const void *arg2) {
 ...
 * Revised:
 */
int get_filename_num_procs(const char *basename) {
  ...
  if (mkstemp(fixXXXXXX) == -1) {
    fprintf(stderr, "get_filename_num_procs: temporary file could not be opened\n");
    return val;
  }
...
int get_min_val_index(const int *array,   /* in */
                      const int length,   /* in */
...
int get_max_val_index(const int *array,   /* in */
                      const int length,   /* in */
 ...
 */
int is_shell_type(char *elem_type) {
  if (!strcmp(elem_type, "SHELL4") || !strcmp(elem_type, "shell4")) {
    return 1;
  } else {
    return 0;
  }
