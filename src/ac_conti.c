...
void continue_problem(Comm_Ex *cx, /* array of communications structures */
                      ...
                      Dpi *dpi)    /* distributed processing information */
{
  ...
  if (ProcID == 0) {
    ...
    case 6: /* ANGULAR CONTINUATION TYPE */
      ...
      GOMA_EH(GOMA_ERROR, "Angular continuation is available only in LOCA!");
      break;
    default:
  ...
  if (Linear_Solver == FRONT) {
    ...
    err = mf_setup(&exo->num_elems, &NumUnknowns[pg->imtrx], &max_unk_elem, &three, &one,
                   exo->elem_order_map, fss->el_proc_assign, fss->level, fss->nopdof, fss->ncn,
                   fss->constraint, front_scratch_directory, &fss->ntra);
  ...
  if (strcmp(Matrix_Format, "msr") == 0) {
    ...
    GOMA_EH(err, "Incompatible matrix solver for petsc_complex, solver must be petsc_complex");
  ...
  for (n = 0; n < MaxPathSteps; n++) {
      ...
      case 5: /* UF */
        for (iUC = 0; iUC < nUC; iUC++) {
          ...
          case 2: /* MT */
            ...
            break;
          case 4: /* UM */
            DPRINTF(stdout, "\n\tMTID=%3d MPID=%5d MDID=%3d", cpuc[iUC].MTID, cpuc[iUC].MPID,
                    cpuc[iUC].MDID);
            break;
          default:
    ...
    do {
      ...
      err = solve_nonlinear_problem(ams[JAC], x, delta_t, theta, x_old, x_older, xdot, xdot_old,
                                    ...
                                    tev_post, gv, rd, gindex, p_gsize, gvec, gvec_elem, path1, exo,
                                    dpi, cx, 0, &path_step_reform, is_steady_state, x_AC, x_AC_dot,
                                    path1, resid_vector_sens, x_sens_temp, x_sens_p, NULL);
      ...
      if (converged) {
        ...
        for (i = 0; i < nn_post_fluxes; i++)
          evaluate_flux(exo, dpi, pp_fluxes[i]->ss_id, pp_fluxes[i]->flux_type,
                        ...
                        pp_fluxes[i]->profile_flag, x, xdot, NULL, delta_s, path1, 1);
        ...
        for (i = 0; i < nn_volume; i++) {
          evaluate_volume_integral(exo, dpi, pp_volume[i]->volume_type, pp_volume[i]->volume_name,
                                   ...
                                   pp_volume[i]->num_params, NULL, x, xdot, delta_s, path1, 1);
        }
    ...
    if ((ni == 1) && (n != 0) && (!const_delta_s)) {
      delta_s_new = path_step_control(num_total_nodes, delta_s, delta_s_old, x, eps, &success_ds,
    ...
    if (i_print) {
      error = write_ascii_soln(x, resid_vector, numProcUnknowns, x_AC, nAC, path1, file);
      if (error) {
      ...
      if (Write_Intermediate_Solutions == 0) {
        write_solution(ExoFileOut, resid_vector, x, x_sens_p, x_old, xdot, xdot_old, tev, tev_post,
                       gv, rd, gvec, gvec_elem, &nprint, delta_s, theta, path1, NULL, exo, dpi);
        nprint++;
      }
    }
    if (Output_Variable_Stats) {
  ...
  if (n == MaxPathSteps && aldALC * (lambdaEnd - path) > 0) {
    DPRINTF(
        stdout,
