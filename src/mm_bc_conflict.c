 ...
 *************************************************************************/
{
  ...
  if (sum_unknowns <= 0) {
    fprintf(stderr, "P_%d: Warning: node %d with zero unknowns has an applied bc\n", ProcID, inode);
  }
...
static int find_first_opening(const int *list, const int lengthList)
...
static void delete_bc_entry(int *bc_list, const int j)
...
/*****************************************************************************/
/*****************************************************************************/
static void compress_bc_list(int *bc_list)
...
/*****************************************************************************/
/*****************************************************************************/
static void add_to_node_unk_bc_list(int *list, int ibc) {
  int i;
  for (i = 0; i < MAX_NODAL_BCS; i++) {
    if (list[i] == -1) {
      list[i] = ibc;
...
int search_bc_dup_list(const int bc_item, int *list) {
  int i;
  for (i = 0; list[i] != -1; i++) {
    if (list[i] == bc_item)
      return i;
...
void check_for_bc_conflicts2D(Exo_DB *exo, Dpi *dpi)
 ...
 ***************************************************************************/
{
  ...
  num_total_nodes = (dpi->num_internal_nodes + dpi->num_boundary_nodes + dpi->num_external_nodes);
          ...
          if (exo->ns_id[ins] == BC_Types[ibc].BC_ID) {
            ...
            for (i = 0; i < exo->ns_num_nodes[ins]; i++) {
              ...
              for (p = 0; p < BC_Types[ibc].desc->vector; p++) {
                offset = find_bc_unk_offset(BC_Types + ibc, matIndex, inode, p, &retn_matIndex,
                                            &vd_retn1);
                if (offset >= 0) {
        ...
        for (iss = 0; iss < exo->num_side_sets; iss++) {
          ...
          if (exo->ss_id[iss] == BC_Types[ibc].BC_ID) {
            ...
            for (es = 0; es < exo->ss_num_sides[iss]; es++) {
               ...
               */
              for (k = exo->ss_node_side_index[iss][es]; k < exo->ss_node_side_index[iss][es + 1];
                   k++) {
                /*
                ...
                for (idof = 0; idof < ndofs; idof++) {
                  if (idof >= nv->Num_Var_Desc_Per_Type[R_MOMENTUM1])
                    continue; /*check needed for XFEM */
                  ...
                  if (offset_mom1 < 0)
                    break;
                  idup1 = find_first_opening(BC_Unk_List[inode][offset_mom1], MAX_NODAL_BCS);
                  idup2 = find_first_opening(BC_Unk_List[inode][offset_mom2], MAX_NODAL_BCS);
                  for (i = 0; i < idup1; i++) {
            ...
            if (BC_Types[ibc].BC_Name == TABLE_BC) {
              ...
              case POLYMER_STRESS22_7: {
                ...
                case I_PQ2: {
                  fprintf(stderr,
                          "WARNING: Boundary condition %d, %s, applied on SS %d, may be used as an "
                          "inlet condition \n",
      ...
      if (BC_Unk_List[inode] != NULL) {
        ...
        for (offset = 0; offset < nv->Num_Unknowns; offset++) {
          ...
          if (idup > 1) {
            ...
            if (Debug_Flag) {
              idup = 0;
              fprintf(stderr, "  DUPLICATE BC at node %d equation %d, material %d: ", inode + 1,
                      eqn, vd->MatID);
            ...
            if (eqn == R_MOMENTUM1) {
               ...
               */
              offset_mom2 = get_nodal_unknown_offset(nv, R_MOMENTUM2, vd->MatID, 0, &vd_retn2);
              idup2 = find_first_opening(BC_Unk_List[inode][offset_mom2], MAX_NODAL_BCS);
              ...
              /* different side sets  */
              offset_mesh1 = get_nodal_unknown_offset(nv, R_MESH1, vd->MatID, 0, &vd_retn1);
              if (offset_mesh1 < 0) {
                idup1 = 0;
              } else {
                idup1 = find_first_opening(BC_Unk_List[inode][offset_mesh1], MAX_NODAL_BCS);
              }
              if (offset_mom2 < 0) {
                idup2 = 0;
              } else {
                  ...
                  for (k = 0; k < idup; k++) {
                    ...
                    if (ibc2 != -1 && ibc1 != ibc2) {
                        ...
                        if (max > 0. && sum / max > 1e-8) {
                          if (Debug_Flag > 0) {
                            fprintf(stderr,
                                    ...
                                    BC_Types[ibc1].desc->name2, inode,
                                    BC_Types[ibc2].BC_Data_Float[0]);
                          }
                          i_delete = 2; /* delete 2nd occurence */
                        } else {
                          if (Debug_Flag > 0) {
                            fprintf(stderr, "    IDENTICAL BC %s at node %d equation %d\n",
                                    BC_Types[ibc1].desc->name2, inode + 1, eqn);
                          }
                        }
                      ...
                      if (BC_Types[ibc2].desc->rotate != NO_ROT ||
                        ...
                        if (BC_Types[ibc2].desc->rotate == R_MESH1) {
                          ...
                          if (p + 1 < pd_glob[0]->Num_Dim) {
                            offset_mesh2 =
                                get_nodal_unknown_offset(nv, eqn + 1, vd->MatID, 0, &vd_retn2);
                            idum =
                                find_first_opening(BC_Unk_List[inode][offset_mesh2], MAX_NODAL_BCS);
                            if (Debug_Flag > 0) {
                          ...
                          if (p + 1 < pd_glob[0]->Num_Dim) {
                            offset_mom2 =
                                get_nodal_unknown_offset(nv, eqn + 1, vd->MatID, 0, &vd_retn2);
                            idum =
                                find_first_opening(BC_Unk_List[inode][offset_mom2], MAX_NODAL_BCS);
                      ...
                      if (i_delete == 1) {
                        if (Debug_Flag > 0) {
                          fprintf(
                              stderr, "    DISCARDING %s at node %d from dup_list because of %s\n",
                              BC_Types[ibc2].desc->name2, inode + 1, BC_Types[ibc1].desc->name2);
                        }
                      ...
                      if (i_delete == 2) {
                        if (Debug_Flag > 0) {
                          fprintf(
                              stderr, "    DISCARDING %s at node %d from dup_list because of %s\n",
                              BC_Types[ibc1].desc->name2, inode + 1, BC_Types[ibc2].desc->name2);
                        }
                ...
                if (BC_Types[ibc1].desc->rotate == NO_ROT &&
                  ...
                  for (k = 0; k < idup; k++) {
                    ...
                    if ((ibc2 != ibc1) && (ibc2 != -1)) {
                       ...
                       */
                      if (bct1 == FLUID_SOLID_BC || bct1 == SOLID_FLUID_BC ||
                       ...
                       *
                       */
                      if (action == 1 && BC_Types[ibc2].desc->rotate != NO_ROT) {
                        if (BC_Types[ibc2].desc->rotate == R_MESH1) {
                          p = eqn - R_MESH1;
                          if (p + 1 < pd_glob[0]->Num_Dim) {
                            offset_mesh2 =
                            ...
                            if (Debug_Flag > 0) {
                              fprintf(stderr,
                                      "    REMOVING BC %s at node %d because of %s and next coord "
                                      "not available\n",
                                      BC_Types[ibc2].desc->name2, inode + 1,
                                      BC_Types[ibc1].desc->name2);
                            }
                  ...
                  for (k = 0; k < idup; k++) {
                    ...
                    if ((ibc2 != ibc1) && (ibc2 != -1) && (ibc1 != -1)) {
                      ...
                      if (BC_Types[ibc2].desc->method == WEAK_INT_SURF ||
                         ...
                         */
                        if (bct2 == CA_BC || bct2 == CA_MOMENTUM_BC || bct2 == MOVING_CA_BC ||
                            bct2 == VELO_THETA_TPL_BC || bct2 == CA_OR_FIX_BC ||
                           ...
                           */
                          sprintf(Err_Msg, "Contact angle at node %d from %s and %s?", inode + 1,
                                  BC_Types[ibc1].desc->name1, BC_Types[ibc2].desc->name1);
                          GOMA_WH(-1, Err_Msg);
                        } else {
                          if (Debug_Flag > 0) {
                            fprintf(stderr,
                                    "    REMOVING SPECIAL BC %s at node %d from dup_list because "
                                    "of %s\n",
                                    BC_Types[ibc2].desc->name2, inode + 1,
                                    BC_Types[ibc1].desc->name2);
                          }
                        ...
                        if (bct1 == bct2 && action == 1) { /* these conditions are the same type */
                          ...
                          case GD_POLYN_BC:
                            if ((BC_Types[ibc1].BC_Data_Int[2] == BC_Types[ibc2].BC_Data_Int[2]) &&
                              ...
                              det = fabs(a1 - b1) + fabs(a2 - b2) + fabs(a3 - b3) + fabs(a4 - b4) +
                                    fabs(a5 - b5) + fabs(a6 - b6) + fabs(a7 - b7);
                            }
                            break;
                          case VNORM_LEAK_BC:
                            a1 = BC_Types[ibc1].BC_Data_Float[0];
                        ...
                        if (i_delete == 0 && action == 1) {
                          ...
                          if (BC_Types[imove].desc->rotate == R_MESH1) {
                            ...
                            if (p + 1 < pd_glob[0]->Num_Dim) {
                              ...
                              if (Debug_Flag > 0) {
                                fprintf(stderr, "    MOVING BC %s at node %d because of %s\n",
                                        BC_Types[imove].desc->name2, inode + 1,
                                        BC_Types[istay].desc->name2);
                              }
    ...
    */
    ...
    if (Debug_Flag > 0 || pd_glob[0]->Num_Dim < 3) {
      ...
      for (i = 0; i < BC_dup_ptr[pg->imtrx]; i++) {
        ...
        if (inode < (dpi->num_internal_nodes + dpi->num_boundary_nodes)) {
          node = Nodes[inode];
          nv = node->Nodal_Vars_Info[pg->imtrx];
          fprintf(bc_dup_out, "\nAt global node %d", node->Global_Node_Num + 1);
          ...
          for (offset = 0; offset < nv->Num_Unknowns; offset++) {
            ...
            if ((bickel = BC_Unk_List[inode][offset][0]) != -1) {
              if (eqn != R_MASS) {
                ...
                fprintf(bc_dup_out, " %-15s", EQ_Name[MAX_VARIABLE_TYPES + vd->Subvar_Index].name1);
                if (vd->MatID != -1) {
                  fprintf(bc_dup_out, " (MatID=%d)", vd->MatID);
                }
                fprintf(bc_dup_out, " gets ");
              ...
              while ((bickel = BC_Unk_List[inode][offset][idup]) != -1) {
                if (idup == 0) {
                  fprintf(bc_dup_out, "  %-15s (%d) from %s %d\n", BC_Types[bickel].desc->name1,
                          bickel, BC_Types[bickel].Set_Type, BC_Types[bickel].BC_ID);
                } else {
                  fprintf(bc_dup_out, "\t\t\t%-15s (%d) from %s %d\n", BC_Types[bickel].desc->name1,
                          bickel, BC_Types[bickel].Set_Type, BC_Types[bickel].BC_ID);
                }
    ...
    for (inode = 0; inode < num_total_nodes; inode++) {
      ...
      if (BC_Unk_List[inode] != NULL) {
        for (ivar = 0; ivar < 2; ivar++) {
          ...
          for (idof = 0; idof < ndofs; idof++) {
            if (idof >= nv->Num_Var_Desc_Per_Type[eqn])
              continue; /*check needed for XFEM*/
            ...
            if (Debug_Flag > 1) {
              fprintf(stderr,
                      "%d BC's at node %d on %d: DC=%d "
                      "WIC=%d SIC=%d PC=%d SP=%d ROT=%d\n",
                      count_BC, inode + 1, ivar, count_DC, count_weak, count_strong, count_coll,
                      count_special, count_rotate);
            }
            /* evaluate if node needs rotation */
            i_rotate = 0;
            if (count_rotate > 0 && count_rotate < pd_glob[0]->Num_Dim) {
              i_rotate = 1;
            }
            ...
            if (count_DC > 0) {
              ...
              if (count_rotate > 0) {
                if (ivar == 0) {
                  ...
                  log_msg(
                      "Don't rotate MOMENTUM at node %d because there is a Dirichlet condition\n",
                      inode + 1);
                }
              }
            }
            ...
            if (ivar == 1 && count_rotate > 1 && two_ss_rot == 1) {
              ...
              log_msg("WARNING: applying 2 rotated momentum conditions on MOMENTUM at "
                      "junction node %d may be dangerous",
                      inode + 1);
            }
          }
    ...
    if (mesh_rotate_node[pg->imtrx] != NULL || mom_rotate_node[pg->imtrx] != NULL) {
      ...
      while (!Use_2D_Rotation_Vectors && ebi < exo->num_elem_blocks) {
        Use_2D_Rotation_Vectors =
            (exo->eb_elem_itype[ebi] == LINEAR_TRI || exo->eb_elem_itype[ebi] == QUAD_TRI);
    ...
    if (Debug_Flag > 0) {
      for (i = 0; i < num_mesh_rotate[pg->imtrx]; i++) {
        fprintf(stderr, "\tMESH node %d rotate on SS %d\n", mesh_rotate_node[pg->imtrx][i] + 1,
                mesh_rotate_ss[pg->imtrx][i]);
      }
    }
    ...
    if (Debug_Flag > 0) {
      for (i = 0; i < num_mom_rotate[pg->imtrx]; i++) {
        fprintf(stderr, "\tMOMENTUM node %d rotate on SS %d\n", mom_rotate_node[pg->imtrx][i] + 1,
                mom_rotate_ss[pg->imtrx][i]);
      }
    }
  ...
  if (ProcID == 0) {
    for (int ibc = 0; ibc < Num_BC; ibc++) {
      if (matrix_used_BC[ibc] == 0) {
        GOMA_WH_MANY(-1, "Boundary condition %d, %s, applied on NS %d, is never used\n", ibc,
                     BC_Types[ibc].desc->name1, BC_Types[ibc].BC_ID);
      }
    }
    free(matrix_used_BC);
  }
...
void check_for_bc_conflicts3D(Exo_DB *exo, Dpi *dpi)
 ...
 *************************************************************************/
{
  ...
  if (Unlimited_Output) {
    ...
    if (ofbc == NULL) {
      sprintf(Err_Msg, "P_%d: cannot open file \"%s\" to write 3D BC info", ProcID, ofbc_fn);
  ...
  for (pg->imtrx = 0; pg->imtrx < upd->Total_Num_Matrices; pg->imtrx++) {
        ...
        for (ins = 0; ins < exo->num_node_sets; ins++) {
          ...
          if (exo->ns_id[ins] == BC_Types[ibc].BC_ID) {
            ...
            for (i = 0; i < exo->ns_num_nodes[ins]; i++) {
              ...
              for (p = 0; p < BC_Types[ibc].desc->vector; p++) {
                offset =
                    find_bc_unk_offset(BC_Types + ibc, matIndex, inode, p, &retn_matIndex, &vd);
                if (offset >= 0) {
                  add_to_node_unk_bc_list(BC_Unk_List[inode][offset], ibc);
                }
              }
      ...
      for (side_index = 0; side_index < exo->ss_num_sides[iss]; side_index++) {
        ...
        for (lni = exo->ss_node_side_index[iss][side_index];
          ...
          if (in_list(exo->ss_id[iss], 0, MAX_SS_PER_NODE, SS_list[inode]) == -1) {
            iptr = 0;
            while (SS_list[inode][iptr] != -1)
              iptr++;
            if (iptr >= MAX_SS_PER_NODE) {
              sprintf(Err_Msg, "Node (%d) wants > %d sidesets - boost MAX_SS_PER_NODE.", inode + 1,
                      MAX_SS_PER_NODE);
              GOMA_EH(GOMA_ERROR, Err_Msg);
            }
          ...
          if (exo->ss_id[iss] == BC_Types[ibc].BC_ID) {
            ...
            for (side_index = 0; side_index < exo->ss_num_sides[iss]; side_index++) {
              ...
              for (lni = exo->ss_node_side_index[iss][side_index];
                ...
                /* Edge conditions */
                if (edgeBC &&
                    (in_list(BC_Types[ibc].BC_ID2, 0, MAX_SS_PER_NODE, SS_list[inode]) != -1)) {
                  node_ok = 1;
                }
                ...
                if (vertexBC &&
                    (in_list(BC_Types[ibc].BC_ID2, 0, MAX_SS_PER_NODE, SS_list[inode]) != -1) &&
                    (in_list(BC_Types[ibc].BC_ID3, 0, MAX_SS_PER_NODE, SS_list[inode]) != -1)) {
                  node_ok = 1;
                }
    ...
    for (irc = 0; irc < Num_ROT; irc++) {
      ...
      if (-1 == in_list(ROT_Types[irc].ss_id[0], 0, len_ssid_list, ssid_list)) {
        sprintf(Err_Msg, "ROT[%d] (ROT = %s %s %d ...) has ss_id_1 = %d not in mesh.", irc,
                rot_eq_type2string(ROT_Types[irc].eq_type),
                rotopology_type2string(ROT_Types[irc].type), ROT_Types[irc].ss_id[0],
      ...
      for (inode = 0; inode < num_total_nodes; inode++) {
        ...
        if (ROT_Types[irc].type == FACE &&
            (in_list(ROT_Types[irc].ss_id[0], 0, MAX_SS_PER_NODE, SS_list[inode]) != -1)) {
          node_ok = 1;
        }
        if (ROT_Types[irc].type == CURVE &&
            (in_list(ROT_Types[irc].ss_id[0], 0, MAX_SS_PER_NODE, SS_list[inode]) != -1) &&
            (in_list(ROT_Types[irc].ss_id[1], 0, MAX_SS_PER_NODE, SS_list[inode]) != -1)) {
          node_ok = 1;
        }
        if (ROT_Types[irc].type == VERTEX &&
            (in_list(ROT_Types[irc].ss_id[0], 0, MAX_SS_PER_NODE, SS_list[inode]) != -1) &&
            (in_list(ROT_Types[irc].ss_id[1], 0, MAX_SS_PER_NODE, SS_list[inode]) != -1) &&
            (in_list(ROT_Types[irc].ss_id[2], 0, MAX_SS_PER_NODE, SS_list[inode]) != -1)) {
        ...
        if (node_ok) {
          ...
          if (ROT_Types[irc].eq_type == R_MESH1)
            eq = VECT_EQ_MESH;
          else if (ROT_Types[irc].eq_type == R_MOMENTUM1)
            eq = VECT_EQ_MOM;
          else
            GOMA_EH(GOMA_ERROR, "Illegal vector equation");
      ...
      if (BC_Unk_List[inode] != NULL) {
        ...
        for (offset = 0; offset < nv->Num_Unknowns; offset++) {
          ...
          if (ROT_list[inode] == NULL) {
            ...
            standard_BC = 1;
          } else if ((eqn == R_MOMENTUM1 || eqn == R_MOMENTUM2 || eqn == R_MOMENTUM3) &&
            ...
            if (eqn == R_MESH1 || eqn == R_MOMENTUM1) {
              ...
              for (p = 0; p < dim; p++) {
                ndup1[p] = 0;
                offset_mom2 = get_nodal_unknown_offset(nv, eqn + p, vd->MatID, 0, &vd_retn2);
              ...
              for (p = 0; p < dim; p++) {
                ...
                if (bct < 0) {
                  ...
                  for (q = 0; q < dim; q++) {
                    for (j = 0; j < ndup1[q]; j++) {
                      offset_mom2 = get_nodal_unknown_offset(nv, eqn + q, vd->MatID, 0, &vd_retn2);
                      ...
                      if (bct == bct1 && bcSS == bcSS1 && save_this_bc1[q][j] == 0) {
                        /*
                        ...
                        bc_found = 1;
#endif
                        ROT_Types[irc].BC_desc[p] = BC_Types[ibc1].desc;
                        ...
                        offset_mom2 =
                            get_nodal_unknown_offset(nv, eqn + p, vd->MatID, 0, &vd_retn2);
                        BC_Unk_List[inode][offset_mom2][ndup1[p]] = ibc1;
                        ndup1[p]++;
                        /* need to deal with GD conditions
                         * here!! */

                        if (BC_Types[ibc1].desc->method == DIRICHLET && p != q) {
                ...
                if (bc_found != 1) {
                  fprintf(ofbc, "Cannot find BC to match ROT [%d], coord %d at node %d\n", irc, p,
                          inode);
                  fprintf(stderr, "Problem on ROT [%d], coord [%d], node (%d)\n", irc, p,
                          inode + 1);
                  if (tilt_bct && tilt_bcSS && tilt_save) {
                    fprintf(stderr, "P_%d: bct = %d, bct1 = %d\n", ProcID, bct, bct1);
                    fprintf(stderr, "P_%d: bcSS = %d, bcSS1 = %d\n", ProcID, bcSS, bcSS1);
                    fprintf(stderr, "P_%d: save_this_bc1[q][j]=%d\n", ProcID, val);
                  }
              ...
              for (p = 0; p < dim; p++) {
                ...
                for (j = 0; j < ndup1[p]; j++) {
                  if (save_this_bc1[p][j]) {
                    BC_Unk_List[inode][offset_mom2][k] = BC_Unk_List[inode][offset_mom2][j];
                    k++;
                  }
                }
                for (j = k; j < ndup1[p]; j++) {
            ...
            if (ndup > 1) {
              ...
              if (j_DC == -1) {
                for (j = 0; j < ndup; j++) {
                  ...
                  bct1 = BC_Types[ibc1].BC_Name;
                  if ((BC_Types[ibc1].desc->method == COLLOCATE_SURF) &&
                      (bct1 != FLUID_SOLID_BC && bct1 != SOLID_FLUID_BC &&
                       bct1 != SOLID_FLUID_RS_BC)) {
                    if (j_PC != -1) {
      ...
      for (i = 0; i < BC_dup_ptr[pg->imtrx]; i++) {
        ...
        for (offset = 0; offset < nv->Num_Unknowns; offset++) {
          ...
          if (BC_Unk_List[inode][offset][0] != -1) {
            ...
            while (BC_Unk_List[inode][offset][idup] != -1) {
              if (Unlimited_Output) {
                fprintf(ofbc, "%s(%d", BC_Types[BC_Unk_List[inode][offset][idup]].desc->name1,
              ...
              if (BC_Types[BC_Unk_List[inode][offset][idup]].BC_ID2 == -1) {
                ...
                if (Unlimited_Output) {
                  fprintf(ofbc, ",%d)", BC_Types[BC_Unk_List[inode][offset][idup]].BC_ID2);
                }
              }
              idup++;
            }
