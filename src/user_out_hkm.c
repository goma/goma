...
static void PRNT_SOLN_VALS(char *soln_name,
                           double l_max,
                           double g_max,
                           int i_max,
                           double l_min,
...
static void SOLN_VALUES(
    int, int, double[], double *, double *, int *, double *, double *, int *, double *, const int);

static void SOLN_TO_MOLES(double[], double[], double[], double[], MATRL_PROP_STRUCT *);
static void GET_SOLN_LASTSPECIES(double[], double[], MATRL_PROP_STRUCT *);

#ifdef USE_CHEMKIN
#ifdef SENKIN_OUTPUT
static void WRITE_SENKIN_FILE(int, int, int, double[], double, int, MATRL_PROP_STRUCT *matID_prop);
#endif
#endif
 ...
 **************************************************************************/
{
  ...
  for (mn = 0; mn < upd->Num_Mat; mn++) {
    ...
    var = VELOCITY1;
    if (pd_local->v[0][var]) {
      SOLN_VALUES(var, 0, soln, &Tmax, &g_Tmax, &iTmax, &Tmin, &g_Tmin, &iTmin, &Tavg, mn);

      PRNT_SOLN_VALS("X Velocity", Tmax, g_Tmax, iTmax, Tmin, g_Tmin, iTmin, Tavg);
    }
    var = VELOCITY2;
    if (pd_local->v[0][var]) {
      SOLN_VALUES(var, 0, soln, &Tmax, &g_Tmax, &iTmax, &Tmin, &g_Tmin, &iTmin, &Tavg, mn);

      PRNT_SOLN_VALS("Y Velocity", Tmax, g_Tmax, iTmax, Tmin, g_Tmin, iTmin, Tavg);
    }
    var = VELOCITY3;
    if (pd_local->v[0][var]) {
      SOLN_VALUES(var, 0, soln, &Tmax, &g_Tmax, &iTmax, &Tmin, &g_Tmin, &iTmin, &Tavg, mn);

      PRNT_SOLN_VALS("Z Velocity", Tmax, g_Tmax, iTmax, Tmin, g_Tmin, iTmin, Tavg);
    }
    var = PRESSURE;
    if (pd_local->v[0][var]) {
      SOLN_VALUES(var, 0, soln, &Tmax, &g_Tmax, &iTmax, &Tmin, &g_Tmin, &iTmin, &Tavg, mn);

      PRNT_SOLN_VALS("Pressure", Tmax, g_Tmax, iTmax, Tmin, g_Tmin, iTmin, Tavg);
    }
    ...
    if (pd_local->v[0][var]) {
      ...
      PRNT_SOLN_VALS("Temperature", Tmax, g_Tmax, iTmax, Tmin, g_Tmin, iTmin, Tavg);
    }
    ...
    if (pd_local->v[0][var]) {
      ...
      if (ProcID == 0) {
        if (matID_prop->DefaultDatabase == DB_CHEMKIN_MAT) {
          WRITE_SENKIN_FILE(status, 1, SenkinNode, soln, time, time_step_num, matID_prop);
        }
      }
#endif
#endif
      ...
      for (k = 0; k < matID_prop->Num_Species_Eqn; k++) {
        SOLN_VALUES(MASS_FRACTION, k, soln, &Tmax, &g_Tmax, &iTmax, &Tmin, &g_Tmin, &iTmin, &Tavg,
                    mn);
        (void)sprintf(char_str, "Mass Fraction for Sp# %2d (%.16s)", k + 1,
                      matID_prop->Species_Names[k]);
        PRNT_SOLN_VALS(char_str, Tmax, g_Tmax, iTmax, Tmin, g_Tmin, iTmin, Tavg);
      }
      ...
      if (matID_prop->Num_Species_Eqn < matID_prop->Num_Species) {
        ...
        SOLN_VALUES(MASS_FRACTION, 0, soln_lastSpecies, &Tmax, &g_Tmax, &iTmax, &Tmin, &g_Tmin,
                    &iTmin, &Tavg, mn);
        sprintf(char_str, "Mass Fraction for Sp# %2d (%.16s)", k + 1, matID_prop->Species_Names[k]);
        PRNT_SOLN_VALS(char_str, Tmax, g_Tmax, iTmax, Tmin, g_Tmin, iTmin, Tavg);
      }
      /*
      ...
      if (pd_local->Species_Var_Type == SPECIES_MASS_FRACTION) {
        ...
        SOLN_TO_MOLES(soln, soln_mole, soln_lastSpecies, soln_mole_lastSpecies, matID_prop);
        for (k = 0; k < matID_prop->Num_Species_Eqn; k++) {
          SOLN_VALUES(MASS_FRACTION, k, soln_mole, &Tmax, &g_Tmax, &iTmax, &Tmin, &g_Tmin, &iTmin,
                      &Tavg, mn);
        ...
        if (matID_prop->Num_Species_Eqn != matID_prop->Num_Species) {
          SOLN_VALUES(MASS_FRACTION, 0, soln_mole_lastSpecies, &Tmax, &g_Tmax, &iTmax, &Tmin,
...
static void SOLN_VALUES(int var_type,
                        int sub_index,
                        double soln[],
                        double *l_max,
                        double *g_max,
 ...
 ************************************************************************/
{
  ...
  int num_owned_nodes = DPI_ptr->num_internal_nodes + DPI_ptr->num_boundary_nodes;
        ...
        for (n = 0; n < num_local_nodes; n++) {
          ...
          if (Nodes[i]->Type.Owned) {
            if (!i_been_there[i]) {
               ...
               */
              ndof = 0;
              if ((i_eqn = Index_Solution(i, var_type, sub_index, ndof, mn, pg->imtrx)) >= 0) {
                num_nodes_mn++;
                s = soln[i_eqn];
                s_sum += fabs(s);
  ...
  *l_min = s_min;
  ...
  if (Num_Proc > 1) {
     ...
     */
    MPI_Allreduce(&num_nodes_mn, &g_solncomp_nodes, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&s_sum, &g_sum, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
#endif
  } else {
    *g_max = s_max;
    *g_min = s_min;
...
static void SOLN_TO_MOLES(double soln[],
                          double soln_mole[],
 ...
 ************************************************************************/
{
  ...
  for (node = 0; node < num_owned_nodes; node++) {
    if ((i_eqn = Index_Solution(node, MASS_FRACTION, 0, ndof, mn, pg->imtrx)) >= 0) {
      ...
      for (k = 0; k < matID_prop->Num_Species_Eqn; k++) {
        soln_mole[i_eqn + k] = soln[i_eqn + k] / (sumwt * matID_prop->molecular_weight[k]);
      }
      if (matID_prop->Num_Species > matID_prop->Num_Species_Eqn) {
        k = matID_prop->Num_Species_Eqn;
        soln_mole_lastSpecies[i_eqn] =
            soln_lastSpecies[i_eqn] / (sumwt * matID_prop->molecular_weight[k]);
      }
    }
  }
...
static void
GET_SOLN_LASTSPECIES(double soln[], double soln_lastSpecies[], MATRL_PROP_STRUCT *matID_prop)
 ...
 ***********************************************************************/
{
  ...
  extern FSUB_TYPE write_header_(int *, int *, int *, int *, int *, int *, int *, int *, double *,
                                 double *, double *, int *, double *, CK_NAME *, int);
  extern FSUB_TYPE write_record_(int *, int *, double *, double *, int *, int *, int *, int *,
  ...
  if (status == 0) {
    ...
    (void)write_header_(&LOUT, &LUNIT, &LSENS, &NSYS, &KK, &II, &ICASE, &NEQ, &TSTOP, &TLIM, TOLS,
                        Ck.Ickwrk_p, Ck.Rckwrk_p, Ck.Cckwrk_p, 16);
  }
  ...
  if (pd_glob[0]->v[pg->imtrx][var]) {
    if ((indx_T = Index_Solution(mesh_id, node, TEMPERATURE, 0, mn, pg->imtrx)) >= 0) {
      T = soln[indx_T];
    }
  } else {
  ...
  (void)write_record_(&LUNIT, &ICASE, &time, MF_vector, &NSYS, &LSENS, &II, Ck.Ickwrk_p,
                      Ck.Rckwrk_p, &T, &pressureAmbient, &VOL);
