...
/**************************************************************************/
/**************************************************************************/
double raoults_law_prxn(JACOBIAN_VAR_DESC_STRUCT *func_jac,
 ...
 **********************************************************************/
{
  ...
  if (!is[intf_id].Processed[wspec]) {
    ...
    if (speciesVT != is[intf_id].SpeciesVT) {
      ...
      is_change1_speciesVT(is, wspec, mp_gas->Num_Species, mp_liq, speciesVT, time, intf_id);
      is_change1_speciesVT(is, mp_gas->Num_Species + wspec, 0, mp_gas, speciesVT, time, intf_id);
      is_change1_speciesVT(is, mp_gas->Num_Species + wspec, mp_gas->Num_Species, mp_liq, speciesVT,
                           time, intf_id);
      convert_species_var(speciesVT, mp_gas, is[intf_id].SpeciesVT, is[intf_id].Var_Value, time);
      ...
      is_change1_lastspecies(is, mp_gas->Num_Species + wspec, 0, mp_gas, intf_id);
      is_change1_lastspecies(is, mp_gas->Num_Species + wspec, mp_gas->Num_Species, mp_liq, intf_id);
    }
  }
  ...
  if (af->Assemble_Jacobian) {
    ...
    for (index_is = 0; index_is < is[intf_id].Num_Terms; index_is++) {
       ...
       */
      index_lvdesc = ei[pg->imtrx]->VDindex_to_Lvdesc[vd->List_Index];
      if (index_lvdesc >= 0 && DOUBLE_NONZERO(is[intf_id].JacMatrix[pos][index_is])) {
        /*
...
void source_vle_prxn(INTERFACE_SOURCE_STRUCT *is,
                     BOUNDARY_CONDITION_STRUCT *bc,
                     MATRL_PROP_STRUCT *mp_gas,
                     MATRL_PROP_STRUCT *mp_liq,
                     int have_T,
                     const int intf_id)
 ...
 ***********************************************************************/
{
  ...
  if (is[intf_id].SpeciesVT != SPECIES_CONCENTRATION) {
    GOMA_EH(GOMA_ERROR, "unimplemented complication");
  }
  C_l_mix = 0.0;
  for (k = 0, pos = mp_gas->Num_Species; k < mp_liq->Num_Species; k++, pos++) {
    C_l_mix += vv[pos];
  }
  ...
  if (is[intf_id].Do_Jac) {
    ...
    if (have_T) {
      Jac[wspec][index_T] =
          (k_f * vv[wspec_liq] * (dpsatdT_w / (RGAS_CONST * T * C_l_mix) - K_equil / T));
    }
...
double is_equil_prxn(JACOBIAN_VAR_DESC_STRUCT *func_jac,
                     BOUNDARY_CONDITION_STRUCT *bc,
                     int ip,
                     ELEM_SIDE_BC_STRUCT *elem_side_bc,
                     double x_dot[MAX_PDIM],
 ...
 **********************************************************************/
{
  ...
  if (!is[intf_id].Processed[wspec_a]) {
    ...
    if (speciesVT != is[intf_id].SpeciesVT) {
      ...
      is_change1_speciesVT(is, wspec_a, mp_a->Num_Species, mp_b, speciesVT, time, intf_id);
      is_change1_speciesVT(is, wspec_b, 0, mp_a, speciesVT, time, intf_id);
      is_change1_speciesVT(is, wspec_b, mp_a->Num_Species, mp_b, speciesVT, time, intf_id);
      /*
  ...
  if (af->Assemble_Jacobian) {
    ...
    for (index_is = 0; index_is < is[intf_id].Num_Terms; index_is++) {
      ...
      if (index_lvdesc >= 0 && DOUBLE_NONZERO(is[intf_id].JacMatrix[pos][index_is])) {
         ...
         *  Jacobian_Var_Desc structure
         */
        jacobianVD_addEntry(func_jac, index_lvdesc, is[intf_id].JacMatrix[pos][index_is]);
      }
    }
  }
  return func_value;
...
void source_is_equil_prxn(INTERFACE_SOURCE_STRUCT *is,
                          BOUNDARY_CONDITION_STRUCT *bc,
                          MATRL_PROP_STRUCT *mp_a,
                          MATRL_PROP_STRUCT *mp_b,
                          int have_T,
                          const int intf_id)
 ...
 ***********************************************************************/
{
  ...
  C_b_mix = 0.0;
  for (k = 0, pos = mp_a->Num_Species; k < mp_b->Num_Species; k++, pos++) {
    C_b_mix += vv[pos];
  }
  ...
  if (is[intf_id].Do_Jac) {
    ...
    for (k = mp_a->Num_Species; k < mp_a->Num_Species + mp_b->Num_Species; k++) {
      Jac[wspec_b][k] += tmp / C_b_mix;
    }
    if (have_T) {
      Jac[wspec_b][index_T] = tmp / Kc_equil * deltaG / (RGAS_CALS * T * T);
    }
    for (k = 0; k < is[intf_id].Num_Terms; k++) {
...
is_masstemp_create(MATRL_PROP_STRUCT *mp_1, MATRL_PROP_STRUCT *mp_2, int do_Jac)
 ...
 **********************************************************************/
{
  ...
  for (i = 0; i < Num_Interface_Srcs; i++) {
    ...
    for (k = 0, pos = mp_1->Num_Species; k < mp_2->Num_Species; k++, pos++) {
...
void is_masstemp_fillin(INTERFACE_SOURCE_STRUCT *is,
                        MATRL_PROP_STRUCT *mp_1,
                        MATRL_PROP_STRUCT *mp_2,
                        int speciesVT_desired,
                        const double time,
                        const int intf_id)
/**********************************************************************
 *
 ...
 **********************************************************************/
{
  ...
  if (speciesVT_desired != mp_1->StateVector_speciesVT) {
    (void)convert_species_var(speciesVT_desired, mp_1, mp_1->StateVector_speciesVT,
  ...
  if (speciesVT_desired != mp_2->StateVector_speciesVT) {
    (void)convert_species_var(speciesVT_desired, mp_2, mp_2->StateVector_speciesVT,
