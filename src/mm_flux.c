  ...
  if ( current_id != -1 )
    {
      ...
      for (i=0,local_flux = local_flux_conv = local_area = 0.; 
	   ...
	   i++)
	{         
	  ei[pg->imtrx]->ielem = elem_list[i];

	  mn = find_mat_number(ei[pg->imtrx]->ielem, exo);
	...
	if ( blk_id == -1 || 
				...
				quantity == DELTA || quantity == LS_DCA ))
		  {
			...
			if(species_id > 0)
			{
			  ...
			  ierr = adaptive_weight( ad_wt, ip_total, ielem_dim-1, ls_F, 0.0,
									  wt_type, ei[pg->imtrx]->ielem_type);
	  ...
	  /* Surface integration over element */
	      ...
	      for (ip = 0; ip < ip_total; ip++)
		  {
			  ...
			  surface_determinant_and_normal (ei[pg->imtrx]->ielem, 
...
/**   get solid stresses  **/
		 ...
		 */
		      ...
		      err = mesh_stress_tensor(TT, dTT_dx, dTT_dp, dTT_dc, 
					       ...
					       delta_t, ei[pg->imtrx]->ielem, ip, ip_total);
		  ...
		  /* but first evaluate properties if variable */
		    ...
		    case VOLUME_FLUX:
			  ...
			  if (cr->MeshMotion == ARBITRARY)
			    ...
			    GOMA_EH(GOMA_ERROR,"Inconsistency in volume-flux specification. Contact Developers");
                        }
                          local_flux += weight*det* local_q ;
		      break;
		    ...
		    case TORQUE:
		      if(pd->CoordinateSystem == PROJECTED_CARTESIAN ||
		         pd->CoordinateSystem == CARTESIAN_2pt5D)
			GOMA_EH(GOMA_ERROR, "TORQUE has not been updated for the PROJECTED_CARTESIAN coordinate system.");
			...
			{
			  GOMA_EH(GOMA_ERROR,"unrecognized porous media type in mm_flux.c");
			}
		      break;
...
/*  initialize variables */
      ...
      dist = table_distance_search(BC_Types[bc_table_id].table, coord, 
               ...
               i1 = Index_Solution (k, MESH_DISPLACEMENT1, 0, 0, -1, pg->imtrx);
...
/*  repulsion function  */
			   ...
			   for( b=0 ; b<pd->Num_Dim ; b++)
                            ...
                            {GOMA_EH(GOMA_ERROR,"Repulsive force not found\n");} 
                         }
			...
			/* For scalar e-field calculations, we will assume the e-vector 
		      ...
		      if(pd->CoordinateSystem == PROJECTED_CARTESIAN ||
		         pd->CoordinateSystem == CARTESIAN_2pt5D)
			GOMA_EH(GOMA_ERROR, "POYNTING has not been updated for the PROJECTED_CARTESIAN coordinate system.");

		      if(pd->CoordinateSystem == SWIRLING || 
			 pd->CoordinateSystem == CYLINDRICAL)
			{
			GOMA_EH(GOMA_ERROR, "POYNTING has not been checked for CYLINDRICAL yet.");
			  for ( a=0; a<VIM; a++)
			    {
			      for ( b=0; b<VIM; b++)
				{
	  ...
	  */
			...
			if (TimeIntegration != STEADY && 
					pd->e[pg->imtrx][MESH_DISPLACEMENT1]) {
				for (j = 0; j < VIM; j++) {
					x_dot[j] = fv_dot->x[j];
				  	} 
				} else {
	...
	if ( J_AC != NULL)
	 ...
	 /* FORCE_X_NEG, FORCE_Y_NEG, FORCE_Z_NEG */
			...
			case FORCE_Z_NEG:
			  ...
			  GOMA_EH(dir, "PANIC in evaluate_flux sensitivity section");
			      ...
			      if( pd->v[pg->imtrx][var] )
				{
				  for(j=0 ; j < ei[pg->imtrx]->dof[var]; j++)
				      ...
				      for( a=0 ; a < dim ; a++ )
					{
				      ...
				      J_AC[ ei[pg->imtrx]->gun_list[var][j] ] += d_term + d_term1 + d_term2;
			      ...
			      if(pd->v[pg->imtrx][var])
				{
				  for( j=0 ; j<ei[pg->imtrx]->dof[var]; j++)
				      ...
				      for( a=0; a < WIM; a++)
					{
				      ...
				      J_AC[ ei[pg->imtrx]->gun_list[var][j] ] += d_term + d_term1 + d_term2 + d_term3;
				    }
				}
			    }
	 ...
	 /* VOLUME_FLUX */
			...
			case CURRENT:
			  ...
			  for(j=0; j<ei[pg->imtrx]->dof[var]; j++)
			      ...
			      for( a=0; a<dim; a++)
				{
			      ...
			      /* J_AC[ ei[pg->imtrx]->gun_list[var][j]  ] += d_term; */
			      J_AC[ ei[pg->imtrx]->gun_list[var][j]  ] = 1.0;
			    }
			...
			case AREA:
			  ...
			  for(p=0; p<dim; p++ )
			      ...
			      if( pd->v[pg->imtrx][var])
				{
				  for (j=0; j<ei[pg->imtrx]->dof[var]; j++)
				    {
				      J_AC[ ei[pg->imtrx]->gun_list[var][j]  ] += weight*fv->dsurfdet_dx[p][j];
				    }
				}
			    }
			  break;
                              ...
                              if( pd->v[pg->imtrx][var])
                                {
                                  for (j=0; j<ei[pg->imtrx]->dof[var]; j++)
                                    {
                                      J_AC[ ei[pg->imtrx]->gun_list[var][j]  ] += 0.5*weight*
			      ...
			      if (pd->v[pg->imtrx][var] )
				{
				  for( j=0; j<ei[pg->imtrx]->dof[var]; j++)
				      ...
				      for( a=0 ; a < dim ; a++)
					{
				      ...
				      J_AC[ ei[pg->imtrx]->gun_list[var][j]  ] += weight * term1 * det +
					                               weight * term2 * det +
 		...
 		(efield[dir]*(-fv->d_grad_V_dmesh[b][p][j]) + (-fv->d_grad_V_dmesh[dir][p][j])*efield[b] - delta(dir,b)*d_term1 ) );
			...
			default:
			  GOMA_EH(GOMA_ERROR,"Constraint sensitivities haven't been implemented for that flux type.");
			  break;
			}
		    }
...
/**  Apply end point conditions when the nset is not found   **/
  	...
  	if ( nset_id != -1 )
		...
		if (num_node_in_set != 1) 
		   GOMA_EH(GOMA_ERROR,"more than one node, this is for 2D only");
		gnn = elem_list[0];
	        ...
	        if ( blk_id == -1 || 
	  	   ...
	  	   GOMA_EH(err, "bf_mp_init");
	           ...
	           id_side = find_id_side (ei[pg->imtrx]->ielem, num_nodes_on_side,
            	   ...
            	   find_nodal_stu(local_node, ei[pg->imtrx]->ielem_type, xi, xi+1, xi+2);
		  ...
		  switch( quantity )
			...
			delta(p,1)*bf[var]->phi[j]*fv->stangent[0][dir]);
				...
				*dsigmadT*bf[var]->phi[j];
			    ...
			    {
			GOMA_EH(GOMA_ERROR, "Point force has not been updated for the PROJECTED_CARTESIAN coordinate system.");
			    }
			}
		      break;
		    default:
...
/*                         mp->surface_tension); */
	...
	if ( J_AC != NULL)
	 ...
	 /* FORCE_X_NEG, FORCE_Y_NEG, FORCE_Z_NEG */
		   ...
		   if(corner_elem == -1) GOMA_EH(GOMA_ERROR,"corner element not found");
  ...
  if( ( eb = in_list(blk_id, 0, exo->num_elem_blocks, exo->eb_id) ) != -1 )
    {
	  ...
	  if ( subgrid_integration_active  )
              ...
              if ((Use_Subelement_Integration = current_elem_on_isosurface(FILL, 0.)))
		{
 	    ...
 	    {
 		for(i=0;i<ei[pg->imtrx]->num_local_nodes;i++)	{ls_F[i]=*esp_old->F[i];}
 		wt_type = 2;
 		if( quantity == I_NEG_FILL || 
				quantity == I_NEG_VOLPLANE ||
				quantity == I_NEG_CENTER_X ||
 		...
 		ierr = adaptive_weight( ad_wt, ip_total, ei[pg->imtrx]->ielem_dim, ls_F, 0.0,
 				 wt_type, ei[pg->imtrx]->ielem_type);
 		if(ierr == -1)printf("adaptive wt problem %d %g %g %g\n",ierr,ls_F[0],ls_F[1],ls_F[2]);
 	    }
	...
	if( quantity == I_SURF_SPECIES ||
		quantity == I_SURF_TEMP )
	  {
	   ...
	   if( quantity == I_SURF_SPECIES)
	     ...
	     {
		ierr=ei[pg->imtrx]->dof[TEMPERATURE]/ei[pg->imtrx]->num_local_nodes;
	   	for(i=0;i<ei[pg->imtrx]->num_local_nodes;i++)	
		   {
...
/* 		  DPRINTF(stderr, "Subgrid integration on element : \t%d\n", elem ); */
	      ...
	      else
		{
 		 ...
 		 if(adaptive_integration_active)
 			...
 			{ fv->wt = wt = Gq_weight (ip, ei[pg->imtrx]->ielem_type); }
		}
	     ...
	     computeCommonMaterialProps_gp(time_value);
             ...
             if( Num_Proc > 1 && dpi->elem_owner[ ei[pg->imtrx]->ielem ] == ProcID ) 
              {
               proc_sum += delta_sum;
              }
  ...
  if(pd->e[pg->imtrx][R_SHELL_SAT_OPEN]) Porous_liq_inventory = sum; 
     ...
     ******************************************************************************/
{
  ...
  if(upd->CoordinateSystem == PROJECTED_CARTESIAN)
    GOMA_WH(-1, "compute_volume_integrand has not been updated for the PROJECTED_CARTESIAN coordinate system.");
      ...
      if( J_AC != NULL )
	{
	  for( p=0; p<dim; p++)
	      ...
	      if( pd->v[pg->imtrx][var] )
		{
		  ...
		  for( j=0; j<ei[pg->imtrx]->dof[var]; j++)
		    {
		      J_AC[ ei[pg->imtrx]->gun_list[var][j] ] += weight * ( h3 * bf[pd->ShapeVar]->d_det_J_dm[p][j] +
    ...
    case I_VOLUME_PLANE:
      {
       ...
       if(num_params < 4)
         {
           GOMA_WH(-1,"not enough plane parameters for VOL_PLANE\n");
         }
  ...
  if (mp->HeatSourceModel == EM_VECTOR_DISS )
	...
	if ( J_AC != NULL)
	  {
	    GOMA_EH(GOMA_ERROR,"Appropriate Jacobian entries for the DISSIP volume integral are not available.\n");
	  }
   ...
   case I_JOULE:
	...
	if ( J_AC != NULL)
	  {
	    GOMA_EH(GOMA_ERROR,"Appropriate Jacobian entries for the I_JOULE volume integral are not available.\n");
	  }
	...
	*sum += weight*det*II;
	...
	if ( J_AC != NULL )
	  {
	    GOMA_EH(GOMA_ERROR,"Jacobian entries for II_GAMMA_DOT Volume integral not implemented.");
	  }
      }
      break;
  ...
  if (efv->ev) {
	...
	GOMA_EH(dir, "PANIC in compute_volume_integrand \n");
	...
	*sum += weight*det*rho*fv->v[dir];
	...
	if ( J_AC != NULL)
	  {
	    GOMA_EH(GOMA_ERROR,"Appropriate Jacobian entries for the MOM volume integral are not available.\n");
	  }
            ...
            if( pd->v[pg->imtrx][var] )
              {
                ...
                for( j=0; j<ei[pg->imtrx]->dof[var]; j++)
                  {
                    J_AC[ ei[pg->imtrx]->gun_list[var][j] ] += rho*Cp*fv->T * weight *
...
/*      GOMA_EH(GOMA_ERROR,"This volumetric integral not yet implemented \n");  */
      }
      break;
            ...
            if( pd->v[pg->imtrx][var] )
              {
                ...
                for( j=0; j<ei[pg->imtrx]->dof[var]; j++)
                  {
                    J_AC[ ei[pg->imtrx]->gun_list[var][j] ] += fv->T * weight *
                              ( h3 * bf[pd->ShapeVar]->d_det_J_dm[p][j] +
                              fv->dh3dq[p]*bf[var]->phi[j] * det_J );
                  }
              }
...
/*      GOMA_EH(GOMA_ERROR,"This volumetric integral not yet implemented \n");  */
	...
	if( vn->ConstitutiveEquation != NOPOLYMER )
	  {
	    GOMA_EH(GOMA_ERROR,"Computation of stress integral for POLYMER models currently no supported.\n");
	  }
	...
	*sum += weight*det*( -3.0*fv->P );
	...
	if ( J_AC != NULL )
	  {
		...
		for ( j=0; j < ei[pg->imtrx]->dof[var]; j++ )
		  {
		    J_AC[ ei[pg->imtrx]->gun_list[var][j] ] += weight*det* ( -3.0* bf[var]->phi[j]);
		  }
	      }
			...
			for ( b=0; b<dim; b++)
			  {
			    ...
			    J_AC [ ei[pg->imtrx]->gun_list[var][j] ] += weight * det * ( d_mu->v[a][j] * 2.0*fv->grad_v[a][a] +
		...
		if( pd->v[pg->imtrx][var] )
		  {
			...
			J_AC[ ei[pg->imtrx]->gun_list[var][j] ] +=  dterm + dterm1;
		      }
		  }
	      }
	  }	  
	...
	if(pd->e[pg->imtrx][R_LUBP]) height = height_function_model
 	...
 	if ( J_AC != NULL )
 	  {
 	    ...
 	    for( j=0 ; j < ei[pg->imtrx]->dof[var]; j++)
 	      {
 		J_AC[ ei[pg->imtrx]->gun_list[var][j]  ] += dwt_dF*height*det;
 	      }
 	  } 
      }
      else
	...
	*sum += height * weight * det * H;
	...
	if ( J_AC != NULL )
	  {
		...
		J_AC[ ei[pg->imtrx]->gun_list[var][j]  ] += height * weight * 
	...
	double alpha, height = 1.0;
       ...
       if(num_params < 5)
         {
           GOMA_WH(-1,"not enough plane parameters for POS-NEG_PLANE_FILL\n");
         }
	...
	if ( J_AC != NULL )
	  {
	    ...
	    for( j=0 ; j < ei[pg->imtrx]->dof[var]; j++)
	      {
		J_AC[ ei[pg->imtrx]->gun_list[var][j]  ] += weight * det * (lsi->gfmag - 1.) *
	...
	*sum += weight*det*lsi->delta;
	...
	if ( J_AC != NULL )
	  {
	    ...
	    for( j=0 ; j < ei[pg->imtrx]->dof[var]; j++)
	      {
		J_AC[ ei[pg->imtrx]->gun_list[var][j]  ] += weight * det * lsi->d_delta_dF[j];
	      }
	  }
      }
      break;
	...
	if ( dir == -1 )
	  GOMA_EH(GOMA_ERROR, "compute_volume_integrand(): ACK! I'm confused!\n");
	...
	if( J_AC != NULL )        
	  {
	    ...
	    for( j=0; j<ei[pg->imtrx]->dof[var]; j++)
	      {
		J_AC[ ei[pg->imtrx]->gun_list[var][j]  ] += weight * det * (sign * lsi->d_H_dF[j]) * fv->v[dir];
	      }
	    
	    var = VELOCITY1 + dir;
	    for( j=0; j<ei[pg->imtrx]->dof[var]; j++)
	      {
		J_AC[ ei[pg->imtrx]->gun_list[var][j]  ] += weight * det * H * bf[var]->phi[j];
	      }
	  }
      }
      break;
	...
	*sum += weight * det * H * fv->x[dir];
	...
	if( J_AC != NULL )        
	  {
	    ...
	    for( j=0; j<ei[pg->imtrx]->dof[var]; j++)
	      {
		J_AC[ ei[pg->imtrx]->gun_list[var][j]  ] += weight * det * (sign * lsi->d_H_dF[j]) * fv->x[dir];
	      }
	...
	if ( J_AC != NULL)
	  {
	    GOMA_EH(GOMA_ERROR,"Appropriate Jacobian entries for the Porous Liquid INV volume integral are not available.\n");
	  }
	...
	if( pd->e[pg->imtrx][R_MASS] ) factor = fv->c[species_no];

    if ( params != NULL ) factor *= params[0];

	if( pd->e[pg->imtrx][R_EFIELD1] )
		{	
           *sum += factor*weight*det*fv->E_field[dir];	
        }
	else if( pd->e[pg->imtrx][R_POTENTIAL] )
		{
           *sum += factor*weight*det*(-fv->grad_V[dir]);
		}		
	else
	  GOMA_EH(GOMA_ERROR,"You need to include the potential and/or electric field variables to compute this integrated quantity\n/");
		  ...
		  for( ip = 0 ; ip < ip_total; ip++ )
		    {
		      ...
		      find_surf_st( ip, elem_type, id_side, ei[pg->imtrx]->ielem_dim, xi, &s, &t, &u);
		      ...
		      surface_determinant_and_normal (elem, ei[pg->imtrx]->iconnect_ptr, 
						      ei[pg->imtrx]->num_local_nodes, 
						      ei[pg->imtrx]->ielem_dim - 1,  
						      id_side,
						      nodes_per_side,
			   ...
			   double J_AC[] )
{
  ...
  switch (quantity) 
	...
	*sum += weight*fv->v[a]*fv->snormal[a]*det;
      ...
      if ( J_AC != NULL )
	{
	  GOMA_EH(GOMA_ERROR, "Global surface sentivities are on my to-do list.") ;
	}
      break;
    case NEG_LS_FLUX:
    case POS_LS_FLUX:
...
/*  variables for adaptive quadrature weight calculation   */
 ...
 current_id, exo);
				...
				quantity == DELTA || quantity == LS_DCA ))
		  {
			...
			if(species_id > 0)
			{
			  ...
			  ierr = adaptive_weight( ad_wt, ip_total, ielem_dim-1,
                                  ls_F, 0.0, wt_type, ei[pg->imtrx]->ielem_type);
			  if(ierr == -1)printf("adaptive wt problem %d %g %g %g\n",ierr,ls_F[0],ls_F[1],ls_F[2]);
		  }
	  ...
	  for (ip = 0; ip < ip_total; ip++)
	    ...
	    GOMA_EH( err, "load_fv_grads_sens");
	    ...
	    if(pd_glob[0]->CoordinateSystem == PROJECTED_CARTESIAN)
	      GOMA_EH(GOMA_ERROR, "evalute_flux_sens has not been updated for the PROJECTED_CARTESIAN coordinate system.");
...
/*     velocities of bottom and top surfaces   */
      ...
      case FORCE_NORMAL:
		...
		{
	GOMA_EH(GOMA_ERROR,"Force Sensitivity Calculation for Solids  not Available");
                for ( a=0; a<VIM; a++)
                  {
                    for ( b=0; b<VIM; b++)
                      {
...
load_fv_grads_sens(void)
{
   ...
   */
  dofs = ei[pg->imtrx]->dof[PVELOCITY1];
  v = PVELOCITY1;
  if(pd->v[pg->imtrx][v])
    {
      for ( p=0; p<VIM; p++)
...
#else
			GOMA_EH(GOMA_ERROR,"Turn off NO_CHEBYSHEV_PLEASE please at compile time.\n");
#endif				
			for(j=0 ; j<9 ; j++)	
...
switch (interface_type)
	...
	case 102:
		...
		break;
 	default:
 		GOMA_EH(GOMA_ERROR,"shouldn't get here - F_type switch \n");
 		break;
 	} /* end of F_type switch */
...
for (i=0 ; i<n_chev ; i++)
	...
	cm = 0.5*(y*(cb-ca)+cb+ca);
 	...
 	switch (j)
		{
		case 0:
			GOMA_EH(GOMA_ERROR," no interior intersections found\n"); break;
		case 1:
			fval[i]=xint[0]; break;
		case 2:
...
switch (interface_type)
	...
	*c[2] + 45*c[2]*c[2] - 36*c[1]*c[3] - 45*c[2]*c[3] + 50*c[3]*c[3]) + 4*
	...
	break;
default:
	GOMA_EH(GOMA_ERROR,"unknown Chebyshev polynomial order");
	break;
}
