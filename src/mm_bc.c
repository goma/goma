  ...
  if (boundary_condition->desc->method == DIRICHLET 
	...
	if (boundary_condition->BC_Name == DX_USER_NODE_BC ||
	  ...
	  if (boundary_condition->BC_Name == F_DIODE_BC)
	    {
	      GOMA_EH(GOMA_ERROR, "F_DIODE_BC has special needs. You need to comment this line out and follow instructions");
	      ...
	      for ( i=0; i<exo->ss_num_sides[iss1]; i++)
		{
		  ...
		  if ( j != -1 )
		      ...
		      for (k=0; k<num_nodes_on_side; k++)
			{
		      ...
		      if (node_ctr == 0) GOMA_EH(GOMA_ERROR, "No nodes on edge");
      ...
      *****************************************************************/
{
  ...
  for (ibc = 0; ibc < Num_BC; ibc++) {
    ...
    if (!strcmp(BC_Types[ibc].Set_Type, "LS")) {
      ...
      if ( ls == NULL)
        {
          GOMA_EH(GOMA_ERROR,"Attempt to apply bc on LS but level set is not active, did you intend PF?\n");
        }
