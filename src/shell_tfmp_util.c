...
void load_tfmp_viscosity_model(double *mu_l, double *mu_g) {
...
void load_gas_density_model(double *Patm, double *rho_g, double *drho_g_dP) {
  // temporary variables
  double Mg, R, T;
...
void load_molecular_diffusion_model(double S,       // saturation
...
void load_relative_permeability_model(double S,        // saturation
...
void load_gas_dissolution_model(
    double h, double Patm, double *J, double *dJ_dP, double *dJ_dS, double *dJ_dh) {
  // temporary variables
  double Dgl, Hgls, Mg, trans_diss, sqrtPI, S, bg, dbg_dP, fS, dfS_dS, lambda, Vd, L, dL_dh, pi;
    ...
    if (S < 1.0 && S > 0.0) {
      ...
      if (S > trans_diss) {
        ...
        if (Vd / lambda / lambda / h >= 1.0) { // Length is maxed for lattice defined by
          ...
          dL_dh = lambda / 2.0 / sqrt(2.0 * (1.0 - Vd / lambda / lambda / h)) * Vd / lambda /
                  lambda / h / h;
        }
      }
...
void load_displacement_coupling_model(double tt,
                                      double delta_t,
                                      double *h,
                                      double *dh_dtime,
                                      double *gradII_h,
  ...
  case GM_NDOTD:
    h0_minus_ndotd(tt, delta_t, h, dh_dtime, gradII_h, dh_dmesh, dh_dnormal, d2h_dtime_dmesh,
                   d2h_dtime_dnormal, d_gradIIh_dmesh, d_gradIIh_dnormal, n_dof, dof_map);
    break;
  case GM_RADIAL:
    rmesh_minus_rroller(tt, delta_t, h, dh_dtime, gradII_h, dh_dmesh, dh_dnormal, d2h_dtime_dmesh,
                        d2h_dtime_dnormal, d_gradIIh_dmesh, d_gradIIh_dnormal, n_dof, dof_map);
    break;
  }
}

void h0_minus_ndotd(double tt,
                    double delta_t,
                    double *h,
                    double *dh_dtime,
                    double *gradII_h,
  ...
  case SIK_S:
    ...
    for (int i = 0; i < ei[pg->imtrx]->dof[MESH_DISPLACEMENT1];
         i++) { // elemental variable degree of freedom i
    ...
    for (int k = 0; k < DIM; k++) { // displacement index k
      ...
      for (int m = 0; m < DIM; m++) {
        for (int j = 0; j < ei[pg->imtrx]->dof[MESH_DISPLACEMENT1];
             j++) { // sensitivity to displacement k degree of freedom j
                    //   gradient direction 'csi'
                    //   |  sensitivity to this displacement
                    //   |  |
                    //   v  v
          d_grad_d_dmesh[k][0][m][j] =
              csigrad[k] * (-1.0f) / det_J / det_J * d_det_J_dmeshkj[m][j] +
              delta(k, m) * bf[MESH_DISPLACEMENT1]->dphidxi[j][0] / det_J;
        }
      }
    }
    // switch indices and load into II vars because we don't need another set of intermediate
    // variables
    for (int k = 0; k < DIM; k++) {   // vector element k
  ...
  case SIK_XY:                    // not really implemented yet but this kinda works
    ...
    for (k = 0; k < DIM; k++) {
      ShellRotate(grad_n[k], dgrad_n_dmesh[k], gradII_n[k], dgradII_n_dmesh[k],
                  n_dof[MESH_DISPLACEMENT1]);
      ShellRotate(grad_disp[k], dgrad_disp_dmesh[k], gradII_disp[k], dgradII_disp_dmesh[k],
                  n_dof[MESH_DISPLACEMENT1]);
    }
    break;
  }
      ...
      *h -= h_sign * (normal[k]) * (fv->d[k]);
      ...
      if (pd->TimeIntegration == TRANSIENT) {
        *(dh_dtime) -= h_sign * (fv->n[k] * fv_dot->d[k] + fv_dot->n[k] * fv->d[k]);
        ...
        for (l = 0; l < DIM; l++) { // gradient direction l
          for (i = 0; i < ei[pg->imtrx]->dof[MESH_DISPLACEMENT1]; i++) {
            d2h_dtime_dmesh[k][i] -= h_sign * fv->n[l] * delta(l, k) *
                                     bf[MESH_DISPLACEMENT1]->phi[i] * (1.0f + 2.0f * tt) / delta_t;
            d2h_dtime_dmesh[k][i] -=
                h_sign * fv_dot->n[l] * delta(k, l) * bf[MESH_DISPLACEMENT1]->phi[i];
          }
        }
        ...
        if (pd->e[pg->imtrx][R_SHELL_NORMAL1]) {
          for (i = 0; i < ei[pg->imtrx]->dof[MESH_DISPLACEMENT1]; i++) {
            ...
            d2h_dtime_dnormal[k][i] -=
                h_sign * fv->d[k] * bf[SHELL_NORMAL1]->phi[i] * (1.0 + 2.0 * tt) / delta_t;
          }
        }
      }
      for (i = 0; i < ei[pg->imtrx]->dof[MESH_DISPLACEMENT1]; i++) {
      ...
      for (l = 0; l < DIM; l++) { // gradient direction l
        for (i = 0; i < ei[pg->imtrx]->dof[MESH_DISPLACEMENT1];
          ...
          for (int m = 0; m < DIM; m++) { // sensitivity to displacement m
            ...
            d_gradIIh_dmesh[l][m][i] -=
                h_sign * d_grad_n_dmesh[k][l][m][i] * fv->d[k] +
...
void rmesh_minus_rroller(double tt,
                         double delta_t,
                         double *h,
                         double *dh_dtime,
                         double *gradII_h,
  ...
  *h = rmesh - rroller;
  ...
  if (mp->ehl_integration_kind == SIK_S) {
    ...
    GOMA_EH(GOMA_ERROR, "I don't know how to set gradII_h unless 'Elastohydrodynamic Lubrication "
                        "Shell Integration Kind' is 'S'.\n"
  ...
  if (pd->TimeIntegration == TRANSIENT) {
    for (int k = 0; k < DIM - 1; k++) {
      for (int j = 0; j < ei[pg->imtrx]->dof[MESH_DISPLACEMENT1]; j++) {
        d2h_dtime_dmesh[k][j] =
            bf[MESH_DISPLACEMENT1 + k]->phi[j] / rmesh *
                ((x[k] - xc[k]) * (1.0 + 2.0 * tt) / delta_t + fv_dot->x[k]) -
            1.0 / rmesh / rmesh * (*dh_dtime) * (x[k] - xc[k]) * bf[MESH_DISPLACEMENT1 + k]->phi[j];
      }
    }
  }
  // set d_gradIIh_dmesh
      ...
      d_dh_dcsi_dmesh[m][j] =
          ((x[m] - xc[m]) * d_dwx_dcsi_dmesh[m][m][j] +
           bf[MESH_DISPLACEMENT1]->phi[j] * dwx_dcsi[m]) /
              rmesh +
          (((x[0] - xc[0]) * dwx_dcsi[0] + (x[1] - xc[1]) * dwx_dcsi[1]) * -1.0 / rmesh / rmesh /
           rmesh * bf[MESH_DISPLACEMENT1]->phi[j] * (x[m] - xc[m]));

      d_gradIIh_dmesh[0][m][j] =
          d_dh_dcsi_dmesh[m][j] / det_J + -dh_dcsi / det_J / det_J * d_det_J_dmeshkj[m][j];
    }
  }
}
...
void detJ_2d_bar(double *det_J, double d_det_J_dmesh[][MDE]) {
...
void ShellBF_2d_bar(int eq_var,
                    int ii,
  ...
  for (int j = 0; j < ei[pg->imtrx]->dof[eq_var]; j++) {
    d_gradII_phi_i_dx[0][0][j] =
        bf[eq_var]->dphidxi[ii][0] * (-1.0) / det_J / det_J * d_det_J_dmesh[0][j];
    d_gradII_phi_i_dx[0][1][j] =
        bf[eq_var]->dphidxi[ii][0] * (-1.0) / det_J / det_J * d_det_J_dmesh[1][j];
  }
}
...
void load_gap_model(GAP_STRUCT *gap) {
  ...
  if (fpclassify(gap->h) != fp_type && gap->h != 0.0) {
    GOMA_EH(GOMA_ERROR,
            "mass term is not normal after retrieving initial value from height_function_model()");
  }
  ...
  if (mp->ehl_integration_kind == SIK_S) {
    if (mp->HeightUFunctionModel == ROLLER) {
      ...
      for (int j = 0; j < ei[pg->imtrx]->dof[MESH_DISPLACEMENT1]; j++) {
        gap->d_gradIIh_dmesh[0][0][j] += 1.0 / sqrt(CUBE(SQUARE(r) - SQUARE(x - xc))) * (x - xc) *
                                         bf[MESH_DISPLACEMENT1]->phi[j] * (x - xc) * gradII_x[0];
        gap->d_gradIIh_dmesh[0][0][j] +=
            1.0 / sqrt(SQUARE(r) - SQUARE(x - xc)) * bf[MESH_DISPLACEMENT1]->phi[j] * gradII_x[0];
        gap->d_gradIIh_dmesh[0][0][j] +=
            1.0 / sqrt(SQUARE(r) - SQUARE(x - xc)) * (x - xc) * dgradII_x_dmesh[0][0][j];
        gap->d_gradIIh_dmesh[0][1][j] +=
            1.0 / sqrt(SQUARE(r) - SQUARE(x - xc)) * (x - xc) * dgradII_x_dmesh[0][1][j];
      }
    } else {
      for (int k = 0; k < DIM; k++) {
  ...
  if (mp->FSIModel == FSI_SHELL_ONLY_MESH) {
    load_displacement_coupling_model(
        gap->tt, gap->delta_t, &(gap->h), &(gap->dh_dtime), gap->gradII_h, gap->dh_dmesh,
        gap->dh_dnormal, gap->d2h_dtime_dmesh, gap->d2h_dtime_dnormal, gap->d_gradIIh_dmesh,
        gap->d_gradIIh_dnormal, gap->n_dof, gap->dof_map);
  }
}
...
void load_roller_normal_into_fv(void) {
  ...
  mag_unnormalized_n =
      sqrt(unnormalized_n[0] * unnormalized_n[0] + unnormalized_n[1] * unnormalized_n[1]);
  fv->snormal[0] = unnormalized_n[0] / mag_unnormalized_n;
  fv->snormal[1] = unnormalized_n[1] / mag_unnormalized_n;

  for (int j = 0; j < ei[pg->imtrx]->dof[MESH_DISPLACEMENT1]; j++) {
    fv->dsnormal_dx[0][0][j] = 1. / mag_unnormalized_n * bf[MESH_DISPLACEMENT1]->phi[j];
    fv->dsnormal_dx[1][0][j] = -unnormalized_n[0] / unnormalized_n[1] / mag_unnormalized_n *
