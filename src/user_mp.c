...
usr_FlowingLiquidViscosity(dbl *param) /* ptr to user-defined parameter list */
{
  ...
  dbl dmudC[MAX_CONC]; /* heat source derivative wrt concentration*/
  dbl dmudX[DIM]; /* heat source derivative wrt displacement*/
  dbl X[DIM], F, T, C[MAX_CONC], V[DIM]; /* Convenient local variables */
  dbl eff_mu, effmu_dT, velo_perm, hole_rad, v_mag=0.;
  dbl dvelo_perm_dT, dvelo_perm_dV;
  dbl A=5.230102, B=-3.67051, C0=4.11014, Aa=0.048797, Ba=-0.00066, Ca=-0.03461;
  dbl M=12.92373, N=0.57084, Ma=0.074003, Na=-0.008004;
  dbl Tref=195.0, dT, v_avg=12.66426;
  dbl eta0=164.067;
...
*/
 ...
 /*NB This ought to be done once for all fields at gauss pt*/
  ...
  for(a=0; a<DIM; a++)v_mag += SQUARE(V[a]);		  /*Do not touch */
  v_mag = sqrt(v_mag);
  for(a=0; a<pd->Num_Species_Eqn; a++) C[a] = fv->c[a];   /*Do not touch */
  ...
  */
  ...
  hole_rad = param[0] + param[1]*X[1] + param[2]*SQUARE(X[1]);
  dT = T-Tref;
  if(1)
	{ 
	v_mag = v_avg;
  	velo_perm = exp(Ma*dT)*pow(v_mag,Na*dT); 
  	dvelo_perm_dV = 0.0;
	}
  else if(v_mag < 0.1*v_avg)
	{ 
	v_mag = 0.1*v_avg;
  	velo_perm = exp(Ma*dT)*pow(v_mag,N+Na*dT)/pow(v_avg,N); 
  	dvelo_perm_dV = 0.0;
	}
  else
	{
  	velo_perm = exp(Ma*dT)*pow(v_mag,N+Na*dT)/pow(v_avg,N); 
  	dvelo_perm_dV = (N+Na*dT)*velo_perm/SQUARE(v_mag);
	}
  dvelo_perm_dT = velo_perm*(Ma +Na*log(v_mag));
  mu = A*exp(Aa*dT)*SQUARE(hole_rad)+B*exp(Ba*dT)*hole_rad+C0*exp(Ca*dT);
  dmudT=A*Aa*exp(Aa*dT)*SQUARE(hole_rad)+B*Ba*exp(Ba*dT)*hole_rad +C0*Ca*exp(Ca*dT);
  
  eff_mu = eta0/(mu*velo_perm);
  effmu_dT = -eta0*(velo_perm*dmudT+mu*dvelo_perm_dT)/SQUARE(mu*velo_perm);
  dmudV[0] = -eta0/mu/SQUARE(velo_perm)*dvelo_perm_dV*V[0];
  dmudV[1] = -eta0/mu/SQUARE(velo_perm)*dvelo_perm_dV*V[1];
  dmudX[1] = -eta0/velo_perm/SQUARE(mu)*
                   (2.*A*exp(Aa*dT)*hole_rad + B*exp(Ba*dT));
  dmudX[1] *= (param[1] + param[2]*2.*X[1]);
#if 0
fprintf(stderr,"perm mu %g %g %g\n",mu,fake_perm,eff_mu);
#endif
 /****************Don't touch these lines***********************/  
  mp->FlowingLiquid_viscosity = eff_mu;                           /*Do not touch */
  mp->d_FlowingLiquid_viscosity[TEMPERATURE] = effmu_dT;	      /*Do not touch */
						              /*Do not touch */
  for ( a=0; a<DIM; a++)                                      /*Do not touch */
    {						              /*Do not touch */
