...
extern void stslc3_(int *m,
                    ...
                    double *errbds,
                    int *select);
extern void cpdnaupc_(MPI_Comm *comm,
                      int *ido,
                      char *bmat,
                      int *nloc,
                      char *which,
                      ...
                      int *lworkl,
                      int *info);
extern void cpdneupc_(MPI_Comm *comm,
                      int *ivec,
                      char *howmny,
                      int *celect,
                      double *d,
...
static int eig_driver(char which[],
                      char bmat[],
                      int iparam[],
                      int mode,
                      double sigma,
...

{
  ...
  az_fail_cnt = eig_driver(which, bmat, iparam, mode, sigma, mu, delta, zeta, nev, ncv, info, tol,
                           eta, cgi->printproc, cgi->numOwnedUnks, cgi->numUnks,
                           con->private_info.step_num, jmax, cei->sort, comm, saved_displacement);

  if (info != 0) {
    if (cgi->printproc > 1)
      printf("  Error %d in eigensolver\n", info);
    exit(-1);
  }
...
/* matShifted is temp matrix, nnz is # nonzeros in matrix */
{
  ...
  while (flag == 1) {
    ...
    cpdnaupc_(&comm, &ido, bmat, &nloc, which, &nev, &tol, resid, &ncv, v, &ldv, iparam, ipntr,
    ...
    if ((ido == -1) || (ido == 1)) {
      ...
      if (mode == SHIFTI) {
        polez3_(&ncv, &new_sigma, &new_mu, &delta, &zeta, &workl[ipntr[5] - 1],
                &workl[ipntr[6] - 1], &workl[ipntr[7] - 1], trans, trans + ncv, trans + 2 * ncv,
                &temp_ncv, &temp_nconv, &tol, &info_p);
      }
      /* Cayley */
      else if (sigma < mu) {
        poleze_(&ncv, &new_sigma, &new_mu, &workl[ipntr[5] - 1], &workl[ipntr[6] - 1],
                &workl[ipntr[7] - 1], trans, trans + ncv, trans + 2 * ncv, &temp_ncv, &temp_nconv,
                &tol, &info_p, workpol, workpol + ncv, workpol + 2 * ncv);
      } else if (sigma > mu) {
        polez2_(&ncv, &new_sigma, &new_mu, &delta, &workl[ipntr[5] - 1], &workl[ipntr[6] - 1],
                &workl[ipntr[7] - 1], trans, trans + ncv, trans + 2 * ncv, &temp_ncv, &temp_nconv,
                &tol, &info_p);
      }
      ...
      if (printproc > 7) {
        ...
        for (kk = 0; kk < temp_ncv; kk++) {
          printf("\t %2d. %g  %g  %g\n", kk, trans[kk], trans[kk + ncv], trans[kk + 2 * ncv]);
        }
  ...
  if (info < 0) {
    ...
    if (mode == SHIFTI) {
      stslc3_(&ncv, &iparam[4], &tol, &sigma, &mu, &delta, &zeta, &trans[temp_ncv - iparam[4]],
              &workl[lworkl - 3 * ncv], &workl[lworkl - 2 * ncv], &workl[lworkl - ncv], select);
    } else if (sigma < mu) {
      stslct_(&ncv, &iparam[4], &tol, &sigma, &mu, &trans[temp_ncv - iparam[4]],
              &workl[lworkl - 3 * ncv], &workl[lworkl - 2 * ncv], &workl[lworkl - ncv], select);
    } else {
      stslc2_(&ncv, &iparam[4], &tol, &sigma, &mu, &delta, &trans[temp_ncv - iparam[4]],
              &workl[lworkl - 3 * ncv], &workl[lworkl - 2 * ncv], &workl[lworkl - ncv], select);
    }
    ...
    ierr = 0;
    sprintf(string, "A");
    cpdneupc_(&comm, &rvec, string, select, d, v, &ldv, &sigma, &mu, &delta, workev, bmat, &nloc,
              &nloc2, which, &nev, &tol, resid, &ncv, iparam, ipntr, workd, workl, &lworkl, &ierr,
              work);
      ...
      for (j = 0; j < nconv; j++) {
        ...
        if (d[j + ncv] == 0.0) {
          ...
          if (j < jmax) {
            ...
            eigenvector_output_conwrap(j, 1, &v[(j)*ldv], d[j], NULL, 0, con_step_num,
                                       saved_displacement);
          }
          ...
          if (j < jmax) {
            if (printproc > 1)
              printf("Printing eigenvector pair for complex eigenvalues:  %g +- %g i\n", d[j],
                     fabs(d[j + ncv]));

            eigenvector_output_conwrap(j, 2, &v[(j)*ldv], d[j], &v[(j + 1) * ldv], fabs(d[j + ncv]),
                                       con_step_num, saved_displacement);
          }

          d[j + 2 * ncv] =
              null_vector_resid(d[j], d[j + ncv], &v[(j)*ldv], &v[(j + 1) * ldv], TRUE);
