...
int assemble_stress_fortin(dbl tt,           /* parameter to vary time integration from
  ...
  for (mode = 0; mode < vn->modes; mode++) {
    ...
    if (jeffreysEnabled) {
      ...
      // lambda2 = mupJeff/G, lambda = mup/G --> lambda2 = lambda*mupJeff/mup
      lambda2 = mupJeff / mup * lambda;
    }
        ...
        for (b = 0; b < VIM; b++) {
          ...
          if (a <= b) /* since the stress tensor is symmetric, only assemble the upper half */
          {
            ...
            for (i = 0; i < ei->dof[eqn]; i++) {
              ...
              lec->R[LEC_R_INDEX(upd->ep[eqn], i)] += mass + advection + diffusion + source;
            }
          }
        }
      }
              ...
              if (pd->v[var]) {
                ...
                for (j = 0; j < ei->dof[var]; j++) {
                  ...
                  if (pd->e[eqn] & T_SOURCE) {
                    ...
                    if (jeffreysEnabled) {
                      /* d(lambda2) = d(lambda1*mupJeff/mup)
                       *            = mupJeff/mup*d(lambda1) + lambda1/mup*d(mupJeff) -
                       * lambda1/(mup^2*mupJeff)*d(mup) = -lambda1/(mup^2*mupJeff)*d(mup)
                       *
                       * this assumes lambda1 and mupJeff are constant, so the computation that
                       * follows will need to be ammended if this changes
                       */
                      const dbl d_lambda2_dT = -lambda1 / (mup * mup * mupJeff) * d_mup->T[j];
                      source -= d_lambda2_dT *
                ...
                if (pd->v[var]) {
                  ...
                  for (j = 0; j < ei->dof[var]; j++) {
                    ...
                    if (pd->e[eqn] & T_SOURCE) {
                      ...
                      if (jeffreysEnabled) {
                        // this assumes lambda1 and mupJeff are constant wrt velocity
                        const dbl d_lambda2_dv_pj = -lambda1 / (mup * mup * mupJeff) * d_mup_dv_pj;
                        source_c -= d_lambda2_dv_pj * at *
                                    (g_dot[a][b] + gt_dot[a][b] + v_dot_del_g[a][b] -
                                     x_dot_del_g[a][b] + v_dot_del_gt[a][b] - v_dot_del_gt[b][a] -
                                     (g_dot_g[a][b] + 2 * gt_dot_g[a][b] + gt_dot_gt[a][b]));
                      }
                ...
                for (j = 0; j < ei->dof[var]; j++) {
                  ...
                  for (w = 0; w < pd->Num_Species_Eqn; w++) {
                    ...
                    if (pd->e[eqn] & T_SOURCE) {
                      ...
                      if (jeffreysEnabled) {
                        // this assumes lambda1 and mupJeff are constant wrt velocity
                        const dbl d_lambda2_C = -lambda1 / (mup * mup * mupJeff) * d_mup->C[w][j];
                        source_a -= d_lambda2_C * at *
                                    (g_dot[a][b] + gt_dot[a][b] + v_dot_del_g[a][b] -
                                     x_dot_del_g[a][b] + v_dot_del_gt[a][b] - v_dot_del_gt[b][a] -
                                     (g_dot_g[a][b] + 2 * gt_dot_g[a][b] + gt_dot_gt[a][b]));
                      }
              ...
              if (pd->v[var]) {
                ...
                for (j = 0; j < ei->dof[var]; j++) {
                  ...
                  if (pd->e[eqn] & T_SOURCE) {
                    ...
                    if (jeffreysEnabled) {
                      // this assumes lambda1 and mupJeff are constant wrt velocity
                      const dbl d_lambda2_P = -lambda1 / (mup * mup * mupJeff) * (d_mup->P[j]);
                      source_a -= d_lambda2_P * at *
                                  (g_dot[a][b] + gt_dot[a][b] + v_dot_del_g[a][b] -
                                   x_dot_del_g[a][b] + v_dot_del_gt[a][b] - v_dot_del_gt[b][a] -
                                   (g_dot_g[a][b] + 2 * gt_dot_g[a][b] + gt_dot_gt[a][b]));
                    }
                  ...
                  for (j = 0; j < ei->dof[var]; j++) {
                    ...
                    if (pd->e[eqn] & T_SOURCE) {
                      ...
                      if (jeffreysEnabled) {
                        ...
                        const dbl d_lambda2_dmesh_pj =
                            -lambda1 / (mup * mup * mupJeff) * d_mup_dmesh_pj;

                        source_jb += (lambda2 * at * (d_vdotdelg_dm + d_vdotdelgt_dm));
                        source_jb -= d_lambda2_dmesh_pj *
                                     (g_dot[a][b] + gt_dot[a][b] + v_dot_del_g[a][b] -
                                      x_dot_del_g[a][b] + v_dot_del_gt[a][b] - v_dot_del_gt[b][a] -
                                      (g_dot_g[a][b] + 2 * gt_dot_g[a][b] + gt_dot_gt[a][b]));
                        ...
                        if (pd->TimeIntegration != STEADY) {
                          ...
                          source_jc -= lambda2 * (d_xdotdelg_dm + d_xdotdelgt_dm);
                      ...
                      for (j = 0; j < ei->dof[var]; j++) {
                        ...
                        if (pd->e[eqn] & T_ADVECTION) {
                          if (DOUBLE_NONZERO(lambda)) {
                            ...
                            advection *=
                                wt_func * wt * at * lambda * pd->etm[eqn][(LOG2_ADVECTION)];
                          }
                        }
    ...
    for (a = 0; a < VIM; a++) {
      for (b = 0; b < VIM; b++) {
        ...
        for (i = 0; i < ei->dof[eqn]; i++) {
          ...
          lec->R[LEC_R_INDEX(upd->ep[eqn], i)] += advection + source;
        }
      }
    }
  }
                            ...
                            int num_local_nodes) /* number of nodes per element */
{
  ...
  for (ip = 0; ip < ip_total_fill; ip++) {
    ...
    surface_determinant_and_normal(ielem, ei->iconnect_ptr, num_local_nodes, ei->ielem_dim - 1,
...

{
  ...
  for (ip = 0; ip < ip_total; ip++) {
    ...
    for (i = 0; i < ei->dof[v]; i++) {
      phi[i] = newshape(xi, ielem_type, PSI, ei->dof_list[v][i], ielem_shape, pd->i[v], i);
    ...
    for (i = 0; i < ei->dof[v]; i++) {
      phi_map[i] = newshape(xi, ielem_type, PSI, ei->dof_list[v][i], ielem_shape, pd->i[v], i);
    }
            ...
            if (pd->v[v]) {
              for (i = 0; i < num_local_nodes; i++) {
                ...
                for (j = 0; j < nvdof; j++) {
                  ...
                  ie = Index_Solution(gnn, v, 0, j, ei->matID_ledof[ledof]);
...
dbl compute_saramito_model_terms(dbl stress[DIM][DIM],
                                 dbl yieldStress,
                                 dbl yieldExpon,
