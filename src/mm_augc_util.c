 ...
 */
void
load_extra_unknownsAC(int iAC,    /* ID NUMBER OF AC'S */
		      ...
		      Dpi *dpi)	  /* distributed processing information */
{
  ...
  if ( augc[iAC].Type == AC_OVERLAP || 
	   augc[iAC].Type == AC_PF_CONSTRAINT )
    {
...
void
update_parameterAC(int iAC,      /* ID NUMBER OF The AC */
		   ...
		   Dpi *dpi)	 /* distributed processing information */
{
  ...
  if( augc[iAC].Type == AC_ARC_LENGTH )
      ...
      for (iCC=0; iCC<nCC; iCC++)
	{
	  ...
	  update_parameterC(iCC, value, x, xdot, xa, cont->Delta_s0,
			    cx, exo, dpi);
	}
    }
	      ...
	      MF_Args *mf_args)	
{
  ...
  */
  ...
  load_extra_unknownsAC(iAC, x_AC, 
			cx, mf_args->exo, mf_args->dpi);
  ...
  if (!have_bAC)
    {
      ...
      update_parameterAC(iAC, mf_args->x, mf_args->xdot, x_AC, 
			 cx, mf_args->exo, mf_args->dpi);
      ...
      err = matrix_fill_full(mf_args->ams, 
			     mf_args->x, 
			     res_p, 
			     mf_args->x_old,
			     mf_args->x_older,
  ...
  */
  ...
  if (!have_dAC)
      ...
      for (jAC = 0;jAC < nAC; jAC++) 
	{
	  ...
	  update_parameterAC(jAC, mf_args->x, mf_args->xdot, x_AC, 
			     cx, mf_args->exo, mf_args->dpi);
	     ...
	     MF_Args *mf_args)
{
  ...
  if (augc[iAC].Type == AC_FLUX)
    {
      inventory = evaluate_flux(mf_args->exo,
				mf_args->dpi,
				augc[iAC].SSID,
				augc[iAC].MFID,
				NULL,
	...
	{
	  user_aug_cond_volume_residuals(iAC, mf_args->x, mf_args->xdot,
					 *(mf_args->delta_t),  *(mf_args->time),
					 x_AC, gAC,  cAC, dAC, numProcUnknowns,
    ...
    {
      if (augc[iAC].d_lsvol_dx == 0) DPRINTF(stderr,"\n can't see vectors ");
      ...
      if (Num_Proc > 1) 
	{
	  MPI_Allreduce(&inventory, &global_inventory, 1, MPI_DOUBLE, MPI_SUM,
			MPI_COMM_WORLD);
	  MPI_Allreduce(&LSvel_inventory, &global_LSvel_inventory, 1, MPI_DOUBLE,
...
/* Locate fluid and solid blocks in exo structure */
  for (ib = 0; ib < exo->num_elem_blocks; ib++)
    {
      if (exo->eb_id[ib] == solid_eb) ibs = ib;
      if (exo->eb_id[ib] == fluid_eb) ibf = ib;
    }
		     ...
		     MF_Args *mf_args)
{
  ...
  for (jAC=0;jAC<nAC;jAC++) 
      ...
      if(augc[jAC].Type == AC_LS_VEL && iAC == jAC ) 
	{
	  ...
	  inventory_p = find_LS_vel( mf_args->exo,  mf_args->dpi, NULL, chosen_vel,
				     mf_args->x, numProcUnknowns); 
	  ...
	  inventory_m = find_LS_vel( mf_args->exo,  mf_args->dpi, NULL, chosen_vel, 
				     mf_args->x, numProcUnknowns);
      ...
      switch(augc[jAC].Type)
        {
          case AC_FLUX:
            ...
            if( Num_Proc > 1 ) 
              {
                MPI_Allreduce(&inventory, &global_inventory, 1,
            ...
            if( Num_Proc > 1 ) 
              {
                MPI_Allreduce(&inventory, &global_inventory, 1,
                              MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
                MPI_Allreduce(&LSvel_inventory, &global_LSvel_inventory, 1,
                              MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
                inventory = global_inventory;
                LSvel_inventory = global_LSvel_inventory;
              }
		...
		
{
  ...
  if ( upd->CoordinateSystem == CYLINDRICAL ||  upd->CoordinateSystem == SWIRLING )
    {
      theta_scale = 2.0*M_PIE;  /* this scaling is to remove the azimuthal factor that is applied by evaluate_flux */
    }
...
/*	  gAC[iAC] = (inventory ) ;  */
  ...
  for(jAC=0; jAC<nAC;jAC++) dAC[iAC][jAC] = 0.0;

return(1);
}

int
arc_length_status(struct con_struct *con, double equation, double delta_y,
 ...
 */
{
  ...
  if (cpi->step_num == 0) return(TRUE);
  param_update = fabs(delta_y) / (reltol * fabs(cgi->param) + abstol);
  scaled_resid = fabs(equation)
...
int
create_overlap_acs(Exo_DB *exo, int iAC)
/*
...
/* For now, only P0 interpolation of Lagrange equation will be supported */
  ...
  augc = (struct AC_Information *) realloc_struct_1
         (augc, struct AC_Information, new_nAC, old_nAC);
              ...
              for (j=0; j<nqp; j++)
                {
                  ...
                  fprintf(ac_id, "Solid element %5d side %d  <%g, %g> AC's:  %3d (X), %3d (Y)\n",
                          e+1, s, Coor[0][inode], Coor[1][inode], ac_count-1, ac_count);
                }
            }
        }
      fclose(ac_id);
...
int
assign_overlap_acs( double x[], Exo_DB *exo )
/*
 *
 */
{
  ...
  for (iAC=0; iAC<nAC; iAC++)
    {
      if (augc[iAC].Type == AC_OVERLAP) break;
    }
  if ( augc[iAC].Type != AC_OVERLAP )
    {
...
/* Read inputs from existing overlap AC */
  ...
  for (; iAC < nAC; iAC++)
   {
     if (augc[iAC].Type == AC_OVERLAP) augc[iAC].lm_elem = -1;
   }
...
int
create_periodic_acs(Exo_DB *exo)
/*
...
/* Save old nAC value */
  ...
  for ( iAC = 0; iAC < old_nAC; iAC++ ) {
    if ( augc[iAC].Type == AC_PERIODIC ) {
      ...
      for (n = beg; n < end; n++) {
        ...
        for ( i = 0; i < count1 && !found; i++ ) {
          if ( node_num == node_list1[i] ) found = TRUE;
        }
        if ( !found ) {
      ...
      for (n = beg; n < end; n++) {
        ...
        for ( i = 0; i < count2 && !found; i++ ) {
          if ( node_num == node_list2[i] ) found = TRUE;
        }
        if ( !found ) {
	...
	for ( j = 0; j < count1 && !found; j++ ) {
	  dim_count = 0;
          for ( k = 0; k < dim; k++ ) {
	    if ( fabs(Coor[k][node_list1[i]]-Coor[k][node_list2[j]]) < 1.e-5*scale ) dim_count++;
	  }
	  if ( dim_count == dim-1 ) {
	    found = TRUE;
	...
	if ( ie >= 0 ) count2++;
      ...
      /* Now, expand augc to make room for them */
      augc = (struct AC_Information *) realloc_struct_1
             (augc, struct AC_Information, new_nAC, nAC);
...
#ifdef NOT_USED
static double fBal_Special(struct AC_Information *augc, double *cAC_iAC, 
		     double *x, Exo_DB *exo)
{
  return 0.0;
}
 ...
 */
double getPositionAC(struct AC_Information *augc, double *cAC_iAC, 
		     double *x, Exo_DB *exo)  {
  double posNode[3];
  int i, ins, inode;
