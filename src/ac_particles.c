		...
		for(k = 0; k < static_exo->ss_num_sides[j]; k++)
		  {
		    ...
		    if(pd->CoordinateSystem != CARTESIAN &&
		      ...
		      GOMA_EH(GOMA_ERROR, "Cannot apply volumetric source particles on this coordinate system.");
...
couple_to_continuum(void)
{
  ...
  if(Particle_Model == SWIMMER_EXPLICIT ||
	...
	GOMA_EH(GOMA_ERROR, "Sorry, only handling CONSTANT density models right now.  Check back later.");
