...
void fluid_stress_sqrt_conf(double Pi[DIM][DIM], STRESS_DEPENDENCE_STRUCT *d_Pi) {
  ...
  */
  ...
  evss_f = 0;
  if (pd->gv[POLYMER_STRESS11] && (vn->evssModel == SQRT_CONF)) {
    evss_f = 1.0;
  }
  ...
  if (pd->gv[POLYMER_STRESS11]) {
    ...
    for (mode = 0; mode < vn->modes; mode++) {
      ...
      for (int ii = 0; ii < VIM; ii++) {
        for (int jj = 0; jj < VIM; jj++) {
          conf[ii][jj] += -(mup / lambda) * (delta(ii, jj) - bdotb[ii][jj]);
        }
      }
    }
    for (int ii = 0; ii < VIM; ii++) {
      for (int jj = 0; jj < VIM; jj++) {
        Pi[ii][jj] += -evss_f * (mu - mus) * gamma_cont[ii][jj] + Heaviside * conf[ii][jj];
      }
    }
  }
  if (gn->ConstitutiveEquation == BINGHAM_MIXED) {
...
void fluid_stress_conf(double Pi[DIM][DIM],
                       STRESS_DEPENDENCE_STRUCT
                           *d_Pi) { /*
        ...
        for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
          ...
          if (pd->v[pg->imtrx][POLYMER_STRESS11]) {
            for (mode = 0; mode < vn->modes; mode++) {
              ...
              d_Pi->T[p][q][j] +=
                  Heaviside * d_mup->T[j] / lambda * (exp_s[mode][p][q] - (double)delta(p, q));
            }
          }
        }
      }
        ...
        for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
          ...
          if (pd->v[pg->imtrx][POLYMER_STRESS11]) {
            for (mode = 0; mode < vn->modes; mode++) {
              ...
              // Log-conformation tensor stress
              d_Pi->nn[p][q][j] +=
                  d_mup->nn[j] / lambda * (exp_s[mode][p][q] - (double)delta(p, q));
            }
          }
        }
      }
        ...
        for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
          ...
          if (pd->v[pg->imtrx][POLYMER_STRESS11]) {
            for (mode = 0; mode < vn->modes; mode++) {
              ...
              // Log-conformation tensor stress
              d_Pi->degrade[p][q][j] +=
                  d_mup->degrade[j] / lambda * (exp_s[mode][p][q] - (double)delta(p, q));
            }
          }
        }
      }
          ...
          for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
            ...
            if (pd->v[pg->imtrx][POLYMER_STRESS11]) {
              for (mode = 0; mode < vn->modes; mode++) {
                ...
                // Log-conformation tensor stress
                d_Pi->pf[p][q][a][j] +=
                    d_mup->pf[a][j] / lambda * (exp_s[mode][p][q] - (double)delta(p, q));
              }
            }
          }
        }
          ...
          for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
            ...
            if (pd->gv[POLYMER_STRESS11]) {
              for (mode = 0; mode < vn->modes; mode++) {
                ...
                d_Pi->v[p][q][b][j] +=
                    Heaviside * d_mup->v[b][j] * (gamma[p][q] - gamma_cont[p][q]);
                // Log-conformation tensor stress
                d_Pi->v[p][q][b][j] +=
                    Heaviside * d_mup->v[b][j] / lambda * (exp_s[mode][p][q] - (double)delta(p, q));
              }
            }
          }
        }
          ...
          for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
            ...
            if (pd->v[pg->imtrx][POLYMER_STRESS11]) {
              for (mode = 0; mode < vn->modes; mode++) {
                ...
                // Log-conformation tensor stress
                d_Pi->X[p][q][b][j] +=
                    d_mup->X[b][j] / lambda * (exp_s[mode][p][q] - (double)delta(p, q));
              }
            }
          }
        }
          ...
          for (b = 0; b < VIM; b++) {
            for (c = 0; c < VIM; c++) {
              ...
              for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
                ...
                /* Note: We use b <= c below to be consistent with the symmetry of the
                   stress tensor and to avoid double contributions to the Jacobian in
                   assemble_momentum */
                if (!(vn->evssModel == LOG_CONF_TRANSIENT ||
            ...
            for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
              ...
              if (pd->v[pg->imtrx][POLYMER_STRESS11]) {
                for (mode = 0; mode < vn->modes; mode++) {
                  ...
                  mup = viscosity(ve[mode]->gn, gamma, d_mup);
                  d_Pi->g[p][q][a][b][j] +=
                      mup * (delta(p, a) * delta(q, b) + delta(p, b) * delta(q, a)) *
                      bf[var]->phi[j];
                }
              }
            }
          ...
          for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
            ...
            if (pd->v[pg->imtrx][POLYMER_STRESS11]) {
              for (mode = 0; mode < vn->modes; mode++) {
                ...
                // Log-conformation tensor stress
                d_Pi->C[p][q][w][j] +=
                    d_mup->C[w][j] / lambda * (exp_s[mode][p][q] - (double)delta(p, q));
              }
            }
          }
        }
