...
// #define ELLIPTIC_MESH_BRAYDEN
#ifdef ELLIPTIC_MESH_BRAYDEN
int assemble_elliptic_mesh(void) {
  ...
  dbl dx_dxi = bf[eqn]->J[0][0];
  dbl dy_dxi = bf[eqn]->J[0][1];
  dbl dx_deta = bf[eqn]->J[1][0];
  dbl dy_deta = bf[eqn]->J[1][1];

  dbl JT = -1.0 * (dx_deta * dy_dxi - dy_deta * dx_dxi);
  dbl absJT = fabs(JT);
  ...
  dbl Snum = (dx_dxi * dx_dxi + dy_dxi * dy_dxi);
  dbl Sden = (dx_deta * dx_deta + dy_deta * dy_deta);
  S[0] = sqrt(Snum / Sden);
  S[1] = sqrt(Sden / Snum);
  T[0] = log(Snum);
  T[1] = log(Sden);
    ...
    for (int aa = 0; aa < dim; aa++) {
      ...
      for (int i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
        ...
        if (diffusion_on) {
          ...
          case 0:
            diffusion +=
                (S[a] + eps_s) * (bf[eqn]->dphidxi[i][0] * (dx_deta * dx_deta + dy_deta * dy_deta) -
                                  bf[eqn]->dphidxi[i][1] * (dx_dxi * dx_deta + dy_dxi * dy_deta));
            break;
          case 1:
            diffusion +=
                (S[a] + eps_s) * (bf[eqn]->dphidxi[i][1] * (dx_dxi * dx_dxi + dy_dxi * dy_dxi) -
                                  bf[eqn]->dphidxi[i][0] * (dx_dxi * dx_deta + dy_dxi * dy_deta));
            break;
          }
          diffusion *= (1.0 / absJT) * wt;
