      ...
      */
      if (nn_volume) {
        ...
        for (i = 0; i < nn_volume; i++) {
          if (pp_volume[i]->volume_type == I_SURF_SPECIES && pp_volume[i]->species_no == -1)
            search_minmax = TRUE;
        }
          ...
          for (i = 0; i < nn_volume; i++) {
            if (pp_volume[i]->volume_type == I_SURF_SPECIES) {
              ...
              if (pp_volume[i]->species_no == -1) {
                ...
                pp_volume[i]->params[pd->Num_Species] = pp_volume[i]->params[pd->Num_Species - 1];
                for (idof = 0; idof < pd->Num_Species_Eqn; idof++) {
                  specavg = specmin[idof] + fraction * (specmax[idof] - specmin[idof]);
                  pp_volume[i]->params[pd->Num_Species] += pp_volume[i]->params[idof] * specavg;
                }
