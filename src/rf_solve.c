...

{
  ...
  if (strcmp(Matrix_Format, "epetra") == 0) {
    ...
    ams[JAC]->RowMatrix = createEpetraRowMatrix(num_internal_dofs + num_boundary_dofs);
    createEpetraGomaProblemGraph(ams[JAC], exo, dpi);
...
/* If exporting variables to another code, save them now! */
  ...
  *te_out = time;
  ...
  if (last_call) {
    if (strcmp(Matrix_Format, "epetra") == 0)
    {
      deleteEpetraRowMatrix(ams[JAC]->RowMatrix);
