...
static int
neighbor_species(Exo_DB *,     /* exo - ptr to exodus ii mesh information   */
                 Dpi *, dbl[], /* x                                         */
                 int,          /* current_elem                              */
                 int,          /* neighbor_elem                             */
...
#ifdef COUPLED_FILL
int assemble_fill(double tt, double dt, PG_DATA *pg_data, const int applied_eqn,
                  double xi[DIM], Exo_DB *exo, double time,
  ...
  dbl supg_term = 0.0; /* Major term for SUPG -- see get_supg_stuff(). */
  dbl vcent[DIM];      /* Element centroid velocity (get_supg_stuff(). */
  dbl d_vcent_du[DIM][MDE]
                [DIM]; /* deriv. of vcent[] w.r.t. nodal velocities.   */
  dbl d_supg_term_du[MDE]
                    [DIM]; /* deriv. of supg_term w.r.t. nodal velocities. */
  dbl d_supg_term_dx[MDE][DIM]; /* deriv. of supg_term w.r.t. mesh coords. */
  dbl d_vrel_d_x_rs[DIM][DIM]
  ...
  if (pd->e[pg->imtrx][R_LUBP_2]) {
    ...
    EH(-1, " if you have a fill equation turned on in the R_LUBP_2 phase, you "
  ...
  if (pd->TimeIntegration != STEADY && pd->gv[MESH_DISPLACEMENT1]) {
    ...
    for (a = 0; a < VIM; a++) {
      x_dot[a] =
          (1. + 2. * tt) * (xx[a] - x_old[a]) * dtinv - 2. * tt * x_dot_old[a];
      if (lubon)
  ...
  if (Fill_Weight_Fcn == FILL_WEIGHT_SUPG) {
    ...
    supg_term = 0.;
    get_supg_stuff(&supg_term, vcent, d_vcent_du, d_supg_term_du,
                   d_supg_term_dx, pd->e[pg->imtrx][R_MESH1]);
  }
    ...
    for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
      ...
      if (pd->v[pg->imtrx][var]) {
        ...
        for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
          ...
          case FILL_WEIGHT_TG: /* Taylor-Galerkin */
            ...
            for (a = 0; a < VIM; a++) {
              advection +=
                  0.5 * (v_rel[a] + v_rel_old[a]) * grad_phi_j[a] * phi_i;
            }
          ...
          case FILL_WEIGHT_SUPG: /* Streamline Upwind Petrov Galerkin (SUPG) */
            ...
            if (lubon) {
              for (a = 0; a < dim; a++)
                advection += LubAux->dv_avg_df[a][j] * grad_F[a] * wfcn;
            }
          ...
          if (lubon) {
            ...
            for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
              ...
              for (a = 0; a < dim; a++) {
                ...
                advection -=
                    (1 + 2 * tt) / dt * phi_j * delta(a, b) * grad_F[a] * phi_i;
                advection += v_rel[a] * fv->d_grad_F_dmesh[a][b][j] * phi_i;
              }
              advection *= pd->etm[pg->imtrx][eqn][(LOG2_ADVECTION)];

              lec->J[peqn][pvar][i][c] +=
                  (mass + advection) * wt * h3 * fv->sdet;
              lec->J[peqn][pvar][i][c] +=
                  rmp[i] * wt * h3 * fv->dsurfdet_dx[b][c];
            } /* j loop */
            ...
            for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
              ...
              case FILL_WEIGHT_TG: /* Taylor Galerkin */
                ...
                mass = 0.;
                advection =
                    -0.5 * phi_j * (1. + 2. * tt) * dtinv * phi_i * grad_F[b];
              ...
              /* lec->J[peqn][pvar][i][j] += rmp[i] * wt * det_J * fv->dh3dq[b]
               * * bf[var]->phi[j]; */
              lec->J[peqn][pvar][i][j] +=
                  rmp[i] * wt * det_J * fv->dh3dq[b] * phi_j;
              lec->J[peqn][pvar][i][j] +=
                  rmp[i] * wt * h3 * bf[eqn]->d_det_J_dm[b][j];
      ...
      for (a = 0; a < VIM; a++) {
        for (b = 0; b < VIM; b++) {
          ...
          for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
            ...
            d_vrel_d_x_rs[a][b][j] +=
                phi_j * (1. + 2. * tt) * dtinv * delta(a, b);

            grad_phi_j = bf[var]->grad_phi[j];

            d_vrel_d_x_rs[a][b][j] -=
                (1. + 2. * tt) * phi_j * dtinv * fv->grad_d_rs[b][a];

            for (c = 0; c < VIM; c++) {
              /*grad_phi_e[i][j][k][l] = e_k e_l : grad(phi_i e_j )*/
              d_vrel_d_x_rs[a][b][j] -=
                  x_dot[c] * bf[var]->grad_phi_e[j][b][c][a];
            }
          }
        }
      }
        ...
        if (pd->v[pg->imtrx][var]) {
          ...
          for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
            ...
            case FILL_WEIGHT_TG: /* Taylor Galerkin */
              ...
              for (a = 0; a < VIM; a++) {
                advection +=
                    (v_dot_Dphi[i] * d_vrel_d_x_rs[a][b][j] * grad_F[a] +
            ...
            case FILL_WEIGHT_G: /* Plain ol' Galerkin */
              ...
              for (a = 0; a < VIM; a++) {
                EH(-1,
                   "This is easy.  Need to copy essentials from TG case above");
              }
            ...
            case FILL_WEIGHT_SUPG: /* Streamline Upwind Petrov Galerkin (SUPG)
              ...
              EH(-1, "Level set fill for Eulerian solid mech incompatible for "
                     "FILL_WEIGHT_SUPG");
      ...
      if (pd->v[pg->imtrx][var] && lubon) {
        ...
        for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
          ...
          ShellBF(var, j, &phi_j, grad_phi_j, grad_II_phi_j,
                  d_grad_II_phi_j_dmesh, n_dof[MESH_DISPLACEMENT1], dof_map);

          mass = 0.0;
          advection = 0.0;
          for (a = 0; a < dim; a++) {
            advection +=
                LubAux->dv_avg_dp1[a][j] * grad_F[a] * wfcn * grad_II_phi_j[a];
...
int assemble_fill_ext_v(double tt, double dt, dbl hsquared[DIM],
                        dbl hh[DIM][DIM], dbl dh_dxnode[DIM][MDE]) {
  /******************************************************************************
   *
  ...
  tau_gls = 1. / sqrt((2. / dt) * (2. / dt) +
                      (2. * ext_v_mag / h_elem) * (2. * ext_v_mag / h_elem));
  for (i = 0; i < dofs; i++) {
    d_tau_ext_v[i] = -4. * tau_gls * tau_gls * tau_gls / (h_elem * h_elem) *
                     *esp->ext_v[i] / dofs;
  }
  ...
  if (Fill_Weight_Fcn == FILL_WEIGHT_SC || Fill_Weight_Fcn == FILL_WEIGHT_GLS) {
    ...
    for (i = 0; i < ei[pg->imtrx]->dof[var]; i++) {
      for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
        ...
        for (a = 0; a < dim; a++)
          d_n_dot_dF_dF[i][j] +=
              bf[var]->grad_phi[i][a] * lsi->d_normal_dF[a][j];
      }
    }
  }
      ...
      if (pd->v[pg->imtrx][var]) {
        ...
        for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
          ...
          case FILL_WEIGHT_GLS: /* Galerkin Least Squares*/
            ...
            advection =
                wfcn * (v_dot_n * lsi->d_gfmag_dF[j]) + d_wfcn_dF * (v_dot_DF);
            source = 0.;
          ...
          case FILL_WEIGHT_TG: /* Taylor-Galerkin */
            ...
            advection =
                phi_i * 0.5 * (v_dot_n + v_dot_n_old) * lsi->d_gfmag_dF[j];
            advection +=
                dt * 0.5 *
                (v_dot_Dphi[i] * v_dot_n * lsi->d_gfmag_dF[j] + v_dot_DF * tmp);
            source = 0.;
            break;
#if 0
                    case FILL_WEIGHT_EXPLICIT:
          ...
          lec->J[peqn][pvar][i][j] +=
              wt * h3 * det_J * (mass + advection + source);
      ...
      if (pd->v[pg->imtrx][var]) {
        ...
        for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
          ...
          case FILL_WEIGHT_TG: /* Taylor-Galerkin */
            ...
            advection += dt * 0.5 *
                         (v_dot_Dphi[i] * lsi->gfmag + tmp * v_dot_DF) * phi_j;
            source = 0.;
          ...
          lec->J[peqn][pvar][i][j] +=
              wt * det_J * h3 * (mass + advection + source);
        }
      }
...
int assemble_fill_gradf(double tt, double dt, dbl hsquared[DIM],
                        dbl hh[DIM][DIM], dbl dh_dxnode[DIM][MDE]) {
  /******************************************************************************
   *
    ...
    for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
      ...
      if (pd->v[pg->imtrx][var]) {
        ...
        for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
          ...
          switch (Fill_Weight_Fcn) {
          case FILL_WEIGHT_GLS: /* Galerkin Least Squares*/
            tmp = wfcn * lsi->d_gfmag_dF[j] + (d_visc_dF[j] * gradF_gradphi[i] +
                                               visc * gradphi_gradphi[i][j]);
...
int assemble_fill(double afill[], /* Jacobian matrix for fill equation  */
                  int ijaf[],     /* pointer to nonzeros in Jacobian matrix   */
                  double rf[],    /* rhs vector   */
                  double dt,      /* current time step size */
                  double tt,      /* parameter to vary time integration from
                                   * explicit (tt = 1) to implicit (tt = 0) */
                  int node_to_fill[], double xi[DIM], Exo_DB *exo,
                  double time) {
  ...
  for (a = 0; a < dim; a++) {
    ...
    if (((pd->TimeIntegration) != STEADY) &&
      ...
      x_dot[a] =
          (1. + 2. * tt) * (xx[a] - x_old[a]) / dt - 2. * tt * x_dot_old[a];
      v_rel[a] = v[a] - x_dot[a];
      v_rel_old[a] = v_old[a] - x_dot_old[a];
    } else {
      x_dot[a] = 0.0;
    ...
    for (i = 0; i < ei[pg->imtrx]->num_local_nodes; i++) {
      ...
      for (ki = 0; ki < nvdofi; ki++) {
        ...
        if (node->DBC[pg->imtrx] &&
            (node->DBC[pg->imtrx][offseti + ki] != -1) && Debug_Flag >= 0) {
          ...
          if (tran->Fill_Weight_Fcn == FILL_WEIGHT_TG) {
            rhs = F_dot *
                  phi_i; /* try implementing an explicit/implicit method */
...

          {
            ...
            if (node->DBC[pg->imtrx] &&
              ...
              if (tran->Fill_Weight_Fcn == FILL_WEIGHT_TG) {
                ...
                afill[ja] +=
                    wt * h3 * det_J * phi_i * phi_j * (1. + 2. * tt) / dt;
                for (a = 0; a < dim; a++) {
                  afill[ja] += wt * h3 * det_J * phi_i *
                               (v_rel[a] + v_rel_old[a]) / 2. *
                               bf[eqn]->grad_phi[jdof][a]; /*    + */
                  /* wt * h3 * det_J * bf[eqn]->grad_phi[idof] [a]  */
                  /* * v_rel[a] * v_rel_old[a] * bf[eqn]->grad_phi[jdof] [a]
                   * *dt/2. */
                ...
                afill[ja] +=
                    wt * h3 * det_J * v_dot_Dphi_j * v_dot_Dphi_i * dt / 2.0;
              } else if (tran->Fill_Weight_Fcn == FILL_WEIGHT_G) {
                /* The "mass" term */
                afill[ja] +=
                    wt * h3 * det_J * wfcn * phi_j * (1. + 2. * tt) / dt;

                /* The advection term */
                for (a = 0; a < dim; a++) {
                  afill[ja] += wt * h3 * det_J * wfcn * v_rel[a] *
                               bf[eqn]->grad_phi[jdof][a];
                }
                ...
                /* The advection term */
                for (a = 0; a < dim; a++) {
                  afill[ja] += wt * h3 * det_J * wfcn * v_rel_old[a] *
                               bf[eqn]->grad_phi[jdof][a];
                }
...
int assemble_fill_ext_v(
    ...
    double tt,      /* parameter to vary time integration from
                     * explicit (tt = 1) to implicit (tt = 0) */
    int node_to_fill[], double hsquared[DIM]) /* element size */
{
  dbl wfcn; /* Holds the final weight function (G, TG, or SUPG) */
  ...
  for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
    ...
    grad_phi_i = bf[eqn]->grad_phi[i];
    for (a = 0; a < VIM; a++)
      gradF_dot_Dphi[i] += grad_phi_i[a] * grad_F[a];
  }
      ...
      for (ki = 0; ki < nvdofi; ki++) {
        ...
        if (node->DBC[pg->imtrx] &&
          ...
          if (tran->Fill_Weight_Fcn == FILL_WEIGHT_G) {
            ...
            /* wfcn = w + p = phi + tau_gls * (ext_v*gradF/grad_F_mag "dot"
             * grad(phi) ) */

            grad_phi_i = bf[eqn]->grad_phi[idof];

            rhs = (F_dot + v_dot_DF) *
                  (phi_i + tau_gls * ext_v / lsi->gfmag * gradF_dot_Dphi[i]);
          }
        ...
        for (j = 0; j < ei[pg->imtrx]->num_local_nodes; j++) {
...

          {
            ...
            if (node->DBC[pg->imtrx] &&
              ...
              if (tran->Fill_Weight_Fcn == FILL_WEIGHT_GLS) {
                d_wfcn_dF = -tau_gls * ext_v * gradF_dot_Dphi[i] *
                            lsi->d_gfmag_dF[j] * (gfmag_inv * gfmag_inv);
                for (a = 0; a < VIM; a++) {
                  d_wfcn_dF += tau_gls * ext_v * gfmag_inv * grad_phi_i[a] *
                               grad_phi_j[a];
                }
...
int integrate_explicit_eqn(
    ...
    dbl *time2, int eqntype, int node_to_fill[], /* this is a map from the
                                                  * node number to the
...
*******************************************************************************/
{
  ...
  canine_chaos(local_order, local_order_plus, local_nnz, local_nnz_plus,
               &global_order, &global_order_plus, &global_nnz,
               &global_nnz_plus);
  ...
  while ((!converged) && (inewton < Max_Steps)) {
    ...
    if (Debug_Flag == -4) {
      ...
      numerical_jacobian_fill(ijaf, afill, xf, rf, delta_t, theta, x, x_old,
                              xdot, Debug_Flag, node_to_fill, exo, dpi);
      if (printing)
       ...
       */
      a_start = ut();
      err = fill_matrix(afill, ijaf, rf, xf, x, x_old, xdot, delta_t, theta,
                        eqntype, node_to_fill, First_Elem_Side_BC_Array, exo,
                        dpi);
      a_end = ut();
      EH(err, "fill_matrix");
    ...
    case UMFPACK2:
      ...
      /*  */
      matr_form = 1;
      UMF_system_id = SL_UMF(UMF_system_id, &first_linear_solver_call,
                             &Factor_Flag, &matr_form, &num_fill_unknowns,
                             &local_nnz, ijaf, ijaf, afill, rf, delta_x);
      /*  */
    ...
    case AMESOS:    /* for the moment we shall default the FILL solve to LU when
                       using AMESOS */
      dcopy1(num_fill_unknowns, rf, delta_x);
      if (inewton == 0) {
        luf(num_fill_unknowns, 0, fill_zeros, afill, ijaf, delta_x,
            (first_linear_solver_call ? 1 : 2));
        first_linear_solver_call = FALSE;
      } else {
        luf(num_fill_unknowns, 0, fill_zeros, afill, ijaf, delta_x, 3);
        first_linear_solver_call = FALSE;
      }
    ...
    case AZTEC:
      ...
      while ((!matrix_solved) && (linear_solver_blk < num_linear_solve_blks)) {
         ...
         */
        AZ_solve(delta_x, rf, ams->options, ams->params, ams->indx, ams->bindx,
                 ams->rpntr, ams->cpntr, ams->bpntr, ams->val, ams->data_org,
                 ams->status, ams->proc_config);
    ...
    case MA28:
       ...
       */
#ifdef HARWELL
      err = cmsr_ma28(num_fill_unknowns, local_nnz, afill, ijaf, delta_x, rf);
#endif /* HARWELL */
#ifndef HARWELL
...
int fill_matrix(double afill[], /* matrix for fill variables only      */
...
******************************************************************************/
{
  ...
  static int inlet_bc_list[MAX_INLET_BC]; /* list of FILL_INLET conditions BCs
                                             indexs */
  static int bc_list_built =
      0; /* flag to indicate the inlet_bc_list has been CONSTRUCTED */
  ...
  ei[pg->imtrx]->elem_blk_id =
      -1234; /* Hardwired to ridiculous, for now. -pas */
  ...
  for (ebi = 0; ebi < exo->num_elem_blocks; ebi++) {
    ...
    if (pd->i[pg->imtrx][FILL] == I_P1) {
      if (dim == 2)
        ielem_type_fill = P1_QUAD;
      if (dim == 3)
        ielem_type_fill = P1_HEX;
      discontinuous = 1;
    } else if (pd->i[pg->imtrx][FILL] == I_P0) {
      if (dim == 2)
        ielem_type_fill = P0_QUAD;
      if (dim == 3)
        ielem_type_fill = P0_HEX;
      discontinuous = 1;
    } else if (pd->i[pg->imtrx][FILL] == I_PQ1) {
      if (dim == 2)
        ielem_type_fill = C_BILINEAR_QUAD;
      if (dim == 3)
        EH(-1, "Sorry PQ1 interpolation has not been implemented in 3D yet.");
      discontinuous = 1;
    } else if (pd->i[pg->imtrx][FILL] == I_PQ2) {
      if (dim == 2)
        ielem_type_fill = BIQUAD_QUAD;
      if (dim == 3)
        EH(-1, "Sorry PQ2 interpolation has not been implemented in 3D yet.");
      discontinuous = 1;
    } else {
    ...
    if (discontinuous && !bc_list_built) {
      ...
      for (bc_input_id = 0; bc_input_id < Num_BC; bc_input_id++) {
        ...
        if (k == MAX_INLET_BC) {
          EH(-1, "Maximum FILL_INLET BCs exceeded.  Adjust MAX_INLET_BC in "
                 "mm_fill_fill.c");
        }
      }

      inlet_bc_list[k] =
          -8458912; /* My office phone number marks the end of the list.  TAB */
      bc_list_built = 1;
    }
    ...
    for (iel = e_start; iel < e_end; iel++) {
      ...
      ip_total =
          elem_info(NQUAD, ielem_type); /* number of  quadrature points */

      num_local_nodes =
          ei[pg->imtrx]->num_local_nodes; /* number of local  basis functions */

      ielem_dim =
          ei[pg->imtrx]->ielem_dim; /* physical dimension  of this element */

      iconnect_ptr =
          ei[pg->imtrx]->iconnect_ptr; /* find pointer to beginning */
      /* of this element's */
      /* connectivity list */

      if (discontinuous) {
        for (k = 0; (fill_inlet_bc = inlet_bc_list[k]) != -8458912; k++) {
          if ((fill_inlet_side =
                   elem_on_ss(exo, BC_Types[fill_inlet_bc].BC_ID, ielem)))
            break;
        }
      }
      ...
      for (ip = 0; ip < ip_total; ip++) {
        ...
        /* 		  if (eqntype == CORRECT )  find_nodal_stu (ip,
         * ielem_type, &s, &t, &u); */
        ...
        fv->wt = wt =
            Gq_weight(ip, ielem_type); /* find quadrature weights for */
        /* current ip */
        ...
        case ADVECT:
           ...
           */
          if (pd->e[pg->imtrx][R_EXT_VELOCITY]) {
            err = assemble_fill_ext_v(afill, ijaf, rf, delta_t, theta,
                                      node_to_fill, hsquared);
            EH(err, "assemble_fill_ext_v");
          } else {
            err = assemble_fill(afill, ijaf, rf, delta_t, theta, node_to_fill);
          }
          ...
          break;
        case CORRECT:
          err = assemble_level_correct(afill, ijaf, rf, delta_t, theta,
                                       node_to_fill);
          EH(err, "assemble_level_correct");
          break;
        case PROJECT:
          err = assemble_level_project(afill, ijaf, rf, delta_t, theta,
                                       node_to_fill);
      ...
      if (discontinuous) {
        ...
        for (face = 0; face < ei[pg->imtrx]->num_sides; face++) {
          ...
          id_side =
              face + 1; /* the exo/patran side number starts with 1 not zero */
          ...
          err = assemble_surface(exo, x, afill, ijaf, rf, delta_t, theta,
                                 node_to_fill, ielem_type, ielem_type_fill,
                                 id_side, F_inlet, neighbor, ielem,
                                 num_local_nodes);
      ...
      if ((eqntype == CORRECT) || (eqntype == PROJECT)) {
        ...
        struct elem_side_bc_struct *elem_side_bc =
            first_elem_side_BC_array[iel];

        if (elem_side_bc != NULL) {
          do {
            int ibc = 0, bc_input_id;

            while ((bc_input_id = (int)elem_side_bc->BC_input_id[ibc++]) !=
                   -1) {
              if (BC_Types[bc_input_id].BC_Name == STRONG_FILL_CA_BC) {
                err = apply_strong_fill_ca_bc(
                    afill, ijaf, x, rf, delta_t, theta, node_to_fill, ielem,
                    ielem_type, num_local_nodes, ielem_dim, iconnect_ptr,
                    elem_side_bc, num_total_nodes,
...
/***************************************************************************************/
/***************************************************************************************/
int assemble_surface(
    Exo_DB *exo,    /* ptr to basic exodus ii mesh information */
    ...
    int node_to_fill[], int ielem_type, /* element type  */
    ...
    int num_local_nodes) /* number of nodes per element */
{
  ...
  for (ip = 0; ip < ip_total_fill; ip++) {
    ...
    surface_determinant_and_normal(ielem, ei[pg->imtrx]->iconnect_ptr,
                                   num_local_nodes,
                                   ei[pg->imtrx]->ielem_dim - 1, id_side,
    ...
    if (vdotn_avg < 0.) {
      ...
      if (neighbor != -1 && vdotn_avg * vdotn_avg / vdotn_norm > 1.e-12) {
        ...
        err =
            neighbor_fill(exo, x, ielem, neighbor, F_neighbor, num_local_nodes,
                          nodes_per_side, local_elem_node_id, ielem_type,
  ...
  if (vdotn_avg < 0.) {
    for (ip = 0; ip < ip_total; ip++) {
      ...
      err = get_side_info(ielem_type, id_side, &nodes_per_side,
                          local_elem_node_id);
      EH(err, "get_side_info");
            ...
            for (ki = 0; ki < nvdofi; ki++) {
              ...
              for (j = 0; j < ei[pg->imtrx]->num_local_nodes; j++) {
                ...
                for (kj = 0; kj < nvdofj; kj++) {
                  ...
                  je = node_to_fill[J] + kj;
                  ja = (ie == je) ? ie
                                  : in_list(je, ijaf[ie], ijaf[ie + 1], ijaf);
                  EH(ja, "Could not find vbl in sparse matrix.");
...
static int
neighbor_fill(Exo_DB *exo, dbl x[], int current_elem, int neighbor_elem,
              dbl F_neighbor[], int num_local_nodes, int nodes_per_side,
              int local_elem_node_id[], int ielem_type, int ielem_type_fill,
              int node_to_fill[], dbl x_n[][DIM])
/*
...

{
  ...
  id_side = find_id_side(neighbor_elem, nodes_per_side, inode,
  ...
  for (ip = 0; ip < ip_total; ip++) {
    ...
    for (i = 0; i < ei[pg->imtrx]->dof[v]; i++) {
      phi[i] = newshape(xi, ielem_type, PSI, ei[pg->imtrx]->dof_list[v][i],
                        ielem_shape, pd->i[pg->imtrx][v], i);
    }

    iconnect_ptr =
        Proc_Connect_Ptr[neighbor_elem]; /* find pointer to beginning */

    ktype = 0;
    iconnect_ptr =
        Proc_Connect_Ptr[neighbor_elem]; /* find pointer to beginning */

    v = pd->ShapeVar; /* ShapeVar is the variable index where we find the
                         element mapping interpolation */

    for (i = 0; i < ei[pg->imtrx]->dof[v]; i++) {
      phi_map[i] = newshape(xi, ielem_type, PSI, ei[pg->imtrx]->dof_list[v][i],
                            ielem_shape, pd->i[pg->imtrx][v], i);
...
int get_side_info(const int ielem_type,
  ...
  /* TAB certifies that this routine is consistent with the exo/patran side/node
   * number conventions.  11/9/98 */
...
int assemble_surface_species(
    Exo_DB *exo, /* ptr to basic exodus ii mesh information */
    Dpi *dpi, double x[], double delta_t, /* current time step size */
    double theta,        /* parameter to vary time integration from
                          * explicit (theta = 1) to implicit (theta = 0) */
    ...
    if (vdotn_avg < 0.) {
      ...
      if (neighbor != -1) {
        err = neighbor_species(exo, dpi, x, ielem, neighbor, conc_neighbor,
                               ...
                               local_elem_node_id, ielem_type, ielem_type_fill,
                               id_side, x_neighbor, dg_neighbor_data);
          ...
          for (w = 0; w < pd->Num_Species_Eqn; w++) {
            ...
            for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
              ...
              for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
                ...
                lec->J[MAX_PROB_VAR + w][MAX_PROB_VAR + w][i][j] += advection;
              }
              ...
              for (b = 0; b < dim; b++) {
                ...
                if (pd->v[pg->imtrx][var]) {
                  ...
                  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
                    ...
                    advection = phi_i * wt * fv->sdet * phi_j * fv->snormal[b] *
                                (fv->c[w] - c_n[w]);

                    lec->J[MAX_PROB_VAR + w][pvar][i][j] += advection;
                  }
                }
              }
                ...
                if (pd->v[pg->imtrx][var]) {
                  ...
                  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
                    ...
                    for (p = 0; p < pd->Num_Dim; p++) {
                      advection +=
                          fv->sdet * fv->v[p] * fv->dsnormal_dx[p][b][j];
                    }
...
neighbor_species(Exo_DB *exo, /* ptr to basic exodus ii mesh information */
                 Dpi *dpi, dbl x[], int current_elem, int neighbor_elem,
                 dbl conc_neighbor[][MAX_CONC], int num_local_nodes,
                 int nodes_per_side, int local_elem_node_id[], int ielem_type,
                 int ielem_type_fill, int current_id_side, dbl **x_n,
                 dg_neighbor_type *dg_neighbor_data)
/*
*   This function take the current element and side and
 *   knowing who the neighboring element is, finds the value
...

{
  ...
  for (ip = 0; ip < ip_total; ip++) {
    ...
    for (i = 0; i < ei[pd->mi[v]]->dof[v]; i++) {
      phi[i] = newshape(xi, ielem_type, PSI, ei[pd->mi[v]]->dof_list[v][i],
    ...
    for (i = 0; i < ei[pd->mi[v]]->dof[v]; i++) {
      phi_map[i] = newshape(xi, ielem_type, PSI, ei[pd->mi[v]]->dof_list[v][i],
                            ielem_shape, pd->i[pd->mi[v]][v], i);
    }
    ...
    if (neighbor_elem == -2) { // flag for parallel neighbor
      ...
      for (int i = 0;
           i < dg_neighbor_data->neighbor_recv_elem_index[dpi->num_neighbors];
           i++) {
        if (dg_neighbor_data->global_recv_elem_list[i] ==
            global_neighbor_elem) {
      ...
      for (i = 0; i < ei[pd->mi[v]]->dof[v]; i++) {
        for (p = 0; p < dim; p++) {
          double coord_val =
              dg_neighbor_data
                  ->nodal_recv_coord[p][global_neighbor_index * nodes_per_elem +
                                        i];
      ...
      int num_dof_species =
          dg_neighbor_data->mass_fraction_num_dof / pd->Num_Species;
      int offset =
          global_neighbor_index * dg_neighbor_data->mass_fraction_num_dof;

      for (int w = 0; w < pd->Num_Species; w++) {
        for (int j = 0; j < num_dof_species; j++) {
          int ie = offset + w * num_dof_species + j;
          conc_neighbor[ip][w] +=
              dg_neighbor_data->nodal_recv_mass_fraction_values[ie] * phi[j];
        }
      }
    } else {
      iconnect_ptr =
...
int elem_on_ss(Exo_DB *exo, int ss_id, int elem) {
  ...
  if ((j = in_list(elem, 0, exo->ss_num_sides[iss],
                   &(exo->ss_elem_list[elem_index]))) == -1) {
    return 0;
  } else {
...
int curvature_momentum_source(double f[DIM], double dfdX[DIM][DIM][MDE],
                              double dfdF[DIM][MDE], double dfdH[DIM][MDE]) {
...
#define CONST_CURVE 0
  ...
  for (b = 0; b < dim; b++) {
    ...
    if (pd->v[pg->imtrx][var]) {
      ...
      for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
        for (a = 0; a < dim; a++) {
          dfdX[a][b][j] += -mp->surface_tension * fv->H * lsi->delta *
  ...
  if (pd->v[pg->imtrx][var]) {
    for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
      for (a = 0; a < dim; a++) {
        dfdH[a][j] = -mp->surface_tension * bf[var]->phi[j] * lsi->delta *
                     lsi->normal[a];
      }
    }
  }
 ...
 *  For convenience the phase function kernels are located in this file.  They
 * should be moved elsewhere as the capability warrants it.
 */

int assemble_phase_function(double time_value, double tt, double dt,
                            double xi[DIM], Exo_DB *exo) {
  int i, j;
  int a, b, p;
  ...
  /* See get_supg_stuff() for a better description of these variables */
  dbl supg_term = 0, vcent[DIM], d_vcent_du[DIM][MDE][DIM],
      d_supg_term_du[MDE][DIM], d_supg_term_dx[MDE][DIM];
      ...
      for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
        ...
        if (ei[pg->imtrx]->active_interp_ledof[ledof]) {
          ...
          case FILL_WEIGHT_SUPG:
            ...
            rhs =
                (pf_dot + v_dot_grad_pf) * (vc_dot_Dphi[i] * supg_term + phi_i);

            break;
          case FILL_WEIGHT_EXPLICIT: /* Vanilla Galerkin weight on advection
                                        equation */
            wfcn = phi_i;
            for (p = 0; p < dim; p++)
          ...
          default:
            EH(-1, "That Fill Weight Function type not currently supported for "
                   "phase function\n");
            break;
          }
        ...
        if (ei[pg->imtrx]->active_interp_ledof[ledof]) {
          ...
          if (pd->v[pg->imtrx][var]) {
            ...
            for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
              ...
              case FILL_WEIGHT_TG:
                ...
                for (p = 0; p < VIM; p++) {
                  tmp +=
                      0.5 * (v_rel[p] + v_rel_old[p]) * grad_phi_j[p] * phi_i;
                }
              ...
              case FILL_WEIGHT_G: /* Vanilla Galerkin weight on advection
                                     equation */
              ...
              case FILL_WEIGHT_SUPG:
                ...
                tmp = (phi_j * (1 + 2.0 * tt) / dt + v_dot_Dphi[j]) * wt * h3 *
                      det_J * wfcn;
              ...
              default:
                EH(-1, "That Fill Weight Function type not currently supported "
                       "for phase function\n");
                break;
              }
            ...
            if (pd->v[pg->imtrx][var]) {
              ...
              for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
                ...
                case FILL_WEIGHT_SUPG:
                  ...
                  for (p = 0; p < dim; p++) {
                    d_wfcn_du +=
                        supg_term * d_vcent_du[p][j][b] * grad_phi_i[p];
                  }
          ...
          if (pd->v[pg->imtrx][var] && lubon) {
            ...
            for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
              ...
              dbl grad_II_phi_j[DIM], d_grad_II_phi_j_dmesh[DIM][DIM][MDE];
              ShellBF(var, j, &phi_j, grad_phi_j, grad_II_phi_j,
                      d_grad_II_phi_j_dmesh, n_dof[MESH_DISPLACEMENT1],
                      dof_map);

              mass = 0.0;
              advection = 0.0;
              for (p = 0; p < dim; p++) {
                advection += LubAux->dv_avg_dp1[p][j] * grad_pf[p] * wfcn *
                             grad_II_phi_j[p];
                advection +=
                    LubAux->dv_avg_dp2[p][j] * grad_pf[p] * wfcn * phi_j;
              }
          ...
          if (pd->v[pg->imtrx][var] && lubon) {
            ...
            for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
              ...
              for (a = 0; a < dim; a++)
                advection +=
                    LubAux->dv_avg_dk[a][j] * grad_pf[a] * phi_i * phi_j;
...
int assemble_pf_constraint(double delta_t, double *pf_constraint, double lambda,
                           double *ptr_d_pf_lm, double *ptr_d_lm_pf) {
  int i, j, ledof;
  int var, pvar, ie;
  double wt, det_J, h3;
  ...
  for (i = 0, sum_old = sum = 0.0; i < pfd->num_phase_funcs; i++) {
    ...
    if (lsi->near) {
      pf_lsi_old[i].dH = 0.5 * (1.0 / lsi->alpha) *
                         (1. + cos(M_PIE * fv_old->pF[i] / lsi->alpha));
      sum_old += 0.5 * (1. + fv_old->pF[i] / lsi->alpha +
                        sin(M_PIE * fv_old->pF[i] / lsi->alpha) / M_PIE);
    } else {
  ...
  for (i = 0; i < pfd->num_phase_funcs; i++) {
    ...
    if (pd->v[pg->imtrx][var]) {
      for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
        ...
        d_pf_lm[i][j] =
            wt * phi_j * pf_lsi_old[i].dH * sum_old * h3 * det_J / delta_t;
