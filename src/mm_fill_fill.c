            ...
            for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
              ...
              case FILL_WEIGHT_G: /* Plain ol' Galerkin */
                ...
                for (a = 0; a < dim; a++) {
                  ...
                  //phi_i; 			    advection -= (1+2*tt)/dt * phi_j * delta(a,b) * grad_F[a] * phi_i;
                  //			    advection += v_rel[a] * fv->d_grad_F_dmesh[a][b][j] *
                  //phi_i;
  ...
  if (af->Assemble_Residual) {
    for (i = 0; i < ei[pg->imtrx]->num_local_nodes; i++) {
      ...
      for (ki = 0; ki < nvdofi; ki++) {
        ...
        if (node->DBC[pg->imtrx] && (node->DBC[pg->imtrx][offseti + ki] != -1) && Debug_Flag >= 0) {
      ...
      for (ki = 0; ki < nvdofi; ki++) {
        ...
        for (j = 0; j < ei[pg->imtrx]->num_local_nodes; j++) {
...

          {
            ...
            if (node->DBC[pg->imtrx] && ((int)node->DBC[pg->imtrx][offseti + ki] != -1) &&
                Debug_Flag >= 0) {
              afill[ja] = delta(i, j);
            } else {
              ...
              if (tran->Fill_Weight_Fcn == FILL_WEIGHT_TG) {
                ...
                for (a = 0; a < dim; a++) {
                  afill[ja] += wt * h3 * det_J * phi_i * (v_rel[a] + v_rel_old[a]) / 2. *
...
int assemble_fill_ext_v(double afill[], /* Jacobian matrix for fill equation  */
                        int ijaf[],     /* pointer to nonzeros in Jacobian matrix   */
                        double rf[],    /* rhs vector   */
                        double dt,      /* current time step size */
                        double tt,      /* parameter to vary time integration from
      ...
      for (ki = 0; ki < nvdofi; ki++) {
        ...
        if (node->DBC[pg->imtrx] && (node->DBC[pg->imtrx][offseti + ki] != -1) && Debug_Flag >= 0) {
          ...
          if (tran->Fill_Weight_Fcn == FILL_WEIGHT_G) {
            ...
            rhs = (F_dot + v_dot_DF) * (phi_i + tau_gls * ext_v / lsi->gfmag * gradF_dot_Dphi[i]);
          }
        ...
        for (j = 0; j < ei[pg->imtrx]->num_local_nodes; j++) {
...

          {
            ...
            if (node->DBC[pg->imtrx] && ((int)node->DBC[pg->imtrx][offseti + ki] != -1) &&
              ...
              tmp = wfcn * (phi_j * (1. + 2. * tt) * dtinv + v_rel_n * lsi->d_gfmag_dF[j]);
...
int assemble_fill_gradf(double afill[], /* Jacobian matrix for fill equation  */
                        int ijaf[],     /* pointer to nonzeros in Jacobian matrix   */
                        double rf[],    /* rhs vector   */
                        double dt,      /* current time step size */
                        double tt,      /* parameter to vary time integration from
...
/******************************************************************************/
/******************************************************************************/
int integrate_explicit_eqn(struct Aztec_Linear_Solver_System *ams, /* cf "sl_util_structs.h" */
...
*******************************************************************************/
{
  ...
  for (i = 0; i < num_fill_unknowns; i++) {
    xfdot[i] = (1.0 + 2.0 * theta) / delta_t * (xf[i] - xf_old[i]) - 2.0 * theta * xfdot_old[i];
  }
  ...
  while ((!converged) && (inewton < Max_Steps)) {
    ...
    if (Debug_Flag == -4) {
      ...
      numerical_jacobian_fill(ijaf, afill, xf, rf, delta_t, theta, x, x_old, xdot, Debug_Flag,
       ...
       */
      a_start = ut();
      err = fill_matrix(afill, ijaf, rf, xf, x, x_old, xdot, delta_t, theta, eqntype, node_to_fill,
    ...
    hide_external(num_fill_unknowns, owned_fill_unknowns, ijaf, ijaf_save, afill);
    ...
    case UMFPACK2:
      ...
      UMF_system_id = SL_UMF(UMF_system_id, &first_linear_solver_call, &Factor_Flag, &matr_form,
                             &num_fill_unknowns, &local_nnz, ijaf, ijaf, afill, rf, delta_x);
      /*  */
    ...
    case AZTEC:
      ...
      while ((!matrix_solved) && (linear_solver_blk < num_linear_solve_blks)) {
         ...
         */
        AZ_solve(delta_x, rf, ams->options, ams->params, ams->indx, ams->bindx, ams->rpntr,
                 ams->cpntr, ams->bpntr, ams->val, ams->data_org, ams->status, ams->proc_config);
    ...
    show_external(num_fill_unknowns, owned_fill_unknowns, ijaf, ijaf_save, afill);
  ...
  for (ebi = 0; ebi < exo->num_elem_blocks; ebi++) {
    ...
    if (discontinuous && !bc_list_built) {
      ...
      for (bc_input_id = 0; bc_input_id < Num_BC; bc_input_id++) {
        ...
        if (k == MAX_INLET_BC) {
          GOMA_EH(GOMA_ERROR,
                  "Maximum FILL_INLET BCs exceeded.  Adjust MAX_INLET_BC in mm_fill_fill.c");
        }
      }
    ...
    for (iel = e_start; iel < e_end; iel++) {
      ...
      if (discontinuous) {
        for (k = 0; (fill_inlet_bc = inlet_bc_list[k]) != -8458912; k++) {
          if ((fill_inlet_side = elem_on_ss(exo, BC_Types[fill_inlet_bc].BC_ID, ielem)))
            break;
        }
      }
      ...
      for (ip = 0; ip < ip_total; ip++) {
        ...
        case ADVECT:
          ...
          if (pd->e[pg->imtrx][R_EXT_VELOCITY]) {
            err = assemble_fill_ext_v(afill, ijaf, rf, delta_t, theta, node_to_fill, hsquared);
          ...
          break;
        case CORRECT:
          err = assemble_level_correct(afill, ijaf, rf, delta_t, theta, node_to_fill);
          GOMA_EH(err, "assemble_level_correct");
          break;
        case PROJECT:
          err = assemble_level_project(afill, ijaf, rf, delta_t, theta, node_to_fill);
      ...
      if (discontinuous) {
        ...
        for (face = 0; face < ei[pg->imtrx]->num_sides; face++) {
          ...
          err =
              assemble_surface(exo, x, afill, ijaf, rf, delta_t, theta, node_to_fill, ielem_type,
                               ielem_type_fill, id_side, F_inlet, neighbor, ielem, num_local_nodes);
          ...
          do {
            ...
            while ((bc_input_id = (int)elem_side_bc->BC_input_id[ibc++]) != -1) {
              if (BC_Types[bc_input_id].BC_Name == STRONG_FILL_CA_BC) {
                err = apply_strong_fill_ca_bc(afill, ijaf, x, rf, delta_t, theta, node_to_fill,
          ...
          do {
            ...
            while ((bc_input_id = (int)elem_side_bc->BC_input_id[ibc++]) != -1) {
              if (BC_Types[bc_input_id].BC_Name == LS_INLET_BC) {
                err = apply_ls_inlet_bc(afill, ijaf, x, rf, node_to_fill, elem_side_bc, exo);
