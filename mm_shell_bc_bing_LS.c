  ...
  if (af->Assemble_Jacobian)
    {
      if (gn->ConstitutiveEquation == BINGHAM)
      {
          ...
          // sensitivity w.r. to LUBRICATION PRESSURE
            ...
            for ( j=0; j<ei[pg->imtrx]->dof[var]; j++)
               {
                ...
                for (ii=0; ii<pd->Num_Dim; ii++)
                  {
                   d_func[0][var][j] +=  LubAux->dq_dp1[ii][j]   * bound_normal[ii] ;
                  }
               }
          }
            ...
            for ( j=0; j<ei[pg->imtrx]->dof[var]; j++)
               {
                ...
                Inn(grad_phi_j, grad_II_phi_j);
                  ...
                  for (ii=0; ii<pd->Num_Dim; ii++)
                    {
                     d_func[0][var][j] += LubAux->dq_dshear_top[ii][j] * bound_normal[ii];
                    }
            }
            }
            ...
            for ( j=0; j<ei[pg->imtrx]->dof[var]; j++)
               {
                ...
                Inn(grad_phi_j, grad_II_phi_j);
                  ...
                  for (ii=0; ii<pd->Num_Dim; ii++)
                    {
                     d_func[0][var][j] += LubAux->dq_dshear_bot[ii][j] * bound_normal[ii];
                    }
            }
            }
            ...
            for ( j=0; j<ei[pg->imtrx]->dof[var]; j++)
               {
                ...
                Inn(grad_phi_j, grad_II_phi_j);
                  ...
                  for (ii=0; ii<pd->Num_Dim; ii++)
                    {
                     d_func[0][var][j] += LubAux->dq_dcross_shear[ii][j] * bound_normal[ii];
                    }
            }
            }
              ...
              for ( j=0; j<ei[pg->imtrx]->dof[var]; j++)
                 {
                  ...
                  Inn(grad_phi_j, grad_II_phi_j);
                    ...
                    for (ii=0; ii<pd->Num_Dim; ii++)
                      {
                       d_func[0][var][j] += LubAux->dq_df[ii][j] * bound_normal[ii];
                      }
              }
              }
