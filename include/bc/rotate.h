...
EXTERN int apply_rotated_bc(double[], /* resid_vector                              */
...
EXTERN void get_ss_element_list(const int,       /* ss_id1 - side set number for elementlist */
...
EXTERN int set_pointers_to_vectors(struct Rotation_Vectors **, /* normal */
...
EXTERN void rotate_eqns_at_node_2D(int, int, int, struct GomaLinearSolverData *);
...
static void rotate_res_jac_mesh(const int,   /* irow_index - Elemental stiffness row      */
                                const int,   /* I - Global node number                    */
                                const int,   /* iconnect_ptr - Pointer to beginning of
                                              * connectivity list for the current element */
