...
EXTERN int apply_contact_bc(double[],          /* Solution vector for the current processor */
                            double[],          /* resid_vector */
                            ...
                            struct elem_side_bc_struct *[],
                            /* An array of pointers to the first surface * integral defined for each
                             * element. It's length is equal to the total number of
...
EXTERN double fv_at_point(double *, int);

EXTERN double fv_old_at_point(double *xi, int var);

EXTERN double fv_and_phi_at_point(double *, int, double *, int);

EXTERN int apply_embedded_colloc_bc(int,      /* ielem - element number */
                                    double[], /* Solution vector for the current processor    */
                                    double,   /* delta_t - current time step size */
                                    double,   /* theta - parameter to vary time integration:
                                               * explicit (theta = 1) --
...
EXTERN void contact_fn_dot_T(double[DIM],                                     /* func       */
