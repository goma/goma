...
EXTERN double heat_capacity                   /* mm_fill_terms.c                  */
    (HEAT_CAPACITY_DEPENDENCE_STRUCT *, dbl); /* time */
...
EXTERN double FoamVolumeSource(double,
                               double,
                               double,
                               double[DIM][MDE],
                               double[MDE],
...
EXTERN int assemble_projection_stabilization(Exo_DB *, double);

EXTERN int assemble_projection_time_stabilization(Exo_DB *exo, double time, double tt, double dt);

EXTERN int assemble_PPPS_generalized(Exo_DB *);
...
EXTERN int assemble_csf_tensor(void);

EXTERN int assemble_curvature_with_normals_source(void);

EXTERN int assemble_curvature_source(void);

EXTERN int assemble_div_n_source(void);

EXTERN int assemble_div_s_n_source(void);

EXTERN int assemble_cap_hysing(double dt, double scale);

EXTERN int assemble_cap_denner_diffusion(double dt, double scale);

EXTERN int assemble_cap_denner_diffusion_n(double dt, double scale);
...
EXTERN double visc_diss_acoustic_source(HEAT_SOURCE_DEPENDENCE_STRUCT *,
                                        dbl *, /* param - General multipliers   */
                                        int);  /* number of parameters   */

EXTERN double em_diss_heat_source(HEAT_SOURCE_DEPENDENCE_STRUCT *,
                                  dbl *, /* param - General multipliers   */
                                  int);  /* number of parameters   */

EXTERN double em_diss_e_curlcurl_source(HEAT_SOURCE_DEPENDENCE_STRUCT *,
                                        dbl *, /* param - General multipliers   */
                                        int);  /* number of parameters   */
