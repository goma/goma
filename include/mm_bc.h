...
EXTERN void find_and_set_Dirichlet(double[], /* x - solution vector at this processor     */
...
EXTERN void free_Edge_BC(struct elem_edge_bc_struct **[], /* First_Elem_Side_BC_Array          */
                         Exo_DB *, /* exo - ptr to FE db                        */
                         Dpi *);   /* dpi - ptr to dist proc info               */

EXTERN void setup_Point_BC(struct elem_side_bc_struct **[], /* First_Elem_Side_BC_Array          */
                           Exo_DB *, /* exo - ptr to FE db                        */
                           Dpi *);   /* dpi - ptr to dist proc info               */
...
EXTERN void check_for_bc_conflicts3D(Exo_DB *, /* exo                                       */
                                     Dpi *);   /* dpi                                       */
...
EXTERN int find_id_side_SS(const int,       /* ielem - element index number         (in) */
                           const int,       /* iss - sideset index number           (in) */
                           const Exo_DB *); /* exo - ptr to FE db                   (in) */

EXTERN void print_setup_Surf_BC(struct elem_side_bc_struct *[]); /* First_Elem_Side_BC_Array */
...
extern int find_bc_unk_offset(
    struct Boundary_Condition *, int, int, int, int *, VARIABLE_DESCRIPTION_STRUCT **);
