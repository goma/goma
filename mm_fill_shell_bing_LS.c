 ...
 ******************************************************************************/
int
assemble_shell_web_structure(double time_value,  /* Time */
			 double theta,       /* Time stepping parameter */
			 double delta_t,     /* Time step size */
			 const double wt,    /* Gauss point weight */
			 double xi[DIM],     /* Local stu coordinates */
    ...
    for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
      ...
      if ( pd->v[pg->imtrx][var] ) {
        ...
        for(j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
          ...
          diffusion = 0.0;
          diffusion += elc->bend_stiffness * d_phi_dxi[i] * d_sh_K_dxi
                       * d_det_J_dmeshbj / det_J_sh / det_J_sh;
 ...
 ******************************************************************************/
int
assemble_shell_web_coordinates(double time_value,  /* Time */
			   double theta,       /* Time stepping parameter */
			   double delta_t,     /* Time step size */
			   const double wt,    /* Gauss point weight */
			   double xi[DIM],     /* Local stu coordinates */
      ...
      for (p = 0; p < pd->Num_Dim; p++)
        {
          ...
          if (pd->e[pg->imtrx][eqn])
		      ...
		      if (n_dof[var] > 0) 
			{
			  ...
			  for (j = 0; j < n_dof[var]; j++) 
			    {			   
			      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += -fv->dsnormal_dx[p][b][j] * 
				phi_i * pd0->etm[pg->imtrx][eqn][(LOG2_DIFFUSION)] * wt * det_J * h3;
			      
			      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += (fv->n[p] - fv->snormal[p]) * 
				phi_i * pd0->etm[pg->imtrx][eqn][(LOG2_DIFFUSION)] * wt * fv->dsurfdet_dx[b][j];
			    }
			}
                    }
                }
  ...
  if ( af->Assemble_Jacobian )
      ...
      if (pvar != -1)
	{
	  for (j=0; j<n_dof[var]; j++)
		...
		if(dof_map[j] == ei[pg->imtrx]->dof_list[var][el1])el2=el1;
	      }
 ...
 ******************************************************************************/
{
  ...
  if (af->Assemble_Jacobian)
	...
	if (Dolphin[pg->imtrx][I][VELOCITY1] > 0 ) {
	   ...
	   *            I wonder if this is fortuitous? I could probably check dof_map[] versus local element node numbers for ei[pg->imtrx]->dof[var]
	   *            and check this out.
	   */
	  var = N_DOT_CURL_V;
	  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++)
			  ...
			  double scale) /* Scale factor, nondimension       */
{
  ...
  if (af->Assemble_Residual) {
    ...
    if (!tension_fixed ) {
      ...
      id_dofshell = id_shell; 
      lec->R[LEC_R_INDEX(upd->ep[pg->imtrx][ieqn_shell],id_dofshell)] +=
        scale * (fv->stangent[0][0] * lec->R[LEC_R_INDEX(upd->ep[pg->imtrx][ieqn_mom1],id_dofmom1)] +
		  ...
		  for ( j_id=0; j_id<ei[pg->imtrx]->dof[var]; j_id++)
		    {
                      lec->J[LEC_J_INDEX(peqn_shell,pvar,id_dofshell,j_id)] +=
                        scale*(fv->snormal[0] * lec->J[LEC_J_INDEX(peqn_mom1,pvar,id_dofmom1,j_id)] +
                               ...
                               fv->dsnormal_dx[0][q][j_id] * lec->R[LEC_R_INDEX(upd->ep[pg->imtrx][ieqn_mom1],id_dofmom1)] +
                               fv->dsnormal_dx[1][q][j_id] * lec->R[LEC_R_INDEX(upd->ep[pg->imtrx][ieqn_mom2],id_dofmom2)]);
		    }
		}
	    }
		  ...
		  for ( j_id=0; j_id<ei[pg->imtrx]->dof[var]; j_id++)
		    {
                      lec->J[LEC_J_INDEX(peqn_shell,pvar,id_dofshell,j_id)] +=
                        scale*(fv->stangent[0][0] * lec->J[LEC_J_INDEX(peqn_mom1,pvar,id_dofmom1,j_id)] +
                               ...
                               fv->dstangent_dx[0][0][q][j_id] * lec->R[LEC_R_INDEX(upd->ep[pg->imtrx][ieqn_mom1],id_dofmom1)] +
                               fv->dstangent_dx[0][1][q][j_id] * lec->R[LEC_R_INDEX(upd->ep[pg->imtrx][ieqn_mom2],id_dofmom2)]);
		    }
		}
	    }
			  ...
			  double scale) /* Scale factor, nondimension       */
{
  ...
  if (af->Assemble_Residual) {
    ...
    if (!tension_fixed ) {
      ...
      lec->R[LEC_R_INDEX(upd->ep[pg->imtrx][ieqn_shell],id_dofshell)] +=	  scale * (fv->stangent[0][0] * lec->R[LEC_R_INDEX(upd->ep[pg->imtrx][ieqn_mom1],id_dofmom1)] +
                                                                   fv->stangent[0][1] * lec->R[LEC_R_INDEX(upd->ep[pg->imtrx][ieqn_mom2],id_dofmom2)]);
	  ...
	  if ( pd->v[pg->imtrx][var] )
	    {
	      ...
	      for ( j_id=0; j_id<ei[pg->imtrx]->dof[var]; j_id++)
		{	
                  /*			lec->J[LEC_J_INDEX(peqn_shell,pvar,id_dofshell,j_id)] += scale* lec->J[LEC_J_INDEX(peqn_mom1,pvar,id_dofmom1,j_id)]	;	*/
                  lec->J[LEC_J_INDEX(peqn_shell,pvar,id_dofshell,j_id)] +=
                    scale*(fv->stangent[0][0] * lec->J[LEC_J_INDEX(peqn_mom1,pvar,id_dofmom1,j_id)] +
                           fv->stangent[0][1] * lec->J[LEC_J_INDEX(peqn_mom2,pvar,id_dofmom2,j_id)]);
		}
		  ...
		  if ( n_dof[var] > 0)   /* NOTE: Cannot use pd->v here! */
		    {
	              ...
	              for(j = 0; j < n_dof[var]; j++) 
		        {
			  lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += d_sn_ang_dsn[p][b] * phi_i *
			    wt * det_J * h3;
		        }
		    }
		}
		                   ...
		                   const Exo_DB *exo)
{
  ...
  if (af->Assemble_Residual)
	...
	{ 
	  ...
	  for (b = 0; b < dim; b++) 
	    {
	      lec->R[LEC_R_INDEX(peqn,i)] -= surfaceDiffusionCoeff * (fv->grad_div_s_v[b] * (bfn->grad_phi[i][b] - tmp * fv->n[b])) * wt * det_J * h3;
	    }
	}
	...
	{ 
	  ...
	  for (b = 0; b < dim; b++) 
	    {
	      lec->R[LEC_R_INDEX(peqn,i)] -= surfaceDiffusionCoeff3 * (fv->grad_curv[b] * (bfn->grad_phi[i][b] - tmp * fv->n[b]))  * wt * det_J * h3;
	    }
	}
      ...
      for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++)
	{
	  ...
	  lec->R[LEC_R_INDEX(peqn,i)] += (fv->n_dot_curl_s_v - n_dot_curl_s_v) * phi_i * wt * det_J * h3; 
	}
      ...
      for (p = 0; p < dim; p++)
	{
	  ...
	  for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++)
	    {
	      phi_i = bf[eqn]->phi[i];
	      lec->R[LEC_R_INDEX(peqn,i)] += (fv->grad_v_dot_n[p] - grad_s_v_dot_n[p]) * phi_i * wt * det_J * h3;
	      ...
	      for (b = 0; b < dim; b++)
		{
	      ...
	      for (b = 0; b < VIM; b++) 
		{
		  lec->R[LEC_R_INDEX(peqn,i)] -= surfaceDiffusionCoeff2 * (fv->serialgrad_grad_s_v_dot_n[p][b] * (bfn->grad_phi[i][b] - tmp * fv->n[b])) * wt * det_J * h3;
		}	      
	    }
	}
	  ...
	  for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++)
		  ...
		  if (n_dof[var] > 0)   /* NOTE: Cannot use pd->v here! */
		    {
		      ...
		      for (j = 0; j < n_dof[var]; j++) 
			{
			  lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= d_grad_s_v_dot_n_dx[p][b][j] * phi_i * wt * det_J * h3;
			}
		      for (j = 0; j < n_dof[var]; j++) 
			{
			  lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += (fv->grad_v_dot_n[p] - grad_s_v_dot_n[p]) * phi_i * wt * (fv->dsurfdet_dx[b][j]);
			}
		    }
		}
	      for (b = 0; b < VIM; b++ )
		  ...
		  if (n_dof[var] > 0)   /* NOTE: Cannot use pd->v here! */
		    {
		      ...
		      for (j = 0; j < n_dof[var]; j++) 
			{
			  lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= d_grad_s_v_dot_n_dv[p][b][j] * phi_i * wt * det_J * h3;
			}
		    }
		}
	      ...
	      if (pd->v[pg->imtrx][var])
		{
		  ...
		  for (b = 0; b < VIM; b++)
			...
			{			  
			  //      lec->R[LEC_R_INDEX(peqn,i)] -= surfaceDiffusionCoeff2 * (fv->serialgrad_grad_s_v_dot_n[p][b] * (bfn->grad_phi[i][b] - tmp * fv->n[b])) * wt * det_J * h3;
			  lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= surfaceDiffusionCoeff2 * bfn->grad_phi[j][b] * (bfn->grad_phi[i][b] - tmp * fv->n[b])  * wt * det_J * h3;
			}
		    }
		}
	      ...
	      for (b = 0; b < dim; b++)
		{
		  ...
		  if (pd->v[pg->imtrx][var])
			...
			{
			  phi_j = bf[var]->phi[j];
			  lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= surfaceDiffusionCoeff2 * fv->serialgrad_grad_s_v_dot_n[p][b] * (- tmp * phi_j)  * wt * det_J * h3;
		      
			  for (a = 0; a < dim; a++)
			    {
			      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= surfaceDiffusionCoeff2 * fv->serialgrad_grad_s_v_dot_n[p][a] * (- phi_j * bfn->grad_phi[i][b] * fv->n[a])  * wt * det_J * h3;
			    } 
			}
		    }
		}
	      ...
	      for (r = 0; r < dim; r++ )
		{
		  ...
		  if (n_dof[var] > 0)
		      ...
		      for (j = 0; j < n_dof[var]; j++) 
			{
			  for (b = 0; b < dim; b++)
			    {
			      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= surfaceDiffusionCoeff2 * fv->serialgrad_grad_s_v_dot_n[p][b] * (bfn->grad_phi[i][b] - tmp * fv->n[b]) * wt * (fv->dsurfdet_dx[r][j]);	
			    }
			}
		      ...
		      for (j = 0; j < ei[pg->imtrx]->dof[var]; j++)
			{
			  ...
			  for (b = 0; b < dim; b++) 
			    {
                              lec->J[LEC_J_INDEX(peqn,pvar,i,jk)] -= surfaceDiffusionCoeff2 * fv->serialgrad_grad_s_v_dot_n[p][b] * (bfn->d_grad_phi_dmesh[i][b][r][j]) * wt *  det_J * h3;
                              lec->J[LEC_J_INDEX(peqn,pvar,i,jk)] -= surfaceDiffusionCoeff2 * fv->d_serialgrad_grad_s_v_dot_n_dmesh[p][b][r][j] * (bfn->grad_phi[i][b] - tmp * fv->n[b]) * wt * det_J * h3;
			      for (a = 0; a < dim; a++) 
				{
                                  lec->J[LEC_J_INDEX(peqn,pvar,i,jk)] -= surfaceDiffusionCoeff2 * fv->serialgrad_grad_s_v_dot_n[p][b] * (- fv->n[a] * bfn->d_grad_phi_dmesh[i][a][r][j] * fv->n[b]) * wt * det_J * h3;
				}
			    }
			}
		    }
      ...
      for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++)
	{
...
#ifdef NEWMETHOD_SHELL_SURF_DIV_V
	  //     lec->R[LEC_R_INDEX(peqn,i)] += (2.0 * fv->curv * fv->n[b] * fv->v[b]) * phi_i * wt * det_J * h3;
	  for (b = 0; b < dim; b++)
	    {
	      var = VELOCITY1 + b;
	      if (n_dof[var] > 0)   /* NOTE: Cannot use pd->v here! */
		{
		  ...
		  for (j = 0; j < n_dof[var]; j++) 
		    {
		      phi_j = bfv->phi[j];
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += (2.0 * fv->curv * fv->n[b] * phi_j) * phi_i * wt * det_J * h3;
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += phi_j * bfn->grad_phi[i][b] * wt * det_J * h3;
		      for (a = 0; a < dim; a++) 
			{ 
			  lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= fv->n[a] * fv->n[b] * phi_j * bfn->grad_phi[i][a] * wt * det_J * h3;
			}
		    }
		}
	    }
	  ...
	  for (b = 0; b < dim; b++)
	      ...
	      if (pd->v[pg->imtrx][var])
		{
		  ...
		  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++)
		    {
		      phi_j = bf[var]->phi[j];
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += (2.0 * fv->curv * phi_j * fv->v[b]) * phi_i * wt * det_J * h3;
		      for (a = 0; a < dim; a++) 
			{
			  lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= phi_j * fv->n[a] *(fv->v[a] * bfn->grad_phi[i][b] + fv->v[b] * bfn->grad_phi[i][a]) * wt * det_J * h3;
			}
		    }
		}
	    }
	  ...
	  if (pd->v[pg->imtrx][var]) 
	      ...
	      for (b = 0; b < dim; b++)
		{
		  ...
		  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) 
		    {
		      phi_j = bf[var]->phi[j];
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += (2.0 * phi_j * fv->n[b] * fv->v[b]) * phi_i * wt * det_J * h3;
		    }
		}
	    }
	  ...
	  for (p = 0; p < dim; p++ )
	      ...
	      if (n_dof[var] > 0)   /* NOTE: Cannot use pd->v here! */
		{
		  ...
		  for (j = 0; j < n_dof[var]; j++) 
		    {
		      ...
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += (tmp_i + tmp * phi_i) * wt * (fv->dsurfdet_dx[p][j]);
		    }
		  /*
		   ...
		   *  We use ei[pg->imtrx]->dof[var] here because it correctly identifies the number of mesh unknowns that are on the shell (3)
		  ...
		  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++)
		    {
		      ...
		      for (b = 0; b < dim; b++) 
			{
                          lec->J[LEC_J_INDEX(peqn,pvar,i,jk)] += fv->v[b] * bfn->d_grad_phi_dmesh[i][b][p][j] * wt * det_J * h3;
			  for (a = 0; a < dim; a++) 
			    {
                              lec->J[LEC_J_INDEX(peqn,pvar,i,jk)] -= fv->n[b] * fv->n[a] * fv->v[a] * bfn->d_grad_phi_dmesh[i][b][p][j] * wt * det_J * h3;
			    }
			}
		    }
		}
	  ...
	  for (b = 0; b < dim; b++ )
	      ...
	      if (n_dof[var] > 0)   /* NOTE: Cannot use pd->v here! */
		{
		  ...
		  for (j = 0; j < n_dof[var]; j++) 
		    {
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += (*(esp->div_s_v[i]) - div_s_V) * phi_i * wt * (fv->dsurfdet_dx[b][j]);
		    }
#else
		  for (j = 0; j < n_dof[var]; j++) 
		    {
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += (fv->div_s_v - div_s_V) * phi_i * wt * (fv->dsurfdet_dx[b][j]);
		    }
#endif
		}
	    }
	  ...
	  if (pd->v[pg->imtrx][var])
	      ...
	      for (b = 0; b < dim; b++)
		{
		  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) 
		    {
		      phi_j = bf[var]->phi[j];	
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= surfaceDiffusionCoeff * bfn->grad_phi[j][b] * (bfn->grad_phi[i][b] - tmp * fv->n[b])  * wt * det_J * h3;
		    }
		}
	    }
	  ...
	  for (b = 0; b < dim; b++)
	      ...
	      if (pd->v[pg->imtrx][var])
		{
		  ...
		  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++)
		    {
		      phi_j = bf[var]->phi[j];
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= surfaceDiffusionCoeff * fv->grad_div_s_v[b] * (- tmp * phi_j)  * wt * det_J * h3;

		      for (a = 0; a < dim; a++)
			{
			  lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= surfaceDiffusionCoeff * fv->grad_div_s_v[a] * (- phi_j * bfn->grad_phi[i][b] * fv->n[a])  * wt * det_J * h3;
			} 
		    }
		}
	    }
		  ...
		  for (j = 0; j < n_dof[var]; j++) 
		    {
		      for (b = 0; b < dim; b++)
			{
			  lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= surfaceDiffusionCoeff * (fv->grad_div_s_v[b] * (bfn->grad_phi[i][b] - tmp * fv->n[b]))   * wt * (fv->dsurfdet_dx[p][j]);
			}
		    }
		  /*
		   *  This has been checked out in the debugger and it passes. What's going on?
		  ...
		  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++)
		    {
		      ...
		      for (b = 0; b < dim; b++) 
			{
                          lec->J[LEC_J_INDEX(peqn,pvar,i,jk)] -= surfaceDiffusionCoeff * fv->grad_div_s_v[b] * (bfn->d_grad_phi_dmesh[i][b][p][j]) * wt *  det_J * h3 ;
                          lec->J[LEC_J_INDEX(peqn,pvar,i,jk)] -= surfaceDiffusionCoeff * fv->d_grad_div_s_v_dmesh[b][p][j] * (bfn->grad_phi[i][b] - tmp * fv->n[b]) * wt *  det_J * h3;
		
			  for (a = 0; a < dim; a++) 
			    {
                              lec->J[LEC_J_INDEX(peqn,pvar,i,jk)] -= surfaceDiffusionCoeff * fv->grad_div_s_v[b] * (- fv->n[a] * bfn->d_grad_phi_dmesh[i][a][p][j] * fv->n[b]) * wt *  det_J * h3;
			    }
			}
		    }
		}
      ...
      for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++)
	{
	  ...
	  for (b = 0; b < dim; b++ )
	      ...
	      if (n_dof[var] > 0)  
		{
		  ...
		  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) 
		    {
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -=  (-0.5) * d_div_s_n_dn[b][j] * phi_i * wt * det_J * h3;
		    }
		}
	    }
		  ...
		  for (j = 0; j < n_dof[var]; j++) 
		    {
		      ...
		      //	printf("we are here\n");
		      //    }
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -=  (-0.5) * d_div_s_n_dx[b][j] * phi_i * wt * det_J * h3;
		    }

		  for (j = 0; j < n_dof[var]; j++)
		    {
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += (fv->curv - curv) * phi_i * wt * (fv->dsurfdet_dx[b][j]);
		    }
		}
	    }
	  ...
	  for (a = 0; a < dim; a++)
	    {
	      tmp += fv->n[a] * bfn->grad_phi[i][a];
       	  ...
       	  //   lec->R[LEC_R_INDEX(peqn,i)] -= surfaceDiffusionCoeff * (fv->grad_curv[b] * (bfn->grad_phi[i][b] - tmp * fv->n[b]))  * wt * det_J * h3;
	  ...
	  if (pd->v[pg->imtrx][var])
	      ...
	      for (b = 0; b < dim; b++)
		{
		  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++) 
		    {
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= surfaceDiffusionCoeff3 * bfn->grad_phi[j][b] * (bfn->grad_phi[i][b] - tmp * fv->n[b])  * wt * det_J * h3;
		    }
		}
	    }
	  ...
	  for (b = 0; b < dim; b++)
	      ...
	      if (pd->v[pg->imtrx][var])
		{
		  ...
		  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++)
		    {
		      phi_j = bf[var]->phi[j];
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= surfaceDiffusionCoeff3 * fv->grad_curv[b] * (- tmp * phi_j)  * wt * det_J * h3;
		      for (a = 0; a < dim; a++)
			{
			  lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= surfaceDiffusionCoeff3 * fv->grad_curv[a] * (- phi_j * bfn->grad_phi[i][b] * fv->n[a]) * wt * det_J * h3;
			}
		    }
		}
	    }
		  ...
		  for (j = 0; j < n_dof[var]; j++) 
		    {
		      for (b = 0; b < dim; b++)
			{
			  lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= surfaceDiffusionCoeff3 * (fv->grad_curv[b] * (bfn->grad_phi[i][b] - tmp * fv->n[b])) * wt * (fv->dsurfdet_dx[p][j]);
			}
		    }
		  /*
		   *  This has been checked out in the debugger and it passes. What's going on?
		  ...
		  for (j = 0; j < ei[pg->imtrx]->dof[var]; j++)
		    {
		      ...
		      for (b = 0; b < dim; b++) 
			{
                          lec->J[LEC_J_INDEX(peqn,pvar,i,jk)] -= surfaceDiffusionCoeff3 * fv->grad_curv[b] * (bfn->d_grad_phi_dmesh[i][b][p][j]) * wt *  det_J * h3 ;
                          lec->J[LEC_J_INDEX(peqn,pvar,i,jk)] -= surfaceDiffusionCoeff3 * fv->d_grad_curv_dmesh[b][p][j] * (bfn->grad_phi[i][b] - tmp * fv->n[b]) * wt *  det_J * h3;
			  for (a = 0; a < dim; a++) 
			    {
                              lec->J[LEC_J_INDEX(peqn,pvar,i,jk)] -= surfaceDiffusionCoeff3 * fv->grad_curv[b] * (- fv->n[a] * bfn->d_grad_phi_dmesh[i][a][p][j] * fv->n[b]) * wt * det_J * h3;
			    }
			}
		    }
		}
      ...
      for (i = 0; i < ei[pg->imtrx]->dof[eqn]; i++)
	{
	  ...
	  for (b = 0; b < dim; b++)
	      ...
	      if (n_dof[var] > 0)   /* NOTE: Cannot use pd->v here! */
		{
		  ...
		  for (j = 0; j < n_dof[var]; j++) 
		    {
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= d_n_dot_curl_s_v_dx[b][j] * phi_i *
			wt * det_J * h3;
		    }
		  /*
		  ...
		  for (j = 0; j < n_dof[var]; j++) 
		    {
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] += (fv->n_dot_curl_s_v - n_dot_curl_s_v) * phi_i *
	  ...
	  for (b = 0; b < VIM; b++)
	      ...
	      if (n_dof[var] > 0)   /* NOTE: Cannot use pd->v here! */
		{
		  ...
		  for (j = 0; j < n_dof[var]; j++) 
		    {
		      lec->J[LEC_J_INDEX(peqn,pvar,i,j)] -= d_n_dot_curl_s_v_dv[b][j] * phi_i * wt * det_J * h3;
		    }
		}
	    }
	}
           ...
           for ( b = 0; b < dim; b++)
              {
               for ( k = 0; k < ei[pg->imtrx]->dof[MESH_DISPLACEMENT1]; k++)
                  {
                   dH_dtime_dmesh[b][k] -= fv->n[i] * delta(i,b) * bf[MESH_DISPLACEMENT1]->phi[k] * (1+2*tt)/dt;
               ...
               for ( k = 0; k < ei[pg->imtrx]->dof[SHELL_NORMAL1]; k++)
                  {
                   ...
                   dH_dtime_dnormal[b][k] -= fv->d[i] * delta(i,b) * bf[SHELL_NORMAL1]->phi[k] * (1+2*tt)/dt;
                  }
              }
          }
      }
	       ...
	       )
{
  ...
  if ( pd->v[pg->imtrx][FILL] ) {
    ...
    if ( bang < 0 ) {
      ...
      *dPdz_nbar = 0.0;
      *dPdz_curv = 0.0;
      for (i=0;i<ei[pg->imtrx]->dof[R_FILL];i++) dPdz_F[i] = 0.0;
    } else {
      *dPdz      *= bang;
      *dPdz_S    *= bang;
      *dPdz_Plub *= bang;
...
assemble_porous_shell_closed(
  ...
  for ( i = 0; i < ei[pg->imtrx]->dof[R_FILL]; i++) vz_F[i] = -k/mu*dPdz_F[i];
	...
	for ( b = 0; b < DIM; b++) {
	  ...
	  for ( j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
	    ...
	    // Assemble Jacobian terms
            lec->J[LEC_J_INDEX(peqn,pvar,i,jj)] += (mass + divergence) * wt * h3 * fv->dsurfdet_dx[b][jj];
...
assemble_porous_shell_gasn(
  ...
  if ( pd->v[pg->imtrx][FILL] ) {
    ...
    flux_N *= HsideM;
    flux_S *= HsideM;
    for ( i = 0; i < ei[pg->imtrx]->dof[R_FILL]; i++) flux_F[i] = flux * d_HsideM_dF[i];
  }
...
assemble_porous_shell_open(
  ...
  if ( pd->v[pg->imtrx][FILL] ) {
    load_lsi( ls->Length_Scale );
    Hside = 1 - lsi->Hn;
    for ( i = 0; i < ei[pg->imtrx]->dof[FILL]; i++) d_Hside_dF[i] = -lsi->d_Hn_dF[i];
  }
 ...
 if (pd->e[pg->imtrx][R_LUBP])
  ...
  if(upd->ep[pg->imtrx][R_LUBP_2] >=0 )
	...
	Hside_2 = 1 - lsi->Hn;
	for ( i = 0; i < ei[pg->imtrx]->dof[FILL]; i++) d_Hside_2_dpF[i] = -lsi->d_Hn_dF[i];
      }
   ...
   */
{
  ...
  if ( pd->v[pg->imtrx][PHASE1] ) {
    ...
    load_lsi( ls->Length_Scale );
    Hside = 1 - lsi->Hn;
    for ( i = 0; i < ei[pg->imtrx]->dof[PHASE1]; i++) d_Hside_dF[i] = -lsi->d_Hn_dF[i];
    ls = ls_old;
  }
...
int
assemble_lubrication_bingham( double time,    /* present time value */
                                double tt,      /* parameter to vary time integration from
                                                   explicit (tt = 1) to implicit (tt = 0)    */
                                double dt,      /* current time step size */  
                                double xi[DIM], /* Local stu coordinates */
...

{
 ...
 if (fabs(shear_top)<epsilon){
	  shear_top += epsilon;
  }
  
  if (fabs(shear_bot)<epsilon){
  	  shear_bot -= epsilon;
    }
  
  if (fabs(cross_shear)<epsilon){
	  cross_shear += epsilon;
    }
  ...
  dbl GRAV[DIM];
      ...
      dbl rho, rho1, rho2, mu2, hsi, dhsi;
      //dbl drho;
      ...
      rho1 = mp->density;
      rho = (1.0 - hsi) * rho1 + hsi * rho2;
      //drho = dhsi * (-rho1 + rho2);
  
  /* Extract pressure gradient magnitude */
     for (i = 0; i < DIM; i++)
        {
         grad_P[i] = fv->grad_lubp[i] - rho * GRAV[i];
        }
  ...
  calculate_lub_q_v_bingham(time, dt);
 ...
 dbl gau_qua_0_1[100][2] = {{0.000367317245253,	0.999856863386721},
     			 {0.000854696326759,	0.999245975319798},
 				 {0.001341962685777,	0.998147567366563},
 				 {0.001827980600663,	0.996562468518722},
 				 {0.002312225031711,	0.994492197621496},
 				 {0.002794214001933,	0.991938770353029},
 				 {0.003273474225423,	0.988904679243459},
 				 {0.003749536627732,	0.985392887881853},
 				 {0.004221935734834,	0.981406827127908},
 				 {0.004690209826847,	0.976950391462746},
 				 {0.005153901287434,    0.972027935068128},
 				 {0.005612557011593,	0.96664426752154},
 				 {0.00606572883149,	    0.960804649072667},
 				 {0.006512973946486,	0.954514785491265},
 				 {0.006953855351859,	0.947780822485363},
 				 {0.007387942263721,	0.940609339692509},
 				 {0.007814810538773,	0.933007344248582},
 				 {0.008234043088073,	0.924982263939796},
 				 {0.008645230284162,	0.9165419399442},
 				 {0.009047970361064,	0.907694619169588},
 				 {0.009441869806687,	0.898448946195157},
 				 {0.009826543747218,	0.888813954824748},
 				 {0.010201616323105,	0.878799059259854},
 				 {0.010566721056264,	0.86841404490101},
 				 {0.010921501208124,	0.857669058786528},
 				 {0.011265610128168,	0.846574599677901},
 				 {0.011598711592627,	0.835141507801571},
 				 {0.011920480132984,	0.823380954257065},
 				 {0.012230601353979,	0.811304430101854},
 				 {0.01252877224079,	    0.798923735123589},
 				 {0.012814701455104,	0.786250966310691},
 				 {0.013088109619773,	0.773298506032547},
 				 {0.013348729591786,	0.760079009940882},
 				 {0.013596306723289,	0.746605394604096},
 				 {0.013830599110396,	0.732890824886679},
 				 {0.014051377829551,	0.718948701086016},
 				 {0.014258427161198,	0.704792645839151},
 				 {0.014451544800563,	0.690436490812315},
 				 {0.014630542055319,	0.675894263186211},
 				 {0.014795244029956,	0.661180171950265},
 				 {0.014945489796666,	0.646308594019236},
 				 {0.015081132552585,	0.631294060185752},
 				 {0.015202039763227,	0.616151240922487},
 				 {0.01530809329199, 	0.600894932047868},
 				 {0.015399189515576,	0.585540040269302},
 				 {0.015475239425246,	0.570101568618057},
 				 {0.015536168713783,	0.554594601790031},
 				 {0.015581917848105,	0.539034291406718},
 				 {0.015612442127425,	0.523435841210796},
 				 {0.015627711726932,	0.507814492210772},
 				 {0.015627711726932,	0.492185507789229},
 				 {0.015612442127425,	0.476564158789204},
 				 {0.015581917848105,	0.460965708593282},
 				 {0.015536168713783,	0.445405398209969},
 				 {0.015475239425246,	0.429898431381943},
 				 {0.015399189515576,	0.414459959730698},
 				 {0.01530809329199, 	0.399105067952132},
 				 {0.015202039763227,	0.383848759077513},
 				 {0.015081132552585,	0.368705939814248},
 				 {0.014945489796666,	0.353691405980764},
 				 {0.014795244029956,	0.338819828049735},
 				 {0.014630542055319,	0.324105736813789},
 				 {0.014451544800563,	0.309563509187685},
 				 {0.014258427161198,	0.295207354160849},
 				 {0.014051377829551,	0.281051298913984},
 				 {0.013830599110396,	0.267109175113321},
 				 {0.013596306723289,	0.253394605395905},
 				 {0.013348729591786,	0.239920990059119},
 				 {0.013088109619773,	0.226701493967453},
 				 {0.012814701455104,	0.213749033689309},
 				 {0.01252877224079, 	0.201076264876411},
 				 {0.012230601353979,	0.188695569898146},
 				 {0.011920480132984,	0.176619045742935},
 				 {0.011598711592627,	0.164858492198429},
 				 {0.011265610128168,	0.153425400322099},
 				 {0.010921501208124,	0.142330941213472},
 				 {0.010566721056264,	0.13158595509899},
 				 {0.010201616323105,	0.121200940740146},
 				 {0.009826543747218,	0.111186045175252},
 				 {0.009441869806687,	0.101551053804843},
 				 {0.009047970361064,	0.092305380830412},
 				 {0.008645230284162,	0.0834580600558},
 				 {0.008234043088073,	0.075017736060204},
 				 {0.007814810538773,	0.066992655751418},
 				 {0.007387942263721,	0.059390660307491},
 				 {0.006953855351859,	0.052219177514637},
 				 {0.006512973946486,	0.045485214508735},
 				 {0.00606572883149, 	0.039195350927333},
 				 {0.005612557011593,	0.03335573247846},
 				 {0.005153901287434,	0.027972064931872},
 				 {0.004690209826847,	0.023049608537254},
 				 {0.004221935734834,	0.018593172872092},
 				 {0.003749536627732,	0.014607112118147},
 				 {0.003273474225423,	0.011095320756541},
 				 {0.002794214001933,	0.008061229646971},
 				 {0.002312225031711,	0.005507802378504},
 				 {0.001827980600663,	0.003437531481278},
 				 {0.001341962685777,	0.001852432633437},
 				 {0.000854696326759,	0.000754024680202},
 				 {0.000367317245253,	0.000143136613279},
     	 };
 ...
 dbl mu_top, mu_bot, shear_top_k_l, shear_bot_k_l, mu_top_k_l, mu_bot_k_l, mu_top1, mu_bot1, mu_top_k_l1, mu_bot_k_l1;
 dbl d_mu_top, d_mu_bot, d_mu_top_k_l, d_mu_bot_k_l, d_shear_top_k_l, d_shear_bot_k_l;
 dbl int_tan_top, int_tan_bot, int_norm_top, int_norm_bot;
 dbl d_int_tan_top_d_shear_top, d_int_tan_bot_d_shear_bot;
 dbl d_int_norm_top_d_shear_top, d_int_norm_bot_d_shear_bot;
 dbl dmu_top_k_l_dF, dmu_bot_k_l_dF, dint_tan_top_dF, dint_tan_bot_dF, dint_norm_top_dF, dint_norm_bot_dF;
 dbl dmu_top_dF, dmu_bot_dF;
 
 dmu_top_k_l_dF = 0.0; dmu_bot_k_l_dF = 0.0;
 dint_tan_top_dF = 0.0; dint_tan_bot_dF = 0.0; dint_norm_top_dF = 0.0; dint_norm_bot_dF = 0.0;
 dmu_top_dF = 0.0; dmu_bot_dF = 0.0;
 
 
 mu_top = 0.0; mu_bot = 0.0; shear_top_k_l = 0.0; shear_bot_k_l = 0.0; mu_top_k_l = 0.0;
 mu_bot_k_l = 0.0;
 d_mu_top = 0.0; d_mu_bot = 0.0; d_mu_top_k_l = 0.0; d_mu_bot_k_l = 0.0; d_shear_top_k_l = 0.0;
 d_shear_bot_k_l = 0.0;
 int_tan_top = 0.0; int_tan_bot = 0.0; int_norm_top = 0.0; int_norm_bot = 0.0;
 d_int_tan_top_d_shear_top = 0.0; d_int_tan_bot_d_shear_bot = 0.0;
 d_int_norm_top_d_shear_top = 0.0; d_int_norm_bot_d_shear_bot = 0.0;
 ...
 for (k = 0; k<n_divis; k++)
 {
	  for (l = 0; l<100; l++)
	  {
		  shear_top_k_l = shear_top / (dbl)n_divis * ((dbl)k + gau_qua_0_1[l][1]);
		  shear_bot_k_l = shear_bot / (dbl)n_divis * ((dbl)k + gau_qua_0_1[l][1]);
		  
		  mu_top_k_l1 = atexp * (muinf + (mu0 - muinf + tau_y * (1.0 - exp(-atexp * fabs(shear_top_k_l) 
		          * fexp)) / (atexp * fabs(shear_top_k_l)))
				* (pow(1.0 + pow(atexp * lam * fabs(shear_top_k_l) , aexp) , (nexp - 1.0) 
				            		 / aexp)));
		  
		  mu_top_k_l = (1.0 - hsi) * mu_top_k_l1 + mu2 * hsi;
		  
		  mu_bot_k_l1 = atexp * (muinf + (mu0 - muinf + tau_y * (1.0 - exp(-atexp * fabs(shear_bot_k_l) 
		      		* fexp)) / (atexp * fabs(shear_bot_k_l)))
		      					* (pow(1.0 + pow(atexp * lam * fabs(shear_bot_k_l) , aexp) , (nexp - 1.0) 
		      				 / aexp)));
		  
		  mu_bot_k_l = (1.0 - hsi) * mu_bot_k_l1 + mu2 * hsi;
		  
		  dmu_top_k_l_dF = (- dhsi) * mu_top_k_l1 + mu2 * dhsi;
		  
		  dmu_bot_k_l_dF = (- dhsi) * mu_bot_k_l1 + mu2 * dhsi;
		  
		  
		  
		  int_tan_top += mu_top_k_l * shear_top_k_l * shear_top * gau_qua_0_1[l][0] / (dbl)n_divis;  
		  
		  
		  
		  int_tan_bot += mu_bot_k_l * shear_bot_k_l * shear_bot * gau_qua_0_1[l][0] / (dbl)n_divis;
		  
		  int_norm_top += log(mu_top_k_l) * shear_top * gau_qua_0_1[l][0] / (dbl)n_divis;
		  
		  int_norm_bot += log(mu_bot_k_l) * shear_bot * gau_qua_0_1[l][0] / (dbl)n_divis; 
		  
		  dint_tan_top_dF += dmu_top_k_l_dF * shear_top_k_l * shear_top * gau_qua_0_1[l][0] / (dbl)n_divis;  
		  
		  
		  
		  dint_tan_bot_dF += dmu_bot_k_l_dF * shear_bot_k_l * shear_bot * gau_qua_0_1[l][0] / (dbl)n_divis;
		  
		  dint_norm_top_dF += dmu_top_k_l_dF / mu_top_k_l * shear_top * gau_qua_0_1[l][0] / (dbl)n_divis;
		  
		  dint_norm_bot_dF += dmu_top_k_l_dF / mu_bot_k_l * shear_bot * gau_qua_0_1[l][0] / (dbl)n_divis; 
		  
		  
		  
		  
		  
		  d_shear_top_k_l = 1.0 / (dbl)n_divis * ((dbl)k + gau_qua_0_1[l][1]);
		  
		  d_shear_bot_k_l = 1.0 / (dbl)n_divis * ((dbl)k + gau_qua_0_1[l][1]);
		  
		  
		  d_mu_top_k_l = (d_shear_top_k_l * atexp * (tau_y * (atexp * fexp * exp(-atexp * fabs(shear_top_k_l) * fexp) 
		  		     * shear_top_k_l / fabs(shear_top_k_l) * atexp * fabs(shear_top_k_l) - (1.0 - exp(-atexp 
		  		    * fabs(shear_top_k_l) * fexp)) * atexp * shear_top_k_l / fabs(shear_top_k_l)) 
		  		    / pow(atexp * fabs(shear_top_k_l) , 2.0) * pow(1.0 + pow(atexp * lam 
		  		    * fabs(shear_top_k_l), aexp), (nexp - 1.0) / aexp) + (mu0 - muinf +tau_y 
		  		    * (1.0 - exp(-atexp * fabs(shear_top_k_l) * fexp)) / (atexp * fabs(shear_top_k_l))) 
		  			* (nexp - 1.0) / aexp * pow(1.0 + pow(atexp * lam * fabs(shear_top_k_l), aexp), (nexp - 1.0 -aexp)
		  			/ aexp) * aexp * pow(atexp * lam * fabs(shear_top_k_l), aexp - 1.0) * atexp 
		  			* lam * shear_top_k_l / fabs(shear_top_k_l))) * (1.0 - hsi);
		  
		  d_mu_bot_k_l = (d_shear_bot_k_l * atexp * (tau_y * (atexp * fexp * exp(-atexp * fabs(shear_bot_k_l) * fexp) 
		  				  * shear_bot_k_l / fabs(shear_bot_k_l) * atexp * fabs(shear_bot_k_l) 
		  		           - (1.0 - exp(-atexp * fabs(shear_bot_k_l) * fexp)) * atexp 
		  					* shear_bot_k_l / fabs(shear_bot_k_l)) / pow(atexp * fabs(shear_bot_k_l) , 2.0) 
		  				   * pow(1.0 + pow(atexp * lam * fabs(shear_bot_k_l), aexp), (nexp - 1.0) / aexp) 
		  				  + (mu0 - muinf +tau_y * (1.0 - exp(-atexp * fabs(shear_bot_k_l) * fexp)) 
		  				  / (atexp * fabs(shear_bot_k_l))) * (nexp - 1.0) / aexp * pow(1.0 
		  				 + pow(atexp * lam * fabs(shear_bot_k_l), aexp), (nexp - 1.0 -aexp)
		  				/ aexp) * aexp * pow(atexp * lam * fabs(shear_bot_k_l), aexp - 1.0) * atexp 
		  				* lam * shear_bot_k_l / fabs(shear_bot_k_l))) * (1.0 - hsi);
		  
		  d_int_tan_top_d_shear_top += d_mu_top_k_l * shear_top_k_l * shear_top * gau_qua_0_1[l][0] 
																				/ (dbl)n_divis; 
		  d_int_tan_top_d_shear_top += mu_top_k_l * d_shear_top_k_l * shear_top * gau_qua_0_1[l][0] 
																				/ (dbl)n_divis;
		  d_int_tan_top_d_shear_top += mu_top_k_l * shear_top_k_l * gau_qua_0_1[l][0] / (dbl)n_divis;
		  
	
		  
		  d_int_tan_bot_d_shear_bot += d_mu_bot_k_l * shear_bot_k_l * shear_bot * gau_qua_0_1[l][0] 
		  																				/ (dbl)n_divis; 
		  d_int_tan_bot_d_shear_bot += mu_bot_k_l * d_shear_bot_k_l * shear_bot * gau_qua_0_1[l][0] 
		  																				/ (dbl)n_divis;
		  d_int_tan_bot_d_shear_bot += mu_bot_k_l * shear_bot_k_l * gau_qua_0_1[l][0] / (dbl)n_divis;
		  
		  
		  d_int_norm_top_d_shear_top += d_mu_top_k_l / mu_top_k_l * shear_top * gau_qua_0_1[l][0] 
																				/ (dbl)n_divis;
		  d_int_norm_top_d_shear_top += log(mu_top_k_l) * gau_qua_0_1[l][0] / (dbl)n_divis;
		  
		  
		  d_int_norm_bot_d_shear_bot += d_mu_bot_k_l / mu_bot_k_l * shear_bot * gau_qua_0_1[l][0] 
		  																		/ (dbl)n_divis;
		  d_int_norm_bot_d_shear_bot += log(mu_bot_k_l) * gau_qua_0_1[l][0] / (dbl)n_divis;
		  
		  
	  }
 }
  
 mu_top1 = atexp * (muinf + (mu0 - muinf + tau_y * (1.0 - exp(-atexp * fabs(shear_top) * fexp)) 
		                           / (atexp * fabs(shear_top))) * (pow(1.0 + pow(atexp * lam 
		                        		   * fabs(shear_top) , aexp) , (nexp - 1.0) / aexp)));
 
 mu_bot1 = atexp * (muinf + (mu0 - muinf + tau_y * (1.0 - exp(-atexp * fabs(shear_bot) * fexp)) 
     		/ (atexp * fabs(shear_bot))) * (pow(1.0 + pow(atexp * lam 
     		 * fabs(shear_bot) , aexp) , (nexp - 1.0) / aexp)));
 
 mu_top = (1.0 - hsi) * mu_top1 + hsi * mu2;
 mu_bot = (1.0 - hsi) * mu_bot1 + hsi * mu2;
 
 dmu_top_dF = (- dhsi) * mu_top1 + dhsi * mu2;
 dmu_bot_dF = (- dhsi) * mu_bot1 + dhsi * mu2;
 
 d_mu_top = (atexp * (tau_y * (atexp * fexp * exp(-atexp * fabs(shear_top) * fexp) 
		     * shear_top / fabs(shear_top) * atexp * fabs(shear_top) - (1.0 - exp(-atexp 
		    * fabs(shear_top) * fexp)) * atexp * shear_top / fabs(shear_top)) 
		    / pow(atexp * fabs(shear_top) , 2.0) * pow(1.0 + pow(atexp * lam 
		    * fabs(shear_top), aexp), (nexp - 1.0) / aexp) + (mu0 - muinf +tau_y 
		    * (1.0 - exp(-atexp * fabs(shear_top) * fexp)) / (atexp * fabs(shear_top))) 
			* (nexp - 1.0) / aexp * pow(1.0 + pow(atexp * lam * fabs(shear_top), aexp), (nexp - 1.0 -aexp)
			/ aexp) * aexp * pow(atexp * lam * fabs(shear_top), aexp - 1.0) * atexp 
			* lam * shear_top / fabs(shear_top))) * (1.0 - hsi);
 
 d_mu_bot = (atexp * (tau_y * (atexp * fexp * exp(-atexp * fabs(shear_bot) * fexp) 
		     * shear_bot / fabs(shear_bot) * atexp * fabs(shear_bot) - (1.0 - exp(-atexp 
		    * fabs(shear_bot) * fexp)) * atexp * shear_bot / fabs(shear_bot)) 
		    / pow(atexp * fabs(shear_bot) , 2.0) * pow(1.0 + pow(atexp * lam 
		    * fabs(shear_bot), aexp), (nexp - 1.0) / aexp) + (mu0 - muinf +tau_y 
		    * (1.0 - exp(-atexp * fabs(shear_bot) * fexp)) / (atexp * fabs(shear_bot))) 
			* (nexp - 1.0) / aexp * pow(1.0 + pow(atexp * lam * fabs(shear_bot), aexp), (nexp - 1.0 -aexp)
			/ aexp) * aexp * pow(atexp * lam * fabs(shear_bot), aexp - 1.0) * atexp 
			* lam * shear_bot / fabs(shear_bot))) * (1.0 - hsi);
          ...
          if (pd->e[pg->imtrx][eqn] & T_SOURCE)
            {
             ...
             for (ii = 0; ii < DIM; ii++)
                {
                 source += - tangent1[ii] * LubAux->gradP_tangent[ii] 
								* (pow(shear_top , 2.0) * mu_top - pow(shear_bot , 2.0) * mu_bot);
         
                 source += - tangent1[ii] * LubAux->gradP_tangent[ii] 
                 				* (int_tan_top - int_tan_bot);
                 
                 source += - cross_shear * tangent1[ii] * LubAux->gradP_normal[ii] 
                 					* (shear_top - shear_bot );
                 
                 source += - cross_shear * tangent1[ii] * LubAux->gradP_normal[ii] 
                                  	* (shear_top * log(mu_top) - shear_bot * log(mu_bot));
                 
                 source += - -cross_shear * tangent1[ii] * LubAux->gradP_normal[ii] 
                                     * (int_norm_top - int_norm_bot);
                }
          ...
          if (pd->e[pg->imtrx][eqn] & T_SOURCE)
            {
             ...
             for (ii = 0; ii < DIM; ii++)
                {
                 source += - tangent2[ii] * LubAux->gradP_tangent[ii] 
								* (pow(shear_top , 2.0) * mu_top - pow(shear_bot , 2.0) * mu_bot);
                 
                 source += - tangent2[ii] * LubAux->gradP_tangent[ii] 
                 				* (int_tan_top - int_tan_bot);
                 
                 source += - cross_shear * tangent2[ii] * LubAux->gradP_normal[ii] 
                 					* (shear_top - shear_bot );
          ...
          if (pd->e[pg->imtrx][eqn] & T_SOURCE)
            {
             ...
             source += - phi_i * (mu_top * shear_top - mu_bot * shear_bot);
	                     ...
	                     for (ii = 0; ii < DIM; ii++)
	                        {
	                         ...
	                         source += 0.0;
	                    	 source += -tangent1[ii] * LubAux->gradP_tangent[ii] 
								      * (d_mu_top * pow(shear_top , 2.0) + mu_top * 2.0 * shear_top);
	                         
	                         source += - tangent1[ii] * LubAux->gradP_tangent[ii] 
	                                          				* d_int_tan_top_d_shear_top;
	                         
	                         source += - cross_shear * tangent1[ii] * LubAux->gradP_normal[ii];
	                         
	                         source += - cross_shear * tangent1[ii] * LubAux->gradP_normal[ii] 
	                                      * (log(mu_top) + shear_top * d_mu_top / mu_top);
	                         
	                         source += - -cross_shear * tangent1[ii] * LubAux->gradP_normal[ii] 
	                                                 * d_int_norm_top_d_shear_top;
	                        }
	                      source *= phi_j; 
	                      source *= phi_i* det_J * h3 * wt;
	                      source *= pd->etm[pg->imtrx][eqn][(LOG2_SOURCE)];
	                     ...
	                     for (ii = 0; ii < DIM; ii++)
	                        {
	                         ...
	                         source +=  - -tangent1[ii] * LubAux->gradP_tangent[ii] 
								      * (d_mu_bot * pow(shear_bot , 2.0) + mu_bot * 2.0 * shear_bot);
	                         
	                         source += - -tangent1[ii] * LubAux->gradP_tangent[ii] 
	                                          				* d_int_tan_bot_d_shear_bot;
	                         
	                         source += - -cross_shear * tangent1[ii] * LubAux->gradP_normal[ii];
	                         
	                         source += - -cross_shear * tangent1[ii] * LubAux->gradP_normal[ii] 
	                                      * (log(mu_bot) + shear_bot * d_mu_bot / mu_bot);
	                         
	                         source += - - -cross_shear * tangent1[ii] * LubAux->gradP_normal[ii] 
	                                                 * d_int_norm_bot_d_shear_bot;
	                        }
	                      source *= phi_j; 
	                      source *= phi_i* det_J * h3 * wt;
	                      source *= pd->etm[pg->imtrx][eqn][(LOG2_SOURCE)];
	                  ...
	                  if (pd->e[pg->imtrx][eqn] & T_SOURCE)
	                    {
	                      ...
	                      for (ii = 0; ii < DIM; ii++)
	                         {
	                          source +=  - phi_j * tangent1[ii] * LubAux->gradP_normal[ii] 
									    * ( (shear_top - shear_bot)
									    	+ (shear_top * log(mu_top) - shear_bot * log(mu_bot))
											-(int_norm_top - int_norm_bot));
	                         }
              ...
              for ( j=0; j<ei[pg->imtrx]->dof[var]; j++)
                {
                  ...
                  calculate_lub_q_v_bingham_sens ( H, veloU, veloL, grad_II_P, phi_j, grad_II_phi_j);         
                     ...
                     for (ii = 0; ii < DIM; ii++)
                        {
                         ...
                         source += - tangent1[ii] * LubAux->dgradP_tangent_dP[ii] 
        								* (pow(shear_top , 2.0) * mu_top - pow(shear_bot , 2.0) * mu_bot);
                         
                        source += - tangent1[ii] * LubAux->dgradP_tangent_dP[ii] 
                         				* (int_tan_top - int_tan_bot);
                         
                         source += - cross_shear * tangent1[ii] * LubAux->dgradP_normal_dP[ii] 
                         					* (shear_top - shear_bot );
      	                  ...
      	                  if ( pd->e[pg->imtrx][eqn] & T_ADVECTION )
      	                    {
      	                     for (ii = 0; ii < DIM; ii++)
      	                        {
      	                         advection += tangent1[ii] * (veloU[ii] - veloL[ii]) * LubAux->dgradP_mag_df * phi_j;
      	                        }
      	                     advection *= phi_i * det_J * h3 * wt;
      	                     advection *= pd->etm[pg->imtrx][eqn][(LOG2_ADVECTION)];
      	                    }
      	                     ...
      	                     for (ii = 0; ii < DIM; ii++)
      	                        {
      	                         ...
      	                         source += - tangent1[ii] * LubAux->dgradP_tangent_df[ii] 
      	        								* (pow(shear_top , 2.0) * mu_top - pow(shear_bot , 2.0) * mu_bot);
      	                         
      	                         source += - tangent1[ii] * LubAux->gradP_tangent[ii] 
      	                             	        			* (pow(shear_top , 2.0) * dmu_top_dF - pow(shear_bot , 2.0) * dmu_bot_dF);
      	                         
      	                         source += - tangent1[ii] * LubAux->dgradP_tangent_df[ii] 
      	                         				* (int_tan_top - int_tan_bot);
      	                         
      	                         source += - tangent1[ii] * LubAux->gradP_tangent[ii] 
      	                             	                         	* (dint_tan_top_dF - dint_tan_bot_dF);
      	                         
      	                         source += - cross_shear * tangent1[ii] * LubAux->dgradP_normal_df[ii] 
      	                         					* (shear_top - shear_bot ); 
      	                         ...
      	                         source += - cross_shear * tangent1[ii] * LubAux->gradP_normal[ii] 
      	                             	                 * (shear_top * dmu_top_dF / mu_top - shear_bot * dmu_bot_dF / mu_bot);
      	                         
      	                         source += - -cross_shear * tangent1[ii] * LubAux->dgradP_normal_df[ii] 
      	                                             * (int_norm_top - int_norm_bot);
      	                         
      	                         source += - -cross_shear * tangent1[ii] * LubAux->gradP_normal[ii] 
      	                             	                              * (dint_norm_top_dF - dint_norm_bot_dF);
		                  ...
		                  if (pd->e[pg->imtrx][eqn] & T_SOURCE)
		                    {
		                      ...
		                      for (ii = 0; ii < DIM; ii++)
		                         {
		                          source +=  - phi_j * tangent2[ii] * LubAux->gradP_normal[ii] 
										    * ( (shear_top - shear_bot)
										    	+ (shear_top * log(mu_top) - shear_bot * log(mu_bot))
												-(int_norm_top - int_norm_bot));
		                         }
	                     ...
	                     for (ii = 0; ii < DIM; ii++)
	                        {
	                         ...
	                         source += - tangent2[ii] * LubAux->dgradP_tangent_dP[ii] 
	        								* (pow(shear_top , 2.0) * mu_top - pow(shear_bot , 2.0) * mu_bot);
	                         
	                         source += - tangent2[ii] * LubAux->dgradP_tangent_dP[ii] 
	                         				* (int_tan_top - int_tan_bot);
	                         
	                         source += - cross_shear * tangent2[ii] * LubAux->dgradP_normal_dP[ii] 
	                         					* (shear_top - shear_bot );
	       	                  ...
	       	                  if ( pd->e[pg->imtrx][eqn] & T_ADVECTION )
	       	                    {
	       	                     for (ii = 0; ii < DIM; ii++)
	       	                        {
	       	                         advection += tangent2[ii] * (veloU[ii] - veloL[ii]) * LubAux->dgradP_mag_df * phi_j;
	       	                        }
	       	                     advection *= phi_i * det_J * h3 * wt;
	       	                     advection *= pd->etm[pg->imtrx][eqn][(LOG2_ADVECTION)];
	       	                    }
	       	                     ...
	       	                     for (ii = 0; ii < DIM; ii++)
	       	                        {
	       	                         ...
	       	                         source += - tangent2[ii] * LubAux->dgradP_tangent_df[ii] 
	       	        								* (pow(shear_top , 2.0) * mu_top - pow(shear_bot , 2.0) * mu_bot);
	       	                         ...
	       	                         source += - cross_shear * tangent2[ii] * LubAux->dgradP_normal_df[ii] 
	       	                         					* (shear_top - shear_bot );
                  ...
                  if (pd->e[pg->imtrx][eqn] & T_SOURCE)
                    {
                    	 ...
                    	 source += - (d_mu_top * shear_top + mu_top);
                  ...
                  if (pd->e[pg->imtrx][eqn] & T_SOURCE)
                    {
                      ...
                      source += - -(d_mu_bot * shear_bot + mu_bot);
       	              ...
       	              for ( j=0; j<ei[pg->imtrx]->dof[var]; j++)
       	                {
       	                  ...
       	                  if ( pd->e[pg->imtrx][eqn] & T_ADVECTION )
       	                    {
       	                     for (ii = 0; ii < DIM; ii++)
       	                        {
       	                         advection += H * LubAux->dgradP_mag_df;
       	                        }
       	                     advection *= phi_i * det_J * h3 * wt;
       	                     advection *= pd->etm[pg->imtrx][eqn][(LOG2_ADVECTION)];
       	                    }
                             ...
                             source += - (shear_top * dmu_top_dF - shear_bot * dmu_bot_dF);
       	                     source *= phi_j;
       	                     source *= phi_i * det_J * h3 * wt;
       	                     source *= pd->etm[pg->imtrx][eqn][(LOG2_SOURCE)];
       	                    }
...
/* ************* ASSEMBLE JACOBIAN OF PRESSURE ************ */
                   ...
                   if (pd->e[pg->imtrx][eqn] & T_DIFFUSION)
                     {
                       ...
                       for (ii = 0; ii < dim; ii++)
                          {
                           diffusion += - grad_II_phi_i[ii] * LubAux->dq_df[ii][j];
                          }
  ...
  return(status);
} /* end of assemble_lubrication_bingham */
...
int
assemble_shell_normal( double xi[DIM],   /* Local stu coordinates */
                       const Exo_DB *exo )
{
 /*
...
int
assemble_shell_curvature( double xi[DIM],   /* Local stu coordinates */
                          const Exo_DB *exo )
{
 /*
  * Integers and indices
          ...
          for (b = 0; b < dim; b++)
             {
              for (j = 0; j < ei[pg->imtrx]->dof[var]; j++)
                 {
                  d_dnormal_dxi_dx[a][0][p][j] +=   dt0_dx[b][p][j] * fv->grad_n[b][a]
                                                  + t0[b] * d_grad_n_dx[b][a][p][j];
                  d_dnormal_dxi_dx[a][1][p][j] +=   dt1_dx[b][p][j] * fv->grad_n[b][a]
                                                  + t1[b] * d_grad_n_dx[b][a][p][j];
                 }
             }
         }
     }
          ...
          for (b = 0; b < dim; b++)
             {
              for (j = 0; j < ei[pg->imtrx]->dof[var]; j++)
                 {
                  d_dnormal_dxi_dnormal[a][0][p][j] +=   dt0_dnormal[b][p][j] * fv->grad_n[b][a]
                                                       + t0[b] * d_grad_n_dnormal[b][a][p][j];
                  d_dnormal_dxi_dnormal[a][1][p][j] +=   dt1_dnormal[b][p][j] * fv->grad_n[b][a]
                                                       + t1[b] * d_grad_n_dnormal[b][a][p][j];
                 }
             }
         }
     }
      ...
      for (a = 0; a < dim; a++)
         {
          for (j = 0; j < ei[pg->imtrx]->dof[var]; j++)
             {
              dcurv0_dx[b][j] -=   t0[a] * d_dnormal_dxi_dx[a][0][b][j]
                                 + dt0_dx[a][b][j] * dnormal_dxi[a][0];
              dcurv1_dx[b][j] -=   t1[a] * d_dnormal_dxi_dx[a][1][b][j]
                                 + dt1_dx[a][b][j] * dnormal_dxi[a][1];
             }
         }
     }
      ...
      for (a = 0; a < dim; a++)
         {
          for (j = 0; j < ei[pg->imtrx]->dof[var]; j++)
             {
              dcurv0_dnormal[b][j] -=   t0[a] * d_dnormal_dxi_dnormal[a][0][b][j]
                                      + dt0_dnormal[a][b][j] * dnormal_dxi[a][0];
              dcurv1_dnormal[b][j] -=   t1[a] * d_dnormal_dxi_dnormal[a][1][b][j]
                                      + dt1_dnormal[a][b][j] * dnormal_dxi[a][1];
             }
         }
     }
...
int
assemble_shell_mesh(double time,   /* Time */
  ...
  */
  ...
  if (pd->e[pg->imtrx][R_TFMP_MASS]) {
	...
	if (pd->TimeIntegration == TRANSIENT ) {
	  ...
	  for (l = 0; l<DIM; l++) {
	    for (i = 0; i<ei[pg->imtrx]->dof[MESH_DISPLACEMENT1]; i++) {
	      d2h_dtime_dmesh[k][i] -= fv->n[l]*delta(k,l)*bf[MESH_DISPLACEMENT1]->phi[i]*(1.0+2*tt)/delta_t;
	      ...
	      d2h_dtime_dnormal[k][i] -= fv->d[k]*delta(k,l)*bf[SHELL_NORMAL1]->phi[i]*(1.0+2.0*tt)/delta_t;
	    }
	  }
	  for(i = 0; i<ei[pg->imtrx]->dof[MESH_DISPLACEMENT1]; i++) {
          ...
          if (pd->e[pg->imtrx][eqn] & T_DIFFUSION)
            {
             for (a = 0; a < dim; a++)
                {
                 diffusion -=   t0[a] * grad_phi_i[a] * N11
                              + t1[a] * grad_phi_i[a] * N12;

                 diffusion -=   K1 * (t0[a] * grad_phi_i[a] * M11 +
                                      t1[a] * grad_phi_i[a] * M12);
                }
             diffusion *= det_J * wt * h3 * pd->etm[pg->imtrx][eqn][(LOG2_DIFFUSION)];
            }
          ...
          if (pd->e[pg->imtrx][eqn] & T_DIFFUSION)
            {
             for (a = 0; a < dim; a++)
                {
                 diffusion -=   t0[a] * grad_phi_i[a] * N12
                              + t1[a] * grad_phi_i[a] * N22;

                 diffusion -=   K2 * (t0[a] * grad_phi_i[a] * M12 +
                                      t1[a] * grad_phi_i[a] * M22);
                }
             diffusion *= det_J * wt * h3 * pd->etm[pg->imtrx][eqn][(LOG2_DIFFUSION)];
            }
          ...
          if (pd->e[pg->imtrx][eqn] & T_DIFFUSION)
            {
             ...
             for (a = 0; a < dim; a++)
                {
                 diffusion +=   t0[a] * grad_phi_i[a] * (M11 + M12)
                              + t1[a] * grad_phi_i[a] * (M12 + M22);
                }
                        ...
                        for (a = 0; a < dim; a++)
                           {
                             diffusion -= (  t0[a] * grad_phi_i[a] * dTT_dx[0][0][b][j]
                                           ...
                                           + K1 * t1[a] * grad_phi_i[a] * dM_dx[0][1][b][j] ) * det_J;
                             diffusion -= (  t0[a] * d_grad_phi_i_dmesh[a][b][j] * N11
                                           + t1[a] * d_grad_phi_i_dmesh[a][b][j] * N12
                                           + K1 * t0[a] * d_grad_phi_i_dmesh[a][b][j] * M11
                                           + K1 * t1[a] * d_grad_phi_i_dmesh[a][b][j] * M12 ) * det_J;
                             diffusion -= (  dt0_dx[a][b][j] * grad_phi_i[a] * N11
                                           + dt1_dx[a][b][j] * grad_phi_i[a] * N12
                                           + K1 * dt0_dx[a][b][j] * grad_phi_i[a] * M11
                                           + K1 * dt1_dx[a][b][j] * grad_phi_i[a] * M12) * det_J;
                             diffusion -= (  t0[a] * grad_phi_i[a] * N11
                                           + t1[a] * grad_phi_i[a] * N12
                                           + K1 * t0[a] * grad_phi_i[a] * M11
                                           + K1 * t1[a] * grad_phi_i[a] * M12) * fv->dsurfdet_dx[b][j];
                           }
                        diffusion *= wt * h3;
                        diffusion *= pd->etm[pg->imtrx][eqn][(LOG2_DIFFUSION)];
                       }
                        ...
                        for (a = 0; a < dim; a++)
                           {
                             diffusion -= (  t0[a] * grad_phi_i[a] * dTT_dx[0][1][b][j]
                                           ...
                                           + K2 * t1[a] * grad_phi_i[a] * dM_dx[1][1][b][j]) * det_J;
                             diffusion -= (  t0[a] * d_grad_phi_i_dmesh[a][b][j] * N12
                                           + t1[a] * d_grad_phi_i_dmesh[a][b][j] * N22
                                           + K2 * t0[a] * d_grad_phi_i_dmesh[a][b][j] * M12
                                           + K2 * t1[a] * d_grad_phi_i_dmesh[a][b][j] * M22) * det_J;
                             diffusion -= (  dt0_dx[a][b][j] * grad_phi_i[a] * N12
                                           + dt1_dx[a][b][j] * grad_phi_i[a] * N22
                                           + K2 * dt0_dx[a][b][j] * grad_phi_i[a] * M12
                                           + K2 * dt1_dx[a][b][j] * grad_phi_i[a] * M22) * det_J;
                             diffusion -= (  t0[a] * grad_phi_i[a] * N12
                                           + t1[a] * grad_phi_i[a] * N22
                                           + K2 * t0[a] * grad_phi_i[a] * M12
                                           + K2 * t1[a] * grad_phi_i[a] * M22) * fv->dsurfdet_dx[b][j];
                           }
                        diffusion *= wt * h3;
                        diffusion *= pd->etm[pg->imtrx][eqn][(LOG2_DIFFUSION)];
                       }
                 ...
                 for ( j=0; j < ei[pg->imtrx]->dof[var]; j++)
                    {
                     ...
                     if (pd->e[pg->imtrx][eqn] & T_DIFFUSION)
                       {
                        diffusion += phi_i * (  K1 * dTT_dx[0][0][b][j]
                                              + K2 * dTT_dx[1][1][b][j] ) * det_J;
                        ...
                        for (a = 0; a < dim; a++)
                           {
                            diffusion += (  dt0_dx[a][b][j] * grad_phi_i[a] * (M11 + M12)
                                          ...
                                          + t1[a] * grad_phi_i[a] * (dM_dx[0][1][b][j] + dM_dx[1][1][b][j])  ) * det_J;

                            diffusion += (  t0[a] * grad_phi_i[a] * (M11 + M12)
                                          + t1[a] * grad_phi_i[a] * (M12 + M22) ) * fv->dsurfdet_dx[b][j];
                           }
                 ...
                 for ( j=0; j < ei[pg->imtrx]->dof[var]; j++)
                    {
                     ...
                     if (pd->e[pg->imtrx][eqn] & T_DIFFUSION)
                       {
                        diffusion += phi_i * (  K1 * dTT_dnormal[0][0][b][j]
                                              + K2 * dTT_dnormal[1][1][b][j] );

                        for (a = 0; a < dim; a++)
                           {
                            diffusion += (  dt0_dnormal[a][b][j] * grad_phi_i[a] * (M11 + M12)
                 ...
                 const Exo_DB *exo)
{
  ...
  if ( af->Assemble_Residual ) {
    ...
    /* Loop over DOF (i) */   
    for(i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
      ShellBF(eqn, i, &phi_i, grad_phi_i, gradII_phi_i,
  ...
  if (af->Assemble_Jacobian) {
    ...
    for ( i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
      ...
      for ( j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
        // Load basis functions
        ShellBF( var, j, &phi_j, grad_phi_j, gradII_phi_j,
                 d_gradII_phi_j_dmesh, n_dof[MESH_DISPLACEMENT1], dof_map );
     ...
     */
      ...
      for ( j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
        ...
        if ( T_DIFFUSION ) {
          ...
          // phi_i*D*krd*del^2(P)
          diff += D*Krd*gradphi_i_dot_gradphi_j
                  + D*dKrd_dS*phi_j*gradS_dot_gradphi_i;
          diff *= dA;
        }
        diff *= etm_diff_eqn;
      ...
      if( mp->FSIModel == FSI_SHELL_ONLY_MESH) {
        for (l = 0; l<pd->Num_Dim; l++) {
          ...
          for ( j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
            ...
            if ( T_MASS ) {
              if (mass_lumping == 1) {
                mass += phi_i*dh_dmesh[l][j]*delta(i,j)*(*esp_dot->tfmp_sat[i])
                        *wt*h3*det_J;

                mass += phi_i*h*delta(i,j)*(*esp_dot->tfmp_sat[i])
              ...
              mass += phi_i*S*d2h_dtime_dmesh[l][j]
                      *wt*h3*det_J;
            ...
            if ( T_ADVECTION ) {
              //convective term
              adv += 3.0*h*h/12.0/mu_l*dh_dmesh[l][j]*S*gradP_dot_gradphi_i
                     *wt*h3*det_J;

              adv += h*h*h/12.0/mu_l*S*(gradP_dot_dgrad_phi_i_dmesh_lj
                                        + dgradP_dmesh_lj_dot_gradphi_i)
                     *wt*h3*det_J;

              adv += h*h*h/12.0/mu_l*S*gradP_dot_gradphi_i
              ...
              adv += phi_i*dh_dmesh[l][j]*veloAVG_dot_gradS
                     * wt*h3*det_J;
              adv += phi_i*h*dveloAVG_dot_gradS_dmesh
                     * wt*h3*det_J;
              adv += phi_i*h*veloAVG_dot_gradS
                     * wt*h3*d_det_J_dmeshkj[l][j];
            ...
            if ( T_DIFFUSION ) {
              ...
              diff += D*Krd*(gradS_dot_dgrad_phi_i_dmesh_lj
                             + dgradS_dmesh_lj_dot_gradphi_i)
                      *wt*h3*det_J;
    ...
    for ( i = 0; i < ei[pg->imtrx]->dof[eqn]; i++) {
          ...
          if ( T_MASS ) {
            if (mp->tfmp_density_model == CONSTANT) {
              ...
              if (mass_lumping == 1) {
                ...
                mass += (phi_i
                         *(h*(1.0-S)*drho_g_dP*phi_j*((1.0+2.0*tt)/delta_t)
                           + drho_g_dP*phi_j*(1.0-S)*dh_dtime
      ...
      if (pd->v[pg->imtrx][var]) /* var = TFMP_SAT */ {
        ...
        for ( j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
          ...
          if( T_MASS ) {
            ...
            if (mass_lumping == 1) {
              mass += (phi_i
                 *(-h*phi_j*drho_g_dP*(*esp_dot->tfmp_pres[i])
      ...
      for (l = 0; l<DIM; l++) {
        ...
        if (pd->v[pg->imtrx][var]) /* var = MESH_DISPLACEMENT1,2,3 */ {
          ...
          for ( j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
            // Load basis functions
            ShellBF( var, j, &phi_j, grad_phi_j,
                     gradII_phi_j, d_gradII_phi_j_dmesh, n_dof[MESH_DISPLACEMENT1], dof_map );
            ...
            if ( T_MASS ) {
              ...
              if (mass_lumping == 1) {
                ...
                mass += phi_i*dh_dmesh[l][j]*(1.0-S)*drho_g_dP*delta(i,j)*(*esp_dot->tfmp_pres[i])
                  *wt*h3*det_J;
                mass += phi_i*h*(1.0-S)*drho_g_dP*delta(i,j)*(*esp_dot->tfmp_pres[i])
                  *wt*h3*d_det_J_dmeshkj[l][j];

                mass += phi_i*rho_g*(1.0-S)*d2h_dtime_dmesh[l][j]
                  *wt*h3*det_J;
                mass += phi_i*rho_g*(1.0-S)*dh_dtime
                  *wt*h3*d_det_J_dmeshkj[l][j];

                mass += -phi_i*drho_g_dP*dh_dmesh[l][j]*delta(i,j)*(*esp_dot->tfmp_sat[i])
                  *wt*h3*det_J;
                mass += phi_i*rho_g*h*delta(i,j)*(*esp_dot->tfmp_sat[i])
                  *wt*h3*d_det_J_dmeshkj[l][j];

              } else {
                mass += (phi_i
                   *(dh_dmesh[l][j]*(1.0-S)*drho_g_dP*(*esp_dot->tfmp_pres[i])
                ...
                mass += (phi_i
                   * (h*(1.0-S)*drho_g_dP*fv_dot->tfmp_pres
                     + rho_g*(1.0-S)*dh_dtime
                     -rho_g*h*fv_dot->tfmp_sat
                      )
                   )
                  * wt*h3*d_det_J_dmeshkj[l][j];
              }
            ...
            if ( T_ADVECTION ) {
              ...
              adv += gradP_dot_gradphi_i*3.0*h*h*dh_dmesh[l][j]*rho_g/12.0/mu_g*Krg
                *wt*h3*det_J;

              adv += (gradP_dot_dgrad_phi_i_dmesh_lj + dgradP_dmesh_lj_dot_gradphi_i)
                * h*h*h*rho_g/12.0/mu_g*Krg
                * wt*h3*det_J;

              adv += gradP_dot_gradphi_i*h*h*h*rho_g/12.0/mu_g*Krg
                * wt*h3*d_det_J_dmeshkj[l][j];

              // plate motion terms

              adv += -dh_dmesh[l][j]*rho_g*(1.0 - S)*veloAVG_dot_gradphi_i
                  * wt*h3*det_J;
              adv += -h*rho_g*(1.0-S)*dveloAVG_dot_gradphi_i_dmesh
                  * wt*h3*det_J;
              adv += -h*rho_g*(1.0 - S)*veloAVG_dot_gradphi_i
                  * wt*h3*d_det_J_dmeshkj[l][j];
        ...
        if (pd->v[pg->imtrx][var]) /* var = SHELL_NORMAL1,2,3 */ {
          ...
          for ( j = 0; j < ei[pg->imtrx]->dof[var]; j++) {
            ...
            if ( T_MASS ) {
              if (mass_lumping == 1) {
                mass += phi_i*dh_dnormal[l][j]*(1.0-S)*drho_g_dP*delta(i,j)*(*esp_dot->tfmp_pres[i]);
                mass += phi_i*rho_g*(1.0-S)*d2h_dtime_dnormal[l][j];
                mass += -phi_i*drho_g_dP*dh_dnormal[l][j]*delta(i,j)*(*esp_dot->tfmp_sat[i]);
            ...
            if ( T_ADVECTION ) {
              adv += gradP_dot_gradphi_i*3.0*h*h*dh_dnormal[l][j]*rho_g/12.0/mu_g*Krg;
...
int
assemble_shell_lubrication(double time,   /* Time */
     double tt,        /* Time stepping parameter */
     double delta_t,      /* Time step size */
     double xi[DIM],      /* Local stu coordinates */
     const Exo_DB *exo)
    ...
    for (int i=0; i<ei[pg->imtrx]->dof[eqn]; i++) {
      ...
      for (int l = 0; l<pd->Num_Dim; l++) {
        ...
        for (int j=0; j<ei[pg->imtrx]->dof[var]; j++) {
          ...
          if (etm_mass_eqn > 0) {
            mass += d2h_dtime_dmesh[l][j]*phi_i
                    *wt*h3*det_J;
            mass += dh_dtime*phi_i
                    *d_det_J_dmeshkj[l][j];
          }
          if (etm_adv_eqn > 0) {
            adv += (gradP_dot_dgrad_phi_i_dmesh_lj + dgradP_dmesh_lj_dot_gradphi_i)
                   *h*h*h/12.0/mu_l
                   *h3*wt*det_J;
            adv += gradP_dot_gradphi_i*3.0*h*h*dh_dmesh[l][j]/12.0/mu_l
                   *h3*wt*det_J;
            adv += gradP_dot_gradphi_i*h*h*h/12.0/mu_l
                   *wt*h3*d_det_J_dmeshkj[l][j];
          }
