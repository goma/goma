 ...
 *
 * Function to calculate flow rate per unit width (q) and average velocity (v)
 * in lubrication flow of bingham-carreau-yasuda liquid
 * 
 * 
 * bikash dey bikash.dey@utah.edu
 * 
 ******************************************************************************/
{
  int i, j, k, l, n_divis;
  ...
  dbl grad_P[DIM], grad_II_P[DIM];
  dbl veloL[DIM], veloU[DIM];
  dbl mu0, nexp, muinf, lam, aexp, tau_y, fexp, atexp, atexp1;
  dbl shear_top, shear_bot, cross_shear, gradP_mag;
  dbl H, H_U, dH_U_dtime, H_L, dH_L_dtime, dH_U_ddH;
  dbl dH_U_dX[DIM],dH_L_dX[DIM], dH_U_dp;
  dbl gradP_tangent[DIM], gradP_normal[DIM], gradP_normal_init[DIM];
  ...
  dbl epsilon = 1.0e-12;
  dbl int_tan_top, int_tan_bot, int_norm_top, int_norm_bot, shear_top_k_l, shear_bot_k_l, mu_top_k_l, mu_bot_k_l, mu_top, mu_bot, mu_top_k_l1, mu_bot_k_l1, mu_top1, mu_bot1;
      ...
      nexp = gn->nexp;
       ...
       for (k = 0; k<n_divis; k++)
       {
     	  for (l = 0; l<100; l++)
     	  {
     		  ...
     		  mu_top_k_l1 = atexp * (muinf + (mu0 - muinf + tau_y * (1.0 - exp(-atexp * fabs(shear_top_k_l) * fexp)) 
     				                        / (atexp * fabs(shear_top_k_l)))
     				   * (pow(1.0 + pow(atexp * lam * fabs(shear_top_k_l) , aexp) , (nexp - 1.0) / aexp)));
     		  
     		  mu_top_k_l = (1.0 - hsi) * mu_top_k_l1 + hsi * mu2;
     		  
     		  mu_bot_k_l1 = atexp * (muinf + (mu0 - muinf + tau_y * (1.0 - exp(-atexp * fabs(shear_bot_k_l) * fexp)) 
                                           / (atexp * fabs(shear_bot_k_l))
 				) * (pow(1.0  + pow(atexp * lam * fabs(shear_bot_k_l) , aexp) , (nexp - 1.0) / aexp)));
     		  
     		  mu_bot_k_l = (1.0 - hsi) * mu_bot_k_l1 + hsi * mu2;
     		  
     		  int_tan_top += pow(mu_top_k_l * shear_top_k_l , 2.0) * shear_top * gau_qua_0_1[l][0] 
																		/ (dbl)n_divis;  
     		  
     		  int_tan_bot += pow(mu_bot_k_l * shear_bot_k_l , 2.0) * shear_bot * gau_qua_0_1[l][0] 
														/ (dbl)n_divis;
     		  
     		  int_norm_top += mu_top_k_l * shear_top_k_l * shear_top * gau_qua_0_1[l][0] / (dbl)n_divis;
     		  
     		  int_norm_bot += mu_bot_k_l * shear_bot_k_l * shear_bot * gau_qua_0_1[l][0] / (dbl)n_divis; 
     		  
     	  }
       }
       
 	  mu_top1 = atexp * (muinf + (mu0 - muinf + tau_y * (1.0 - exp(-atexp * fabs(shear_top) * fexp)) 
 			                    / (atexp * fabs(shear_top)))
 			    * (pow(1.0 + pow(atexp * lam * fabs(shear_top) , aexp) , (nexp - 1.0) / aexp)));
 	  
 	  mu_top = (1.0 - hsi) * mu_top1 + hsi * mu2;
 	  
 	  mu_bot1 = atexp * (muinf + (mu0 - muinf + tau_y * (1.0 - exp(-atexp * fabs(shear_bot) * fexp)) 
                             / (atexp * fabs(shear_bot))
 					) * (pow(1.0  + pow(atexp * lam * fabs(shear_bot) , aexp) , (nexp - 1.0) / aexp)));
       ...
       for (i = 0; i < DIM; i++)
          {
           ...
           q[i] += 1.0 / (gradP_mag) * (veloU[i] * mu_top * shear_top 
        		           - veloL[i] * mu_bot * shear_bot);
           

           
           q[i] += -1.0 / pow(gradP_mag , 2.0) * gradP_tangent[i] / 2.0 
        		       * (pow(mu_top , 2.0) * pow(shear_top , 3.0) - pow(mu_bot , 2.0) 
        		    		   * pow(shear_bot , 3.0) - (int_tan_top - int_tan_bot)); 

           
           q[i] += -1.0 / pow(gradP_mag  , 2.0) * gradP_normal[i] * cross_shear 
        		     * (mu_top * pow(shear_top , 2.0) - mu_bot * pow(shear_bot , 2.0)
...
void
calculate_lub_q_v_bingham_sens (     
		                             double H,
                                     double veloU[DIM],
                                     double veloL[DIM],
                                     double grad_II_P[DIM],
 ...
 *
 * Function to calculate flow rate per unit width (q) and average velocity (v)
 * of bingham liquid's sensitivities w.r.t. pretty much anything you can think of
 *
 *
 * bikash dey bikash.dey@ttu.edu
 *
 ******************************************************************************/
{
  int i, j;
  ...
  dbl dq_dshear_top[DIM], dq_dshear_bot[DIM], dq_dcross_shear[DIM];
  dbl dv_avg_dP[DIM];
  dbl dv_avg_dshear_top[DIM], dv_avg_dshear_bot[DIM], dv_avg_dcross_shear[DIM];
  dbl dq_dF[DIM], dv_avg_dF[DIM];
  dbl dgradP_mag_dP;
  dbl dgradP_tangent_dP[DIM], dgradP_normal_dP[DIM], dgradP_normal_init_dP[DIM];
  dbl dgradP_mag_dF;
  dbl dgradP_tangent_dF[DIM], dgradP_normal_dF[DIM], dgradP_normal_init_dF[DIM];
  ...
  dbl GRAV[DIM];
    ...
    dbl rho, rho1, rho2, drho, mu2, hsi, dhsi;
  ...
  for (i = 0; i < DIM; i++)
    ...
    if (fabs(GRAV[0]) < epsilon && fabs(GRAV[1]) < epsilon && fabs(GRAV[2]) < epsilon)
    {}
    else
    {
    for (i = 0; i < DIM; i++)
       {
    	for (j = 0; j < DIM; j++)
    	{
        dgradP_mag_dF +=  -grad_II_P[i] * (GRAV[j] * delta(i, j) - GRAV[j] * fv->snormal[i] * fv->snormal[j]); 
    	}
       }
    dgradP_mag_dF *=   drho / gradP_mag;
    }
    ...
    else
    {
    	for (i = 0; i < DIM; i++)
    	{
        	for (j = 0; j < DIM; j++)
        	{
        		dgradP_tangent_dF[i] +=  - (GRAV[j] * delta(i, j) - GRAV[j] * fv->snormal[i] * fv->snormal[j]); 
        	}
    		dgradP_tangent_dF[i] =  dgradP_tangent_dF[i] * drho / gradP_mag  -
    				grad_II_P[i] * dgradP_mag_dF/pow(gradP_mag, 2);
    	}
    }
    ...
    for (i = 0; i < DIM; i++)
       {
        dgradP_normal_dP[i] =  dgradP_normal_init_dP[i]
                            - gradP_tangent[i] * (2. * grad_II_P[2] * grad_phi_j[2] / pow(gradP_mag, 2)
                                                - 2. * grad_II_P[2] * grad_II_P[2] 
												* dgradP_mag_dP / pow(gradP_mag, 3) )
                            - (grad_II_P[2] * grad_II_P[2]) * dgradP_tangent_dP[i];
       }


    dgradP_normal_init_dF[0] =  -dgradP_tangent_dF[1];
    dgradP_normal_init_dF[1] =   dgradP_tangent_dF[0];
    dgradP_normal_init_dF[2] =   dgradP_tangent_dF[2];
    
    for (i = 0; i < DIM; i++)
    {
    	dgradP_normal_dF[i] =  0.0;
    }
    ...
    for (i = 0; i < DIM; i++)
       {
        dgradP_normal_dF[i] =  dgradP_normal_init_dF[i]
                            - gradP_tangent[i] * (2. * grad_II_P[2] * (-drho * (GRAV[2] 
						- fv->snormal[2] * (GRAV[0] * fv->snormal[0] + GRAV[1] * fv->snormal[1] + GRAV[2] * fv->snormal[2]) )) / pow(gradP_mag, 2)
                                                - 2. * grad_II_P[2] * grad_II_P[2] 
												* dgradP_mag_dF / pow(gradP_mag, 3) )
                            - (grad_II_P[2] * grad_II_P[2]) * dgradP_tangent_dF[i];
       }
  ...
  /******* CALCULATE FLOW RATE SENSITIVITIES ***********/
    	 ...
    	 for (k = 0; k<n_divis; k++)
    	 {
    		  for (l = 0; l<100; l++)
    		  {
    			  ...
    			  d_mu_bot_k_l = (d_shear_bot_k_l * atexp * (tau_y * (atexp * fexp * exp(-atexp * fabs(shear_bot_k_l) * fexp) 
    			  		     * shear_bot_k_l / fabs(shear_bot_k_l) * atexp * fabs(shear_bot_k_l) - (1.0 - exp(-atexp 
    			  		    * fabs(shear_bot_k_l) * fexp)) * atexp * shear_bot_k_l / fabs(shear_bot_k_l)) 
    			  		    / pow(atexp * fabs(shear_bot_k_l) , 2.0) * pow(1.0 + pow(atexp * lam 
    			  		    * fabs(shear_bot_k_l), aexp), (nexp - 1.0) / aexp) + (mu0 - muinf +tau_y 
    			  		    * (1.0 - exp(-atexp * fabs(shear_bot_k_l) * fexp)) / (atexp * fabs(shear_bot_k_l))) 
    			  			* (nexp - 1.0) / aexp * pow(1.0 + pow(atexp * lam * fabs(shear_bot_k_l), aexp), (nexp - 1.0 -aexp)
    			  ...
    			  d_int_tan_top_d_shear_top += 2.0 * mu_top_k_l * d_mu_top_k_l * pow(shear_top_k_l , 2.0) 
    			                                * shear_top * gau_qua_0_1[l][0] / (dbl)n_divis; 
    			  
    			  d_int_tan_top_d_shear_top += 2.0 * shear_top_k_l * d_shear_top_k_l * pow(mu_top_k_l , 2.0) 
                                                 * shear_top * gau_qua_0_1[l][0] / (dbl)n_divis; 
    			  
    			  d_int_tan_top_d_shear_top += pow(mu_top_k_l * shear_top_k_l , 2.0) * gau_qua_0_1[l][0] 
																		/ (dbl)n_divis;
    			  
    			  
    			  
    			  d_int_tan_bot_d_shear_bot += 2.0 * mu_bot_k_l * d_mu_bot_k_l * pow(shear_bot_k_l , 2.0) 
    			      	* shear_bot * gau_qua_0_1[l][0] / (dbl)n_divis; 
    			      			  
    			  d_int_tan_bot_d_shear_bot += 2.0 * shear_bot_k_l * d_shear_bot_k_l * pow(mu_bot_k_l , 2.0) 
    			                          * shear_bot * gau_qua_0_1[l][0] / (dbl)n_divis; 
    			      			  
    			  d_int_tan_bot_d_shear_bot += pow(mu_bot_k_l * shear_bot_k_l , 2.0) * gau_qua_0_1[l][0] 
    			  										/ (dbl)n_divis;
    			  
    			  
    			  d_int_norm_top_d_shear_top += d_mu_top_k_l * shear_top_k_l * shear_top * gau_qua_0_1[l][0] 
    																					/ (dbl)n_divis;
    			  d_int_norm_top_d_shear_top += mu_top_k_l * d_shear_top_k_l * shear_top * gau_qua_0_1[l][0] 
																				/ (dbl)n_divis;
    			  
    			  d_int_norm_top_d_shear_top += mu_top_k_l * shear_top_k_l * gau_qua_0_1[l][0] 
    			  														/ (dbl)n_divis;
    			  
    			  d_int_norm_bot_d_shear_bot += d_mu_bot_k_l * shear_bot_k_l * shear_bot * gau_qua_0_1[l][0] 
    			      													/ (dbl)n_divis;
    			  d_int_norm_bot_d_shear_bot += mu_bot_k_l * d_shear_bot_k_l * shear_bot * gau_qua_0_1[l][0] 
    			  						/ (dbl)n_divis;
    			      			  
    			  d_int_norm_bot_d_shear_bot += mu_bot_k_l * shear_bot_k_l * gau_qua_0_1[l][0] 
    			      			  			/ (dbl)n_divis;
    			  
    			  
    			  dint_tan_top_dF += 2.0 * mu_top_k_l * dmu_top_k_l_dF * pow(shear_top_k_l , 2.0) * shear_top * gau_qua_0_1[l][0] 
													/ (dbl)n_divis;  
    			  
    			  
    			  
    			  dint_tan_bot_dF += 2.0 * mu_bot_k_l * dmu_bot_k_l_dF * pow(shear_bot_k_l , 2.0) * shear_bot * gau_qua_0_1[l][0] 
    			  													/ (dbl)n_divis; 
    			  
    			  dint_norm_top_dF += dmu_top_k_l_dF * shear_top_k_l * shear_top * gau_qua_0_1[l][0] / (dbl)n_divis;
    			  
    			  dint_norm_bot_dF += dmu_bot_k_l_dF * shear_bot_k_l * shear_bot * gau_qua_0_1[l][0] / (dbl)n_divis; 
         ...
         for (i = 0; i < DIM; i++)
            {
        	 ...
        	 dq_dshear_top[i] += 1.0 / (gradP_mag ) * veloU[i] * (d_mu_top * shear_top + mu_top);
        	 
        	            
        	 dq_dshear_top[i] += -1.0 / pow(gradP_mag , 2.0) * gradP_tangent[i] / 2.0 
        	         		       * (2.0 * pow(mu_top , 1.0) * d_mu_top * pow(shear_top , 3.0) 
        	         		    + pow(mu_top , 2.0) * 3.0 * pow(shear_top , 2.0) - d_int_tan_top_d_shear_top ); 

        	            
        	 dq_dshear_top[i] += -1.0 / pow(gradP_mag  , 2.0) * gradP_normal[i] * cross_shear 
        	         		     * (d_mu_top * pow(shear_top , 2.0) + mu_top * 2.0 * pow(shear_top , 1.0)
        	          		           - d_int_norm_top_d_shear_top ); 
        	 
 
        	            
        	 dq_dshear_top[i] *= phi_j;
        	 

        	 dv_avg_dshear_top[i] = dq_dshear_top[i] / H;


        /*Evaluate lubrication flowrate and average velocity sensitivity w.r.t. bottom wall shear rate */
        	 
        	 dq_dshear_bot[i] = 0.0;

        	 dq_dshear_bot[i] += 1.0 / (gradP_mag ) * veloL[i] * (d_mu_bot * shear_bot + mu_bot);
        	 

        	 dq_dshear_bot[i] += -1.0 / pow(gradP_mag , 2.0) * gradP_tangent[i] / 2.0 
        			 * (2.0 * mu_bot * d_mu_bot * pow(shear_bot , 3.0) + pow(mu_bot , 2.0) 
        					 * 3.0 * pow(shear_bot , 2.0) - d_int_tan_bot_d_shear_bot ); 

        	 dq_dshear_bot[i] += -1.0 / pow(gradP_mag , 2.0) * gradP_normal[i] 
											* cross_shear * (d_mu_bot * pow(shear_bot , 2.0) 
        	                      + mu_bot * 2.0 * shear_bot - d_int_norm_bot_d_shear_bot ); 
        	 

        	 dq_dshear_bot[i] *= -phi_j;

        	 dv_avg_dshear_bot[i] = dq_dshear_bot[i] / H;



         /*Evaluate lubrication flowrate and average velocity sensitivity w.r.t. cross stream shear stress */
        	 
        	  
        	 dq_dcross_shear[i] = 0.0;
        	 
        	 dq_dcross_shear[i] =     - phi_j / pow(gradP_mag , 2) * gradP_normal[i]  *
                (  mu_top * pow(shear_top , 2) - mu_bot * pow(shear_bot, 2) 
                            -(int_norm_top - int_norm_bot));
        	
             
             dv_avg_dcross_shear[i] = dq_dcross_shear[i] / H;


         /*Evaluate lubrication flowrate and average velocity sensitivity w.r.t. pressure */
     
        	 dq_dP[i] = 0.0;
        	 


             dq_dP[i] += -1.0 / pow(gradP_mag , 2.0) * dgradP_mag_dP * (veloU[i] * mu_top * shear_top 
          		           - veloL[i] * mu_bot * shear_bot);
             
             
             dq_dP[i] += 2.0 / pow(gradP_mag , 3.0) * dgradP_mag_dP  * gradP_tangent[i] / 2.0 
          		       * (pow(mu_top , 2.0) * pow(shear_top , 3.0) - pow(mu_bot , 2.0) 
          		    		   * pow(shear_bot , 3.0) - (int_tan_top - int_tan_bot)); 
             
             dq_dP[i] += -1.0 / pow(gradP_mag , 2.0) * dgradP_tangent_dP[i] / 2.0 
                       		       * (pow(mu_top , 2.0) * pow(shear_top , 3.0) - pow(mu_bot , 2.0) 
                       		    		   * pow(shear_bot , 3.0) - (int_tan_top - int_tan_bot)); 

             
             dq_dP[i] += 2.0 / pow(gradP_mag , 3.0) * dgradP_mag_dP  * gradP_normal[i] * cross_shear 
          		     * (mu_top * pow(shear_top , 2.0) - mu_bot * pow(shear_bot , 2.0)
           		           - (int_norm_top - int_norm_bot)); 
        	 
        	 
             dq_dP[i] += -1.0 / pow(gradP_mag , 2.0) * dgradP_normal_dP[i] * cross_shear 
                       		     * (mu_top * pow(shear_top , 2.0) - mu_bot * pow(shear_bot , 2.0)
                        		           - (int_norm_top - int_norm_bot)); 
             
 
                        		                                                         		                                                              
             dv_avg_dP[i] =	dq_dP[i] / H;   
             
             /*Evaluate lubrication flowrate and average velocity sensitivity w.r.t. LEVEL SET */
             
             dq_dF[i] = 0.0;
             
             
             dq_dF[i] += -1.0 / pow(gradP_mag , 2.0) * dgradP_mag_dF * (veloU[i] * mu_top * shear_top 
          		           - veloL[i] * mu_bot * shear_bot);
             
             dq_dF[i] += 1.0 / (gradP_mag) * (veloU[i] * dmu_top_dF * shear_top 
                       		           - veloL[i] * dmu_bot_dF * shear_bot);
             
             dq_dF[i] += - -2.0 / pow(gradP_mag , 3.0) * dgradP_mag_dF * gradP_tangent[i] / 2.0 
          		       * (pow(mu_top , 2.0) * pow(shear_top , 3.0) - pow(mu_bot , 2.0) 
          		    		   * pow(shear_bot , 3.0) - (int_tan_top - int_tan_bot));
             
             dq_dF[i] += -1.0 / pow(gradP_mag , 2.0) * dgradP_tangent_dF[i] / 2.0 
                       		       * (pow(mu_top , 2.0) * pow(shear_top , 3.0) - pow(mu_bot , 2.0) 
                       		    		   * pow(shear_bot , 3.0) - (int_tan_top - int_tan_bot)); 
             
             dq_dF[i] += -1.0 / pow(gradP_mag , 2.0) * gradP_tangent[i] / 2.0 
                            * (2.0 * mu_top * dmu_top_dF * pow(shear_top , 3.0) - 2.0 * mu_bot * dmu_bot_dF 
                                    * pow(shear_bot , 3.0) - (dint_tan_top_dF - dint_tan_bot_dF)); 
                          
             
             dq_dF[i] += - - 2.0 / pow(gradP_mag  , 3.0) * dgradP_mag_dF * gradP_normal[i] * cross_shear 
          		     * (mu_top * pow(shear_top , 2.0) - mu_bot * pow(shear_bot , 2.0)
           		           - (int_norm_top - int_norm_bot)); 
             
             dq_dF[i] += -1.0 / pow(gradP_mag  , 2.0) * dgradP_normal_dF[i] * cross_shear 
                       		     * (mu_top * pow(shear_top , 2.0) - mu_bot * pow(shear_bot , 2.0)
                        		           - (int_norm_top - int_norm_bot)); 
             
             dq_dF[i] += -1.0 / pow(gradP_mag  , 2.0) * gradP_normal[i] * cross_shear 
                       		     * (dmu_top_dF * pow(shear_top , 2.0) - dmu_bot_dF * pow(shear_bot , 2.0)
                        		           - (dint_norm_top_dF - dint_norm_bot_dF));
             
             dq_dF[i] *= phi_j;

             dv_avg_dF[i] = dq_dF[i] / H; 
     ...
     LubAux->dgradP_mag_dP = dgradP_mag_dP;
     LubAux->dgradP_mag_df = dgradP_mag_dF;
     ...
     for (i = 0; i < DIM; i++)
        {
         ...
         LubAux->dgradP_tangent_df[i] = dgradP_tangent_dF[i];
         LubAux->dgradP_normal_df[i] = dgradP_normal_dF[i];
         

         for ( j = 0; j < ei[pg->imtrx]->dof[SHELL_SHEAR_TOP]; j++) 
            {
             LubAux->dq_dshear_top[i][j] = dq_dshear_top[i];
             LubAux->dv_avg_dshear_top[i][j] = dv_avg_dshear_top[i];
            }
         for ( j = 0; j < ei[pg->imtrx]->dof[SHELL_SHEAR_BOT]; j++) 
            {
             LubAux->dq_dshear_bot[i][j] = dq_dshear_bot[i];
             LubAux->dv_avg_dshear_bot[i][j] = dv_avg_dshear_bot[i];
            }
         for ( j = 0; j < ei[pg->imtrx]->dof[SHELL_CROSS_SHEAR]; j++) 
            {
             LubAux->dq_dcross_shear[i][j] = dq_dcross_shear[i];
             LubAux->dv_avg_dcross_shear[i][j] = dv_avg_dcross_shear[i];
            }
         for ( j = 0; j < ei[pg->imtrx]->dof[LUBP]; j++) 
            {
             LubAux->dq_dp1[i][j] = dq_dP[i];
             LubAux->dv_avg_dp1[i][j] = dv_avg_dP[i];
            }
         
         for ( j = 0; j < ei[pg->imtrx]->dof[FILL]; j++) 
            {
             LubAux->dq_df[i][j] = dq_dF[i];
             LubAux->dv_avg_df[i][j] = dv_avg_dF[i];
            }
        }
